...
namespace RISEFireProductionFP64
{
	namespace
	{
		...
		void HashFluxValues( std::uint64_t& hash, const std::vector<double>& values )
		{
			HashEOSUInt64(hash,values.size());
			for(const double value:values)HashEOSFloat(hash,value);
		}
			...
			const std::array<std::vector<double>,3>& velocity )
		{
			...
			for(const unsigned char byte:domain)HashEOSByte(hash,byte);
			for(const std::vector<double>& axis:velocity)HashFluxValues(hash,axis);
			return hash;
		}

		bool SameFloatVectorBits( const std::vector<double>& first,
			const std::vector<double>& second )
		{
			if(first.size()!=second.size())return false;
			for(std::size_t value=0u;value<first.size();++value)if(std::memcmp(
				&first[value],&second[value],sizeof(double))!=0)return false;
			return true;
		}
			...
			const FireProductionScalarPhysicalFluxPrerequisiteRequest& physical )
		{
			...
			for(const std::vector<double>& axis:advective.frozenVelocityMPerS)
				HashFluxValues(hash,axis);
			for(const double value:advective.ambient)HashEOSFloat(hash,value);
			...
			for(const double value:advective.enthalpyBoundsJPerKG)HashEOSFloat(hash,value);
			...
			const FireProductionScalarFCTRequest& request )
		{
			...
			for(const double value:request.ambient)HashEOSFloat(hash,value);
			...
			for(const double value:request.enthalpyBoundsJPerKG)HashEOSFloat(hash,value);
			...
			const FireProductionScalarFCTRequest& request )
		{
			...
			for(const std::vector<double>& axis:request.frozenVelocityMPerS)
				HashFluxValues(hash,axis);
			...
			const std::array<std::vector<double>,3>& alpha )
		{
			...
			for(const std::vector<double>& axis:alpha)HashFluxValues(hash,axis);
			return hash;
		}
	...
	bool FireProductionFrozenSourcePacketSealMatchesBeginningState(
		...
		const FireProductionProjectionShape& shape,
		const std::vector<double>& conservativeValues,
		const std::vector<double>& temperatureK,std::string* error )
	{
		...
		const FireProductionProjectionShape& shape,std::uint64_t& bytes )
	{
		...
		if(shape.nx<4u||shape.nx>1024u||shape.ny<4u||shape.ny>1024u||
			...
			!(shape.cellWidthM>0.0))return false;
		...
		return AddMetalValueBuffer(3u*cells,sizeof(double),bytes)&&
			AddMetalValueBuffer(1u,sizeof(std::uint64_t),bytes)&&
			AddMetalValueBuffer(7u*32u+64u,sizeof(double),bytes)&&
			AddMetalValueBuffer(6u*(1u+5u*128u),sizeof(double),bytes)&&
		...
		std::string* error )
	{
		...
		const std::uint64_t allFaces=FireProductionProjectionFaceCount(shape,0u)+
			FireProductionProjectionFaceCount(shape,1u)+
			FireProductionProjectionFaceCount(shape,2u);
		if(!AddBytes(allFaces,30u*sizeof(double),payloadBytes)){
	...
	namespace
	{
			...
			const FireProductionScalarHeunFluxStage& stage,std::string* error )
		{
			...
			if(shape.nx<4u||shape.nx>1024u||shape.ny<4u||shape.ny>1024u||
				shape.nz<4u||shape.nz>1024u||!(shape.cellWidthM>0.0)||
				!std::isfinite(shape.cellWidthM)||!(pair.timeStepS>0.0)||
			...
			for(const double value:pair.lowFlux)if(!std::isfinite(value))
				return Fail(error,"scalar Heun composite low flux is nonfinite");
			for(const double value:pair.fluxDelta)if(!std::isfinite(value))
				return Fail(error,"scalar Heun composite flux delta is nonfinite");
			for(const double value:stage.physicalMassFluxKGPerM2S)if(!std::isfinite(value))
				return Fail(error,"scalar Heun physical mass flux is nonfinite");
			for(const double value:stage.physicalEnergyFluxWPerM2)if(!std::isfinite(value))
				return Fail(error,"scalar Heun physical energy flux is nonfinite");
			for(unsigned int axis=0u;axis<3u;++axis){
				for(const double value:stage.physicalGasFluxKGPerM2S[axis])if(!std::isfinite(value))
					return Fail(error,"scalar Heun physical gas flux is nonfinite");
				for(const double value:stage.advectiveGasLowFluxKGPerM2S[axis])if(!std::isfinite(value))
					return Fail(error,"scalar Heun advective gas low flux is nonfinite");
				for(const double value:stage.advectiveGasFluxDeltaKGPerM2S[axis])if(!std::isfinite(value))
				...
				if(pair.boundary[2u*axis]==FireProductionProjectionPeriodic){
					...
					for(std::size_t z=0u;z<zEnd;++z)for(std::size_t y=0u;y<yEnd;++y)
						for(std::size_t x=0u;x<xEnd;++x){
							...
							auto same=[](const double a,const double b){return std::memcmp(
								&a,&b,sizeof(double))==0;};
			...
			for(unsigned int axis=0u;axis<3u;++axis){const std::size_t faces=
				FireProductionProjectionFaceCount(shape,axis);for(std::size_t face=0u;
					...
					if(stage.role==FireProductionScalarHeunFluxRole::HeunAverage){
						...
						sizeof(double))!=0)return Fail(error,
		...
		FireProductionScalarHeunFluxStage& result,std::string* error )
	{
		...
		try {
			...
			if(!QueryFireProductionScalarHeunFluxStageCPUPayloadBytes(
				...
				FireProductionProjectionFaceCount(shape,0u)+
				FireProductionProjectionFaceCount(shape,1u)+
				FireProductionProjectionFaceCount(shape,2u),28u*sizeof(double),workingSetBytes))
			...
			for(std::size_t component=0u;component<9u;++component)if(std::memcmp(
				...
				sizeof(double))!=0)return Fail(error,"scalar Heun raw stage ambient differs");
			...
			const RISE::FireSimulationMethaneRecord& record=
				RISE::FireSimulationMethaneRecord::PhysicalV1();
			if(!(advectiveFluxPair.timeStepS>0.0)||!std::isfinite(
			...
			for(const double value:advectiveFluxPair.lowFlux)if(!std::isfinite(value))
				return Fail(error,"scalar Heun advective low flux is nonfinite");
			for(const double value:advectiveFluxPair.fluxDelta)if(!std::isfinite(value))
				return Fail(error,"scalar Heun advective flux delta is nonfinite");
			for(const double value:physicalFlux.physicalMassFluxKGPerM2S)
				if(!std::isfinite(value))return Fail(error,
					"scalar Heun physical mass input is nonfinite");
			for(const double value:physicalFlux.physicalEnergyFluxWPerM2)
				if(!std::isfinite(value))return Fail(error,
					"scalar Heun physical energy input is nonfinite");
			for(unsigned int axis=0u;axis<3u;++axis)for(const double value:
			...
			for(unsigned int axis=0u;axis<3u;++axis){
				const std::size_t faces=FireProductionProjectionFaceCount(shape,axis);
				computed.advectiveGasLowFluxKGPerM2S[axis].assign(faces,0.0);
				computed.advectiveGasFluxDeltaKGPerM2S[axis].assign(faces,0.0);
				for(std::size_t face=0u;face<faces;++face){const std::size_t packed=
					expectedOffset[axis]+face;double physicalGas=0.0,advectiveLow=0.0,
						advectiveDelta=0.0;
					for(std::size_t component=1u;component<=6u;++component){
		...
		FireProductionScalarProjectionTargetSeal& result,std::string* error )
	{
		...
		try {
			...
			if(!pressureOpen&&!computed.targetPerS_.empty()){
				double mean=0.0;for(const double value:computed.targetPerS_)
					mean+=static_cast<double>(value);
				mean/=static_cast<double>(computed.targetPerS_.size());
				for(double& value:computed.targetPerS_){
					value=static_cast<double>(static_cast<double>(value)-mean);
		...
		FireProductionScalarProjectionTargetSeal& result,std::string* error )
	{
		...
		try {
			...
			for(std::size_t cell=0u;cell<cells;++cell){
				double value=static_cast<double>(corrected[cell]-mean);
				if(!std::isfinite(value))return Fail(error,
		...
		FireProductionScalarProjectionTargetSeal& result,std::string* error )
	{
		...
		try {
			...
			computed.targetPerS_.assign(cells,0.0);
			...
			for(std::size_t z=0u;z<shape.nz;++z)for(std::size_t y=0u;y<shape.ny;++y)
				for(std::size_t x=0u;x<shape.nx;++x){
					...
					if(!record.DivergenceFromDiscreteRateByComponentOrder(state.data(),state.size(),
						...
						RISE::FireStateProducerPrecision::Binary32,physicalPerS,error))return false;
					double target=static_cast<double>(physicalPerS+
		...
		FireProductionScalarHeunFluxStage& result,std::string* error )
	{
		...
		try {
			...
			auto average=[](const std::vector<double>& x,const std::vector<double>& y,
				std::vector<double>& output){output.resize(x.size());for(std::size_t value=0u;
					value<x.size();++value){const double mean=0.5*(x[value]+y[value]);
			...
			const double epsilon32=static_cast<double>(std::numeric_limits<double>::epsilon()),
		...
		try {
			...
			for( unsigned int axis=0u;axis<3u;++axis ) {
				...
				for( std::size_t face=0u;face<faces;++face ) {
					...
					const double second=request.highGasFluxKGPerM2S[axis][face];
			...
			for( unsigned int axis=0u;axis<3u;++axis ) {
				...
				for( std::size_t face=0u;face<faces;++face ) {
					...
					const double physical=request.physicalGasFluxKGPerM2S[axis].empty()?0.0:
						request.physicalGasFluxKGPerM2S[axis][face];
					const double delta=secondFluxIsDelta?second:second-low;
					const double accepted=low+request.sharedFaceAlpha[axis][face]*delta+
						physical;
