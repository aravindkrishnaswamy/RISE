...
namespace RISEFireProductionFP64
{
	...
	namespace
	{
		...
		bool SameOwnerFloatBits(const std::array<std::vector<double>,3>& a,
			const std::array<std::vector<double>,3>& b)
		{
			for(unsigned int axis=0u;axis<3u;++axis)
		...
		FireProductionProjectedHeunCoupledStageResult& result,std::string* error)
	{
		...
		try {
			...
			auto buildStage=[&](const FireProductionProjectionResult& projection,
				...
				std::vector<double> contextState=state,contextTemperature=temperatureK;
				std::array<std::vector<double>,3> contextVelocity=projection.velocityMPerS;
			...
			auto canonicalProjection=[&](const FireProductionScalarProjectionTargetSeal& ownerTarget,
				...
				std::size_t selected=0u;double selectedDiscrepancy=0.0;bool haveSelected=false;
		...
		FireProductionProjectedHeunOwnerResult& result,std::string* error)
	{
		...
		try {
			...
			auto endpointFlux=[&](const FireProductionProjectionResult& projected,
				...
				std::vector<double> contextState=work_.conservativeValues;
				std::vector<double> contextTemperature=work_.committedEOS.temperatureK;
				std::array<std::vector<double>,3> contextVelocity=projected.velocityMPerS;
				FireProductionProjectedHeunTransportContext context;
			...
			const std::array<std::vector<double>,3>* bootstrapDiscrepancyVelocity=
				&bootstrapProjection.velocityMPerS;
			std::array<std::vector<double>,3> injectedBootstrapDiscrepancyVelocity;
			if(OwnerTestFailure("r2-bootstrap-used-class-discrepancy")){
				...
				for(unsigned int side=0u;side<6u;++side)if(
					...
					const double outward=bootstrapClass[side][0u]?
						request_.endpointVelocityToleranceMPerS+0.25:
						-request_.endpointVelocityToleranceMPerS-0.25;
					injectedBootstrapDiscrepancyVelocity[axis][FaceIndex(
						shape,axis,x,y,z)]=(positive?1.0:-1.0)*outward;
			...
			const double bootstrapTrajectoryMaximum=OwnerOpenClassDiscrepancy(shape,
			...
			for(std::uint32_t iteration=0u;iteration<request_.maximumPicardIterations;
				...
				std::array<std::vector<double>,3> injectedDiscrepancyVelocity;
				if(OwnerTestFailure("r2-used-class-discrepancy")){
					...
					for(unsigned int side=0u;side<6u;++side)if(
						...
						const std::size_t y=axis==1u?(positive?shape.ny:0u):0u;
						const std::size_t z=axis==2u?(positive?shape.nz:0u):0u;
						const double outward=projectedClass[side][0u]?
							request_.endpointVelocityToleranceMPerS+0.25:
							-request_.endpointVelocityToleranceMPerS-0.25;
						injectedDiscrepancyVelocity[axis][FaceIndex(shape,axis,x,y,z)]=
							(positive?1.0:-1.0)*outward;
						break;
					}
