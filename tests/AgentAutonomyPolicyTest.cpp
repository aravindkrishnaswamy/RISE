...
static void TestMcpLayer()
{
	...
	// "--agent-autonomy=read" reason the other three share.
	{
		...
		Check( tools.isArray() && tools.size() == 40,
		       "tools/list under Read STILL lists all 40 tools (mutating tools are ANNOTATED, not hidden)" );
	...
	// where resolve_proposal actually reaches the session).
	{
		...
		const JsonValue& tools = env.get( "result" ).get( "tools" );
		Check( tools.isArray() && tools.size() == 40, "tools/list under Commit lists all 40 tools" );
		int annotatedCount = 0;
		for( std::size_t i = 0; i < tools.size(); ++i ) {
	...
	// no textual clue that its edit only STAGES rather than commits.
	{
		...
		const JsonValue& tools = env.get( "result" ).get( "tools" );
		Check( tools.isArray() && tools.size() == 40, "tools/list under Propose lists all 40 tools" );
