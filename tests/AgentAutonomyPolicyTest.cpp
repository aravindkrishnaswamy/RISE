...
static void TestMcpLayer()
{
	...
	// "--agent-autonomy=read" reason the other three share.
	{
		...
		Check( tools.isArray() && tools.size() == 31,
		       "tools/list under Read STILL lists all 31 tools (mutating tools are ANNOTATED, not hidden)" );
		...
		Check( annotatedCount == 12, "EXACTLY 12 tool descriptions carry the generic read-refusal note under Read (the mutating set incl. propose_patches/insert_chunks/remove_chunks/insert_material_scaffold/insert_geometry_scaffold/replace_geometry_scaffold, S2 (2026-08-11) build_element/place_element and -- arc 81, 2026-08-12 -- light_scene, no more no less; resolve_proposal has its own distinct note)" );
	}
	...
	// where resolve_proposal actually reaches the session).
	{
		...
		const JsonValue& tools = env.get( "result" ).get( "tools" );
		Check( tools.isArray() && tools.size() == 31, "tools/list under Commit lists all 31 tools" );
		int annotatedCount = 0;
		for( std::size_t i = 0; i < tools.size(); ++i ) {
	...
	// no textual clue that its edit only STAGES rather than commits.
	{
		...
		const JsonValue& tools = env.get( "result" ).get( "tools" );
		Check( tools.isArray() && tools.size() == 31, "tools/list under Propose lists all 31 tools" );
