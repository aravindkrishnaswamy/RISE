...
int main()
{
	...
	// ---- OTG (arc-86 slice 1, fix F1): a GROUPED member's transform commit must ROUND-TRIP. -------------
	//      `group` composes its matrix G into every listed member at derive time, so the member's
	//      GetFinalTransformMatrix() is `G * M`.  This commit writes that matrix to the member's `matrix`
	//      param, and the re-derive applies BOTH the authored matrix AND the group's push again -- so
	//      committing the composed matrix verbatim re-derives to `G * G * M`, and EVERY subsequent gizmo
	//      drag or panel edit compounds another G, without bound.  The fix divides G back out at commit
	//      time (writing `G^-1 * final`), so `G * (G^-1 * final) == final`.
	//
	//      WHICH ASSERTION ACTUALLY CATCHES THE BUG (red-proven 2026-08-15 by making the commit skip the
	//      G division): OTG2 and OTG3 fail; OTG1 does NOT.  OTG1 only compares two successive derives of
	//      the SAME Document, and a Document that already holds the squared matrix re-derives to that same
	//      squared matrix every time -- self-consistent and wrong.  The discriminating assertions are the
	//      ABSOLUTE pose check (OTG2) and the repeated-commit drift check (OTG3).  Do not "simplify" this
	//      case down to OTG1.
	{
		const char* tg = "cst_s3_objxform_group.RISEscene";
		{ std::ofstream o( tg );
		  o << "RISE ASCII SCENE 7\n"
		       "scene_variant\n{\nname night\n}\n"
		       "uniformcolor_painter\n{\nname p1\ncolor 1 0 0\n}\n"
		       ...
		       "sphere_geometry\n{\nname g\nradius 1\n}\n"
		       "standard_object\n{\nname o\ngeometry g\nmaterial m\norientation 30 0 0\nposition 0 0 0\n}\n"
		       // A group with BOTH a rotation and a translation, so G does not commute with the
		       // member's own transform and a lost/doubled G cannot cancel out numerically.
		       "group\n{\nname grp\nmember o\nposition 2 0 -1\norientation 0 45 0\n}\n"; }
		Job* j = new Job();
		Check( j->LoadAsciiSceneViaCst( tg ), "OTG: loads the grouped-object variant scene via CST" );
		SceneEditController c( *j, 0 );
		c.SetSelection( Cat::Object, String( "o" ) );

		// Sanity: the object really IS indexed as a group member (else this whole case is vacuous).
		Check( j->GetGroupMembership( "o", nullptr, nullptr, 0 ),
		       "OTG: the object is registered in the group-membership side index" );

		double authored[16]; ObjMat16( *j, "o", authored );
		// OTG1: one panel position edit -> live mutate + commit + re-derive.  Read the live matrix, then
		// force ANOTHER full re-derive (a material edit on a variant scene) and require it to reproduce
		// the SAME matrix.  Pre-fix the second derive lands on G*G*M and this fails.
		Check( c.SetPropertyForCategory( Cat::Object, String( "position" ), String( "0 5 0" ) ), "OTG1: grouped-member position edit applies" );
		double moved[16]; ObjMat16( *j, "o", moved );
		Check( !Mat16Eq( authored, moved ), "OTG1: the position edit changed the grouped member's transform" );
		c.SetSelection( Cat::Material, String( "m" ) );
		Check( c.SetPropertyForCategory( Cat::Material, String( "reflectance" ), String( "p2" ) ), "OTG1: material edit applies (D2)" );
		double afterD2[16]; ObjMat16( *j, "o", afterD2 );
		Check( Mat16Eq( moved, afterD2 ), "OTG1: the grouped member's transform ROUND-TRIPS through the D2 (G divided out at commit)" );

		// OTG2: the round-tripped pose is the EXPECTED one, not merely self-consistent -- a commit that
		//       divided out the WRONG matrix could still round-trip.  Two independent halves:
		//         * the LINEAR part must be groupRotation * memberRotation, i.e. the group's rotation is
		//           still applied EXACTLY ONCE (twice would give rotY(90), zero would give plain Rx(30));
		//         * the TRANSLATION must be exactly the requested (0,5,0) -- SetObjectPosition is an
		//           absolute WORLD-space contract that already corrects for outer transform-stack
		//           matrices (SceneEditor.cpp's SetObjectPosition arm), so the group's own translation is
		//           deliberately absorbed, and it must not reappear on the re-derive.
		{
			const Matrix4 linearM = Matrix4Ops::YRotation( 45.0 * PI / 180.0 )
			                      * Matrix4Ops::XRotation( 30.0 * PI / 180.0 );
			const Scalar* lp = &linearM._00;
			const Scalar* ap = 0;
			Matrix4 actualM;
			{
				Scalar* w = &actualM._00;
				for( int q = 0; q < 16; ++q ) w[q] = static_cast<Scalar>( afterD2[q] );
				ap = &actualM._00;
			}
			bool linearOk = true;
			for( int col = 0; col < 3; ++col ) for( int row = 0; row < 3; ++row ) {
				const int k = col * 4 + row;
				if( std::fabs( static_cast<double>( lp[k] ) - static_cast<double>( ap[k] ) ) > 1e-9 ) linearOk = false;
			}
			Check( linearOk, "OTG2: the round-tripped ROTATION is groupRotation * memberRotation (G applied EXACTLY once)" );
			Check( std::fabs( afterD2[12] - 0.0 ) <= 1e-9
			    && std::fabs( afterD2[13] - 5.0 ) <= 1e-9
			    && std::fabs( afterD2[14] - 0.0 ) <= 1e-9,
			       "OTG2: the round-tripped WORLD position is exactly the requested 0 5 0 (SetObjectPosition's world contract)" );
		}

		// OTG3: REPEATED commits do not drift.  Re-issuing the SAME position value still routes a full
		// commit; pre-fix each one multiplied in another G, so the divergence compounded per edit.
		c.SetSelection( Cat::Object, String( "o" ) );
		for( int rep = 0; rep < 3; ++rep ) {
			Check( c.SetPropertyForCategory( Cat::Object, String( "position" ), String( "0 5 0" ) ), "OTG3: repeat position edit applies" );
		}
		double afterRepeats[16]; ObjMat16( *j, "o", afterRepeats );
		Check( Mat16Eq( moved, afterRepeats ),
		       "OTG3: repeated commits of the same pose do NOT compound the group transform (no unbounded G^n drift)" );
		j->release();
		std::remove( tg );
	}

	// ---- OTG-CSG (H4 hardening, 2026-08-15): a grouped CSG_OBJECT member (cstKind == 2) round-trips through
	//      the SAME commit path OTG above pins for a standard_object (cstKind == 1) member.  No prior case
	//      exercised a csg_object as a group member, which is the only path that runs the gateM/DecomposeRigid
	//      interaction (SceneEditor.cpp's ApplyForwardMutation cstKind==2 branch, and
	//      CommitPendingCstObjectTransforms's kind==2 branch) against a matrix that ALSO has a group's G divided
	//      out of it first.  csg_object has no `matrix` param -- only `position`/`orientation` -- so the group
	//      here is deliberately rotation+translation ONLY (no scale): both G and the csg's own composed transform
	//      must stay RIGID (no shear/scale) for DecomposeRigid to succeed at all, which is exactly the domain this
	//      gate exists for; a scaled group would need CSG3/CSG4-style refusal coverage instead of a round trip. ----
	{
		const char* tg = "cst_s3_objxform_group_csg.RISEscene";
		{ std::ofstream o( tg );
		  o << "RISE ASCII SCENE 7\n"
		       "scene_variant\n{\nname night\n}\n"
		       "uniformcolor_painter\n{\nname p1\ncolor 1 0 0\n}\n"
		       ...
		       "standard_object\n{\nname b\ngeometry g\nmaterial m\nposition 1 0 0\n}\n"
		       "csg_object\n{\nname cu\nobja a\nobjb b\noperation union\nmaterial m\nposition 1 0 0\norientation 0 0 30\n}\n"
		       // Rotation + translation only -- no `scale` (csg_object cannot represent one anyway,
		       // and DecomposeRigid needs the composed transform to stay shear/scale-free).
		       "group\n{\nname grp\nmember cu\nposition 2 0 -1\norientation 0 45 0\n}\n"; }
		Job* j = new Job();
		Check( j->LoadAsciiSceneViaCst( tg ), "OTG-CSG: loads the grouped-csg-object variant scene via CST" );
		SceneEditController c( *j, 0 );

		Check( j->GetGroupMembership( "cu", nullptr, nullptr, 0 ),
		       "OTG-CSG: the csg_object is registered in the group-membership side index" );

		c.SetSelection( Cat::Object, String( "cu" ) );
		double authored[16]; ObjMat16( *j, "cu", authored );

		// A POSITION edit on a grouped csg_object: forward-mutate gate (gateM/DecomposeRigid) must succeed
		// against the GROUP-LOCAL matrix, and the commit must decompose that same group-local matrix to
		// `position`/`orientation` -- not the composed (G * M) one, else the re-derive would double-apply G.
		Check( c.SetPropertyForCategory( Cat::Object, String( "position" ), String( "0 5 0" ) ),
		       "OTG-CSG1: a position edit on a grouped csg_object member applies (gateM/DecomposeRigid succeeds)" );
		double moved[16]; ObjMat16( *j, "cu", moved );
		Check( !Mat16Eq( authored, moved ), "OTG-CSG1: the position edit changed the grouped csg member's transform" );
		c.SetSelection( Cat::Material, String( "m" ) );
		Check( c.SetPropertyForCategory( Cat::Material, String( "reflectance" ), String( "p2" ) ), "OTG-CSG1: material edit applies (D2)" );
		double afterD2[16]; ObjMat16( *j, "cu", afterD2 );
		Check( Mat16Eq( moved, afterD2 ),
		       "OTG-CSG1: the grouped csg member's transform ROUND-TRIPS through the D2 (G divided out at commit, same as a standard_object member)" );

		// An ORIENTATION edit, same round-trip discipline (CSG2's rotate case, now under a group).
		c.SetSelection( Cat::Object, String( "cu" ) );
		Check( c.SetPropertyForCategory( Cat::Object, String( "orientation" ), String( "0 0 90" ) ),
		       "OTG-CSG2: an orientation edit on a grouped csg_object member applies" );
		double rotated[16]; ObjMat16( *j, "cu", rotated );
		Check( !Mat16Eq( afterD2, rotated ), "OTG-CSG2: the orientation edit rotated the grouped csg member" );
		Check( c.SetPropertyForCategory( Cat::Material, String( "reflectance" ), String( "p1" ) ), "OTG-CSG2: material edit applies (D2)" );
		double afterD2b[16]; ObjMat16( *j, "cu", afterD2b );
		Check( Mat16Eq( rotated, afterD2b ),
		       "OTG-CSG2: the grouped csg member's orientation edit ROUND-TRIPS through the D2 too" );

		// REPEATED commits of the same pose do not drift (OTG3's discipline, on the csg component-commit path).
		c.SetSelection( Cat::Object, String( "cu" ) );
		for( int rep = 0; rep < 3; ++rep ) {
			Check( c.SetPropertyForCategory( Cat::Object, String( "position" ), String( "0 5 0" ) ), "OTG-CSG3: repeat position edit applies" );
		}
		double afterRepeats[16]; ObjMat16( *j, "cu", afterRepeats );
		Check( c.SetPropertyForCategory( Cat::Material, String( "reflectance" ), String( "p2" ) ), "OTG-CSG3: material edit applies (D2)" );
		double afterRepeatsD2[16]; ObjMat16( *j, "cu", afterRepeatsD2 );
		Check( Mat16Eq( afterRepeats, afterRepeatsD2 ),
		       "OTG-CSG3: repeated commits of the same pose on a grouped csg member do NOT compound the group transform" );
		j->release();
		std::remove( tg );
	}
