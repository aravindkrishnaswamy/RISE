...
namespace FireProductionRoundoffAdapter
{
	...
	// Frozen r136 certificate arithmetic from reviewed 034d4af5. It propagates
	// density construction uncertainty, whereas the r201 resident-owner gate
	// begins at already-rounded private density inputs. Neither replaces the
	// other's scope. Only the explicitly historical diagnostic selects this.
	inline bool EvaluateHistoricalR136ProjectionStreamingEvidence(
		const Trace::FireProductionProjectionRequest& request,
		const Trace::FireProductionProjectionResult& projection,const bool restoration,
		ResidentStepTraceResult::ProjectionStreamingEvidence& evidence)
	{
	...
	inline bool AdvanceResidentStepTrace(const RISE::FireProductionResidentStepRequest& request,
		...
		const bool historicalR136Certificate=false)
	{
		...
		for(unsigned int side=0u;side<6u;++side)projection.boundary[side]=force.boundary[side];
		{
			...
			if(!(historicalR136Certificate?EvaluateHistoricalR136ProjectionStreamingEvidence:
				EvaluateProjectionStreamingEvidence)(projection,computed.physicalProjection,
		...
		projection.divergenceTargetPerS=Promote(request.restorationDivergenceTargetPerS);
		{
			...
			if(!Trace::ProjectFireProductionRestorationCPU(projection,computed.projection,error))
				return false;
			if(!(historicalR136Certificate?EvaluateHistoricalR136ProjectionStreamingEvidence:
				EvaluateProjectionStreamingEvidence)(projection,computed.projection,true,
