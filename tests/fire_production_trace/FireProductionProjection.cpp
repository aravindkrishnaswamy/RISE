...
namespace RISEFireProductionTrace
{
	namespace
	{
		...
		{
			std::size_t nx,ny,nz;
			FireProductionRoundoffTrace::TraceFloat spacing[3];
			std::vector<FireProductionRoundoffTrace::TraceFloat> density,rhs,pressure,temporary,residual,diagonal;
			std::array<std::vector<FireProductionRoundoffTrace::TraceFloat>,3> inverseFaceDensity;
		};
		...
		FireProductionRoundoffTrace::TraceFloat BlellochSum( const std::vector<FireProductionRoundoffTrace::TraceFloat>& values )
		{
		...
		FireProductionRoundoffTrace::TraceFloat RemoveMean( std::vector<FireProductionRoundoffTrace::TraceFloat>& values )
		{
			const FireProductionRoundoffTrace::TraceFloat mean=BlellochSum(values)/static_cast<FireProductionRoundoffTrace::TraceFloat>(values.size());
			for( FireProductionRoundoffTrace::TraceFloat& value : values ) value-=mean;
			return mean;
		}
		...
		FireProductionRoundoffTrace::TraceFloat ArithmeticMean( FireProductionRoundoffTrace::TraceFloat first, FireProductionRoundoffTrace::TraceFloat second )
		{
			return 0.5f*first+0.5f*second;
		}

		FireProductionRoundoffTrace::TraceFloat StoredFaceDensity( const Level& level,
			const std::array<FireProductionProjectionBoundary,6>& boundary,
			FireProductionRoundoffTrace::TraceFloat ambientDensity, unsigned int axis,
			std::size_t x, std::size_t y, std::size_t z )
		{
			...
			const FireProductionRoundoffTrace::TraceFloat interior=level.density[CellIndex(level.nx,level.ny,
				highX,highY,highZ)];
		...
		void BuildLevelCoefficients( Level& level,
			const std::array<FireProductionProjectionBoundary,6>& boundary,
			FireProductionRoundoffTrace::TraceFloat ambientDensity )
		{
			...
			for( std::size_t z=0;z<nz;++z ) for( std::size_t y=0;y<ny;++y )
				for( std::size_t x=0;x<=nx;++x ) {
					...
					if( x>0u&&x<nx ) level.inverseFaceDensity[0][face]=1.0f/
						...
						if( boundary[side]==FireProductionProjectionPeriodic ) {
							const FireProductionRoundoffTrace::TraceFloat value=1.0f/
								StoredFaceDensity(level,boundary,ambientDensity,0u,x,y,z);
			...
			for( std::size_t z=0;z<nz;++z ) for( std::size_t y=0;y<ny;++y )
				for( std::size_t x=0;x<nx;++x ) {
					FireProductionRoundoffTrace::TraceFloat diagonal=0.0f;
					...
					for( unsigned int axis=0;axis<3u;++axis ) {
						...
						if( axis==1u ) ++highY;
						if( axis==2u ) ++highZ;
						const FireProductionRoundoffTrace::TraceFloat inverseSpacing2=1.0f/(level.spacing[axis]*level.spacing[axis]);
						const FireProductionRoundoffTrace::TraceFloat low=level.inverseFaceDensity[axis][FaceIndex(nx,ny,
							axis,lowX,lowY,lowZ)]*inverseSpacing2;
						const FireProductionRoundoffTrace::TraceFloat high=level.inverseFaceDensity[axis][FaceIndex(nx,ny,
		...
		void ApplyOperator( const Level& level,
			const std::array<FireProductionProjectionBoundary,6>& boundary,
			const std::vector<FireProductionRoundoffTrace::TraceFloat>& pressure, std::vector<FireProductionRoundoffTrace::TraceFloat>& output )
		{
			const std::size_t nx=level.nx,ny=level.ny,nz=level.nz;
			...
			for( std::size_t z=0;z<nz;++z ) for( std::size_t y=0;y<ny;++y )
				for( std::size_t x=0;x<nx;++x ) {
					...
					FireProductionRoundoffTrace::TraceFloat value=0.0f;
					const std::size_t coordinate[3]={x,y,z};
					const std::size_t extent[3]={nx,ny,nz};
					for( unsigned int axis=0;axis<3u;++axis ) {
						...
						if( axis==1u ) ++hy;
						if( axis==2u ) ++hz;
						const FireProductionRoundoffTrace::TraceFloat scale=1.0f/(level.spacing[axis]*level.spacing[axis]);
						const FireProductionRoundoffTrace::TraceFloat low=level.inverseFaceDensity[axis][FaceIndex(nx,ny,
							axis,lx,ly,lz)]*scale;
						const FireProductionRoundoffTrace::TraceFloat high=level.inverseFaceDensity[axis][FaceIndex(nx,ny,
							axis,hx,hy,hz)]*scale;
		...
		{
			const FireProductionRoundoffTrace::TraceFloat omega=2.0f/3.0f;
		...
		void RestrictAverage( const Level& fine, Level& coarse,
			const std::vector<FireProductionRoundoffTrace::TraceFloat>& values, std::vector<FireProductionRoundoffTrace::TraceFloat>& restricted )
		{
			...
			for( std::size_t cell=0;cell<restricted.size();++cell )
				restricted[cell]/=static_cast<FireProductionRoundoffTrace::TraceFloat>(count[cell]);
		}

		void InterpolationCoordinate( std::size_t fine, std::size_t fineExtent,
			std::size_t coarseExtent, std::size_t& first, std::size_t& second, FireProductionRoundoffTrace::TraceFloat& weight )
		{
			FireProductionRoundoffTrace::ProjectionInterpolationScope topologyScope(fine,fineExtent,coarseExtent);
			if( fineExtent==coarseExtent ) {first=fine;second=fine;weight=0.0f;return;}
			const FireProductionRoundoffTrace::TraceFloat position=(static_cast<FireProductionRoundoffTrace::TraceFloat>(fine)+0.5f)*
				static_cast<FireProductionRoundoffTrace::TraceFloat>(coarseExtent)/static_cast<FireProductionRoundoffTrace::TraceFloat>(fineExtent)-0.5f;
			if( position<=0.0f ) {first=0u;second=0u;weight=0.0f;return;}
			if( position>=static_cast<FireProductionRoundoffTrace::TraceFloat>(coarseExtent-1u) ) {
				first=coarseExtent-1u;second=first;weight=0.0f;return;
			}
			first=static_cast<std::size_t>(std::floor(position));second=first+1u;
			weight=position-static_cast<FireProductionRoundoffTrace::TraceFloat>(first);
		}
		...
		void ProlongateAndAdd( const Level& coarse, Level& fine, FireProductionRoundoffTrace::TraceFloat damping )
		{
			for( std::size_t z=0;z<fine.nz;++z ) for( std::size_t y=0;y<fine.ny;++y )
				for( std::size_t x=0;x<fine.nx;++x ) {
					...
					FireProductionRoundoffTrace::TraceFloat wx=0.0f,wy=0.0f,wz=0.0f;
					...
					for( unsigned int iz=0;iz<2u;++iz ) for( unsigned int iy=0;iy<2u;++iy )
						for( unsigned int ix=0;ix<2u;++ix ) {
							const FireProductionRoundoffTrace::TraceFloat weight=(ix?wx:1.0f-wx)*(iy?wy:1.0f-wy)*
								(iz?wz:1.0f-wz);
		...
		FireProductionRoundoffTrace::TraceFloat CellCenteredVelocity( const FireProductionProjectionRequest& request,
			const std::array<std::vector<FireProductionRoundoffTrace::TraceFloat>,3>& velocity, unsigned int axis,
			std::size_t x, std::size_t y, std::size_t z )
		{
	...
	bool FireProductionProjectionResidualWithinBand( FireProductionRoundoffTrace::TraceFloat maximumResidualPerS,
		...
		std::string* error )
	{
		...
		for( unsigned int axis=0;axis<3u;++axis ) {
			...
			for( const FireProductionRoundoffTrace::TraceFloat value : request.provisionalMomentumKGPerM2S[axis] )
		...
		for( const FireProductionRoundoffTrace::TraceFloat value : request.gasDensityKGPerM3 )
			if( !std::isfinite(value)||!(value>0.0f) )
				return Fail(error,"production projection density is invalid");
		for( const FireProductionRoundoffTrace::TraceFloat value : request.divergenceTargetPerS )
			if( !std::isfinite(value) )
		...
		std::string* error )
	{
		...
		FireProductionRoundoffTrace::TraceFloat maximumVelocity=0.0f;
		for( unsigned int axis=0;axis<3u;++axis ) {
			...
			for( std::size_t z=0;z<ez;++z ) for( std::size_t y=0;y<ey;++y )
				for( std::size_t x=0;x<ex;++x ) {
					...
					const FireProductionRoundoffTrace::TraceFloat density=StoredFaceDensity(fine,request.boundary,
		...
		for( unsigned int side=0;side<6u;++side ) {
			...
			for( std::size_t second=0;second<secondCount;++second )
				for( std::size_t first=0;first<firstCount;++first ) {
					...
					const std::size_t face=FaceIndex(nx,ny,axis,x,y,z);
					const FireProductionRoundoffTrace::TraceFloat outward=(positive?1.0f:-1.0f)*result.velocityMPerS[axis][face];
					...
					if( FireProductionRoundoffTrace::EvaluateBranch(FireProductionRoundoffTrace::BranchSite::OpenBoundaryActiveSet,[&](){return outward<0.0f;}) ) {
						...
						for( unsigned int tangent=0;tangent<3u;++tangent ) if( tangent!=axis ) {
							const FireProductionRoundoffTrace::TraceFloat value=CellCenteredVelocity(request,result.velocityMPerS,
		...
		auto divergence=[&](const std::array<std::vector<FireProductionRoundoffTrace::TraceFloat>,3>& velocity,
		...
		for( unsigned int axis=0;axis<3u;++axis ) for( const FireProductionRoundoffTrace::TraceFloat value:result.velocityMPerS[axis] )
			maximumVelocity=std::max(maximumVelocity,std::fabs(value));
		for( std::size_t z=0;z<nz;++z ) for( std::size_t y=0;y<ny;++y )
			for( std::size_t x=0;x<nx;++x ) {
				const std::size_t cell=CellIndex(nx,ny,x,y,z);
				const FireProductionRoundoffTrace::TraceFloat residual=execution==CPUProjectionRestoration?
				...
				for( unsigned int axis=0;axis<3u;++axis ) for( unsigned int high=0;high<2u;++high ) {
					...
					const FireProductionRoundoffTrace::TraceFloat coefficient=2.0f*fine.inverseFaceDensity[axis][
		...
		while( hierarchy.back().nx>4u||hierarchy.back().ny>4u||hierarchy.back().nz>4u ) {
			...
			coarse.spacing[0]=parent.spacing[0]*static_cast<FireProductionRoundoffTrace::TraceFloat>(parent.nx)/
				static_cast<FireProductionRoundoffTrace::TraceFloat>(coarse.nx);
			coarse.spacing[1]=parent.spacing[1]*static_cast<FireProductionRoundoffTrace::TraceFloat>(parent.ny)/
				static_cast<FireProductionRoundoffTrace::TraceFloat>(coarse.ny);
			coarse.spacing[2]=parent.spacing[2]*static_cast<FireProductionRoundoffTrace::TraceFloat>(parent.nz)/
				static_cast<FireProductionRoundoffTrace::TraceFloat>(coarse.nz);
		...
		for( unsigned int axis=0;axis<3u;++axis ) {
			...
			for( std::size_t z=0;z<ez;++z ) for( std::size_t y=0;y<ey;++y )
				for( std::size_t x=0;x<ex;++x ) {
					...
					const std::size_t coordinate=axis==0u?x:(axis==1u?y:z);
					const std::size_t extent=axis==0u?nx:(axis==1u?ny:nz);
					FireProductionRoundoffTrace::TraceFloat gradient=0.0f;
					if( coordinate>0u&&coordinate<extent ) {
						std::size_t lx=x,ly=y,lz=z,hx=x,hy=y,hz=z;
						...
						if( request.boundary[side]==FireProductionProjectionPeriodic ) {
							...
							const FireProductionRoundoffTrace::TraceFloat pb=boundaryPressure[side][SideFaceIndex(shape,side,cx,cy,cz)];
		...
		for( unsigned int side=0;side<6u;++side ) {
			...
			for( std::size_t second=0;second<secondCount;++second )
				for( std::size_t first=0;first<firstCount;++first ) {
					...
					const FireProductionRoundoffTrace::TraceFloat discrepancy=inflow?std::max(FireProductionRoundoffTrace::TraceFloat(0.0f),outward):std::max(FireProductionRoundoffTrace::TraceFloat(0.0f),-outward);
					result.maximumOpenComplementarityDiscrepancyMPerS=std::max(
						result.maximumOpenComplementarityDiscrepancyMPerS,discrepancy);
				}
		}
		const FireProductionRoundoffTrace::TraceFloat length=shape.cellWidthM*static_cast<FireProductionRoundoffTrace::TraceFloat>(
			std::max(nx,std::max(ny,nz)));
		FireProductionRoundoffTrace::TraceFloat maximumRestorationTarget=0.0f;
		...
		for( const FireProductionRoundoffTrace::TraceFloat value : result.pressurePa ) if( !std::isfinite(value) ) {
			result=FireProductionProjectionResult();
			return Fail(error,"production projection produced nonfinite pressure");
		}
		for( unsigned int axis=0;axis<3u;++axis ) {
			for( const FireProductionRoundoffTrace::TraceFloat value:result.faceDensityKGPerM3[axis] )
				if( !std::isfinite(value)||!(value>0.0f) ) {
					result=FireProductionProjectionResult();
					return Fail(error,"production projection produced invalid face density");
				}
			for( const FireProductionRoundoffTrace::TraceFloat value:result.momentumKGPerM2S[axis] )
				if( !std::isfinite(value) ) {
					result=FireProductionProjectionResult();
