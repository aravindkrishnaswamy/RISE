...
namespace RISEFireProductionTrace
{
	...
	namespace
	{
			...
			const std::array<std::vector<FireProductionRoundoffTrace::TraceFloat>,3>& alpha)
		{
			...
			for(const std::vector<FireProductionRoundoffTrace::TraceFloat>& axis:alpha){
				OwnerHashUInt64(hash,axis.size());
				for(const FireProductionRoundoffTrace::TraceFloat value:axis)OwnerHashDouble(hash,static_cast<double>(value));
			}
			return hash;
		}
		...
		void OwnerHashFloatAxes(std::uint64_t& hash,
			const std::array<std::vector<FireProductionRoundoffTrace::TraceFloat>,3>& values)
		{
			for(const std::vector<FireProductionRoundoffTrace::TraceFloat>& axis:values)OwnerHashFloats(hash,axis);
		}
		...
		bool SameOwnerFloatBits(const std::array<std::vector<FireProductionRoundoffTrace::TraceFloat>,3>& a,
			const std::array<std::vector<FireProductionRoundoffTrace::TraceFloat>,3>& b)
		{
			for(unsigned int axis=0u;axis<3u;++axis)
		...
		FireProductionRoundoffTrace::TraceFloat OwnerTangentialCellVelocity(
			const FireProductionProjectionShape& shape,
			const std::array<std::vector<FireProductionRoundoffTrace::TraceFloat>,3>& velocity,
		...
		bool OwnerIntegratedOpenHead(const FireProductionProjectionShape& shape,
			const std::array<FireProductionProjectionBoundary,6>& boundary,
			const FireProductionRoundoffTrace::TraceFloat ambientDensity,
			const FireProductionProjectionResult& r0,
			const FireProductionProjectionResult& r1,
			std::array<std::vector<FireProductionRoundoffTrace::TraceFloat>,6>& head,std::string* error)
		{
			for(unsigned int side=0u;side<6u;++side){
				...
				for(std::size_t second=0u;second<secondCount;++second)
					for(std::size_t first=0u;first<firstCount;++first){
						...
						FireProductionRoundoffTrace::TraceFloat speed0=r0.velocityMPerS[axis][face]*r0.velocityMPerS[axis][face];
						FireProductionRoundoffTrace::TraceFloat speed1=r1.velocityMPerS[axis][face]*r1.velocityMPerS[axis][face];
						for(unsigned int tangent=0u;tangent<3u;++tangent)if(tangent!=axis){
							const FireProductionRoundoffTrace::TraceFloat u0=OwnerTangentialCellVelocity(shape,r0.velocityMPerS,
								tangent,cx,cy,cz);
							const FireProductionRoundoffTrace::TraceFloat u1=OwnerTangentialCellVelocity(shape,r1.velocityMPerS,
								tangent,cx,cy,cz);
							speed0+=u0*u0;speed1+=u1*u1;
						}
						const std::size_t index=second*firstCount+first;
						const FireProductionRoundoffTrace::TraceFloat value=-0.25f*ambientDensity*((r0.pressureOpenInflow[
		...
		std::array<std::vector<unsigned char>,6> OwnerNextOpenClass(
			...
			const std::array<std::vector<FireProductionRoundoffTrace::TraceFloat>,3>& velocity,const FireProductionRoundoffTrace::TraceFloat tolerance)
		{
			...
			for(unsigned int side=0u;side<6u;++side){
				...
				for(std::size_t second=0u;second<secondCount;++second)
					for(std::size_t first=0u;first<firstCount;++first){
						...
						if(axis==1u){x=first;y=positive?shape.ny:0u;z=second;}
						if(axis==2u){x=first;y=second;z=positive?shape.nz:0u;}
						const FireProductionRoundoffTrace::TraceFloat outward=(positive?1.0f:-1.0f)*
		...
		FireProductionRoundoffTrace::TraceFloat OwnerOpenClassDiscrepancy(
			const FireProductionProjectionShape& shape,
			const std::array<FireProductionProjectionBoundary,6>& boundary,
			const std::array<std::vector<unsigned char>,6>& classification,
			const std::array<std::vector<FireProductionRoundoffTrace::TraceFloat>,3>& velocity,
			const FireProductionRoundoffTrace::TraceFloat tolerance)
		{
			FireProductionRoundoffTrace::TraceFloat result=0.0f;
			for(unsigned int side=0u;side<6u;++side){
				if(boundary[side]!=FireProductionProjectionPressureOpen)continue;
				const unsigned int axis=side/2u;const bool positive=(side&1u)!=0u;
				const std::size_t firstCount=axis==0u?shape.ny:shape.nx;
				...
				for(std::size_t second=0u;second<secondCount;++second)
					for(std::size_t first=0u;first<firstCount;++first){
						...
						const FireProductionRoundoffTrace::TraceFloat outward=(positive?1.0f:-1.0f)*
							velocity[axis][FaceIndex(shape,axis,x,y,z)];
						const FireProductionRoundoffTrace::TraceFloat discrepancy=classification[side][index]?
							std::max(FireProductionRoundoffTrace::TraceFloat(0.0f),outward-tolerance):
							std::max(FireProductionRoundoffTrace::TraceFloat(0.0f),-outward-tolerance);
			...
			try {
				...
				for(unsigned int axis=0u;axis<3u;++axis){
					...
					for(std::size_t face=0u;face<alpha[axis].size();++face){
						const FireProductionRoundoffTrace::TraceFloat value=alpha[axis][face];
				...
				const FireProductionRoundoffTrace::TraceFloat scale=request.timeStepS/shape.cellWidthM;
				for(std::size_t z=0u;z<shape.nz;++z)for(std::size_t y=0u;y<shape.ny;++y)
					for(std::size_t x=0u;x<shape.nx;++x){
						const std::size_t cell=CellIndex(shape,x,y,z);
						for(std::size_t component=0u;component<components;++component){
							FireProductionRoundoffTrace::TraceFloat value=computed.lowState[component*cells+cell];
							for(unsigned int axis=0u;axis<3u;++axis){
				...
				auto inequalityValue=[&](const FireProductionRoundoffTrace::TraceFloat* value,const std::size_t inequality){
					if(inequality==0u)return -value[0];
					if(inequality==1u){FireProductionRoundoffTrace::TraceFloat closure=value[0];
						for(std::size_t species=0u;species<7u;++species)
							closure-=value[1u+species];
						return closure;}
					if(inequality<9u)return -value[inequality-1u];
					if(inequality==9u){FireProductionRoundoffTrace::TraceFloat lower=-value[8];
						for(std::size_t species=0u;species<7u;++species)
							lower+=request.enthalpyBoundsJPerKG[species]*value[1u+species];
						return lower;}
					FireProductionRoundoffTrace::TraceFloat upper=value[8];for(std::size_t species=0u;species<7u;++species)
						upper-=request.enthalpyBoundsJPerKG[7u+species]*value[1u+species];
					return upper;
				};
				for(std::size_t cell=0u;cell<cells;++cell){
					std::array<FireProductionRoundoffTrace::TraceFloat,9> value={{}};FireProductionRoundoffTrace::TraceFloat rowScale=1.0f;
					for(std::size_t component=0u;component<components;++component){
					...
					for(std::size_t inequality=0u;inequality<inequalities;++inequality){
						const FireProductionRoundoffTrace::TraceFloat excess=inequalityValue(value.data(),inequality);
				...
				std::vector<FireProductionRoundoffTrace::TraceFloat> baseGas(cells,0.0f),acceptedGas(cells,0.0f);
				FireProductionRoundoffTrace::TraceFloat ambientGas=0.0f;for(std::size_t component=1u;component<=6u;++component)
				...
				for(unsigned int component=0u;component<3u;++component){
					...
					for(std::size_t z=0u;z<zEnd;++z)for(std::size_t y=0u;y<yEnd;++y)
						for(std::size_t x=0u;x<xEnd;++x){
							...
							auto restricted=[&](const std::vector<FireProductionRoundoffTrace::TraceFloat>& gas){
							...
							const FireProductionRoundoffTrace::TraceFloat base=restricted(baseGas),acceptedValue=restricted(acceptedGas),
							...
							const FireProductionRoundoffTrace::TraceFloat residual=std::fabs(acceptedValue-advanced);
	...
	bool ComputeFireProductionGasDensityCPU(
		...
		std::vector<FireProductionRoundoffTrace::TraceFloat>& gasDensityKGPerM3,std::string* error )
	{
		...
		const FireProductionProjectedHeunTransportCoefficients& result )
	{
		...
		for(const std::vector<FireProductionRoundoffTrace::TraceFloat>& axis:*context.projectedVelocityMPerS)
		...
		FireProductionProjectedHeunCoupledStageResult& result,std::string* error)
	{
		...
		try {
			...
			auto buildStage=[&](const FireProductionProjectionResult& projection,
				...
				std::vector<FireProductionRoundoffTrace::TraceFloat> contextState=state,contextTemperature=temperatureK;
				std::array<std::vector<FireProductionRoundoffTrace::TraceFloat>,3> contextVelocity=projection.velocityMPerS;
			...
			auto canonicalProjection=[&](const FireProductionScalarProjectionTargetSeal& ownerTarget,
				...
				std::size_t selected=0u;FireProductionRoundoffTrace::TraceFloat selectedDiscrepancy=0.0f;bool haveSelected=false;
				for(std::size_t i=0u;i<provedCycleBranches.size();++i){
					...
					FireProductionRoundoffTrace::TraceFloat discrepancy=OwnerOpenClassDiscrepancy(shape,
					...
					if(!terminalRejectedClass&&OwnerTestFailure("active-cycle-selection-bias")&&
						i==0u)
						discrepancy=std::numeric_limits<FireProductionRoundoffTrace::TraceFloat>::max();
			...
			for(std::uint32_t iteration=0u;iteration<request_.maximumPicardIterations;
				...
				if(havePrior&&classStable&&residual<=request_.projectionTolerancePerS){
					...
					FireProductionRoundoffTrace::TraceFloat limiterDiscrepancy=0.0f;
					std::array<std::vector<FireProductionRoundoffTrace::TraceFloat>,3> selectedAlpha=verifiedScalar.sharedFaceAlpha;
					for(unsigned int axis=0u;axis<3u;++axis)for(std::size_t face=0u;
						face<selectedAlpha[axis].size();++face){
						const FireProductionRoundoffTrace::TraceFloat discrepancy=std::fabs(scalarAcceptance.sharedFaceAlpha[
		...
		FireProductionProjectedHeunOwnerResult& result,std::string* error)
	{
		...
		try {
			const FireProductionProjectionShape& shape=request_.source.Shape();
			const std::size_t cells=shape.CellCount();
			std::vector<FireProductionRoundoffTrace::TraceFloat> gasDensity;
			if(!OwnerGasDensity(shape,work_.conservativeValues,gasDensity))
				return Fail(error,"projected-Heun R2 gas density is invalid");
			std::array<std::vector<FireProductionRoundoffTrace::TraceFloat>,6> integratedHead;
			...
			auto endpointFlux=[&](const FireProductionProjectionResult& projected,
				...
				std::vector<FireProductionRoundoffTrace::TraceFloat> contextState=work_.conservativeValues;
				std::vector<FireProductionRoundoffTrace::TraceFloat> contextTemperature=work_.committedEOS.temperatureK;
				std::array<std::vector<FireProductionRoundoffTrace::TraceFloat>,3> contextVelocity=projected.velocityMPerS;
				FireProductionProjectedHeunTransportContext context;
			...
			for(std::uint32_t iteration=0u;iteration<request_.maximumPicardIterations;
				...
				const std::array<std::vector<FireProductionRoundoffTrace::TraceFloat>,3>* discrepancyVelocity=
					&projected.velocityMPerS;
				std::array<std::vector<FireProductionRoundoffTrace::TraceFloat>,3> injectedDiscrepancyVelocity;
				if(OwnerTestFailure("r2-used-class-discrepancy")){
					...
					for(unsigned int side=0u;side<6u;++side)if(
						...
						const std::size_t y=axis==1u?(positive?shape.ny:0u):0u;
						const std::size_t z=axis==2u?(positive?shape.nz:0u):0u;
						const FireProductionRoundoffTrace::TraceFloat outward=projectedClass[side][0u]?
							request_.endpointVelocityToleranceMPerS+0.25f:
							-request_.endpointVelocityToleranceMPerS-0.25f;
