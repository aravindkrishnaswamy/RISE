...
{
	std::printf( "S4: chat-loop tool table (seventeen tools, three providers) + SetSkillIndex...\n" );
	...
	// Anthropic: read_skill present with a schema.
	{
		...
		Check( tools.isArray() && tools.size() == 17, "anthropic body carries seventeen tools" );
		bool saw = false;
	...
	// Gemini: seventeen functionDeclarations, read_skill present.
	{
		AgentChatLoop loop;
		...
		Check( decls.isArray() && decls.size() == 17, "gemini body carries seventeen functionDeclarations" );
		bool saw = false;
	...
	// OpenAI Responses: sixteen flat function tools, read_skill present.
	{
		...
		JsonValue root = ParseBody( loop.BuildRequest( "sk-test" ).body );
		const JsonValue& tools = root.get( "tools" );
		Check( tools.isArray() && tools.size() == 17, "openai body carries seventeen tools" );
		bool saw = false;
		for( std::size_t i = 0; i < tools.size(); ++i ) {
