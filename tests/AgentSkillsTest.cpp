...
{
	std::printf( "S4: chat-loop tool table (eighteen tools, three providers) + SetSkillIndex...\n" );
	...
	// Anthropic: read_skill present with a schema.
	{
		...
		Check( tools.isArray() && tools.size() == 18, "anthropic body carries eighteen tools" );
		bool saw = false;
	...
	// Gemini: eighteen functionDeclarations, read_skill present.
	{
		AgentChatLoop loop;
		...
		Check( decls.isArray() && decls.size() == 18, "gemini body carries eighteen functionDeclarations" );
		bool saw = false;
	...
	// OpenAI Responses: eighteen flat function tools, read_skill present.
	{
		AgentChatLoop loop;
		...
		JsonValue root = ParseBody( loop.BuildRequest( "sk-test" ).body );
		const JsonValue& tools = root.get( "tools" );
		Check( tools.isArray() && tools.size() == 18, "openai body carries eighteen tools" );
		bool saw = false;
		for( std::size_t i = 0; i < tools.size(); ++i ) {
