...
|---|---|---|
| `document` | `op:"param_equals"` — `target`, `param`, `value`, `chunkKind?`, `numeric?` | `target` chunk's `param` value string-equals `value`. `chunkKind` narrows the by-name chunk lookup (needed when more than one chunk shares a bare name across kinds). `numeric:true` tokenizes both sides as space-separated floats and compares component-wise within `1e-4` (so `"0.0 0.0 1.0"` grades equal to `"0 0 1"`) |
| `document` | `op:"param_range"` — `target`, `param`, `min:[...]`, `max:[...]`, `chunkKind?` | `target.param`'s float tokens are each within the matching `[min_i, max_i]` band, component-wise — a tolerant "any reasonable blue" grade instead of one fixture's exact numbers |
