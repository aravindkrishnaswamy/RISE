...
- **Nothing is pre-baked.** The dial relief is authored IN-SCENE as `dialfn_*` `expression_function2d` math fields displaced onto a flat `cartesian_disk_geometry` (`displaced_geometry … uv_seam_fold FALSE`) — no guilloché-specific C++ geometry; the oxide doses are `guilloche_oxide_painter` functions, the strap a GENERAL `sweep_geometry` (authored 28-point FKM profile + drape path), the stitches a GENERAL `path_instances_geometry` (SDF thread capsule stamped along the two authored row paths) — all evaluated at scene-parse time. **`guilloche_disk_geometry` was RETIRED 2026-06** (the relief moved into the scene); its C++ relief math is gone, but `GuillocheField`'s oxide/thermal physics (`OxideDose`/`AbsoluteThicknessNm`/`SpallMask`/`TorchMask`) stays — the oxide painter still uses it.
- The six dial patterns are proven == the original `GuillocheField::Height` relief to 1e-6 in `tests/ExpressionFunction2DTest.cpp` (templated builders in `tests/GuillocheDialExpr.h` are the single source the test and a chunk-text emitter share). The oxide/thermal physics is pinned by `tests/GuillocheFieldTest.cpp`. The Python bakers stay in this folder as the historical reference; `tests/ProceduralMeshTest.cpp` now pins only the GENERAL `sweep_geometry` / `path_instances_geometry` factories.
...
- **Strap shape** → the `sweep_geometry` chunk: drape = the repeatable 3D `point` lines, cross-section = the `profile_point` lines, taper = `end_scale_x`.  The stitch rows are SEPARATE authored paths on the two `path_instances_geometry` chunks — re-derive them when the drape/profile changes (offset formula in the scene comment).
