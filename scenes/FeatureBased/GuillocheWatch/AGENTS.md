...
- **Nothing is pre-baked.** The dial meshes are `guilloche_disk_geometry` chunks, the oxide doses `guilloche_oxide_painter` functions, the strap a GENERAL `sweep_geometry` (authored 28-point FKM profile + drape path), the stitches a GENERAL `path_instances_geometry` (SDF thread capsule stamped along the two authored row paths) — all evaluated at scene-parse time in C++ (`src/Library/Painters/GuillocheField.h`, the factories in `src/Library/RISE_API.cpp`).
