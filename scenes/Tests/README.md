...
- `Hair/`: Chiang hair/fur BCSDF + `hair_geometry` groom render-level regression scenes (docs/HAIR_FUR_DESIGN.md Phase 1) — `hair_furnace` (sigma_a=0 groom in a uniform env, the render-level energy-conservation gate), `hair_melanin_ladder` (three grooms differing only in eumelanin concentration, brightness must decrease monotonically), `hair_backlit_tt` (light behind the groom, exercising the TT rim regime and the documented NEE-transmissive-hemisphere limitation), and `hair_styled` (comb/clump/curl/frizz grooming showcase through the `hair_geometry` parser). `tests/HairRenderTest.cpp` renders equivalent scenes in-process for numeric assertions; these files are the human-renderable and CST-golden-covered companions
