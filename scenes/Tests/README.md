...
- `Geometry/`: primitive and CSG sanity scenes, plus the object-parenting scene-graph check, plus
  `csg_positioned_operands` (a csg_object with its own `position` on top of ALREADY-positioned
  operands -- the "87" operand-rebase semantics, in the supported IN-VIEW case; intentionally
  trips `Job::AddCSGObject`'s parse-time rebase warning, and deliberately omits the
  `allow_transformed_operands` acknowledgment flag so it stays a live tripwire, see
  docs/SCENE_CONVENTIONS.md)
- `GlobalIllumination/`: focused GI baselines such as final gather
- `GUI/`: fixture scenes that stress the GUI's own surfaces (not just the renderer). `viewport_reattach_probe_a`/`_b` are two tiny, distinct, fast-rendering scenes used to drive the Mac GUI's headless `ViewportReattachProbe` (see `build/XCode/rise/RISE-GUI/App/RISEApp.swift`) scene-A-then-scene-B sequence; not meant to be rendered for their own sake. The other three are the doc-88 Phase 3 (node-graph canvas) S23 stress set, each render-verified (tiny film/samples) and graph-verified (Painter/Material node + edge counts against `SceneEditController::ReadPainterMaterialGraph`): `graph_stress_wide` — 128 Painter/Material chunks (96 painters + 32 materials), 207 edges, one hub painter fanned out to 24 referrers and several more at 5+, a 7-rank-deep reference chain, and at least one of every doc-88 arc chunk kind (`expression_painter`, `ramp_painter`, `mapping_painter`, `stochastic_tile_painter`, `scatter_painter`, `voronoi3d_painter{space world}`, `blend_painter` across all five modes) — the canvas layout/perf/badge workout; `graph_stress_editing` — 20 Painter/Material chunks purpose-built for edit-flow exercises, each documented in-file as "exercise X by doing Y": a painter shared across exactly 3 materials (the S19 ownership-REFUSE + Duplicate-node escape hatch), three deliberate orphans (badge check), a same-base-name-adjacent pair (`metal_trim`/`metal_trim_2`, the create-node dedupe picker), a `png_painter{file}` node (the file-slot picker flow), and a `ggx_material` with its optional `rs` slot left unbound (the unspelled-bind flow); `panel_stress_params` — 12 Painter/Material chunks built for the property-panel + `PainterPreview` load test: three `expression_painter` nodes each with 10-12 fully-annotated (`min`/`max`/`step`/`label`) params and a 5-6-deep sequential `def` chain (the def-stage thumbnail strip), a 12-stop `ramp_painter`, and two `scalar_painter{expression}` nodes with a `vec3`-typed body (genuine per-channel R≠G≠B scalars, one driving a `dielectric_material`'s dispersive `ior`)
- `Hair/`: Chiang hair/fur BCSDF + `hair_geometry` groom render-level regression scenes (docs/HAIR_FUR_DESIGN.md Phases 1-3) — `hair_furnace` (sigma_a=0 groom in a uniform env, the render-level energy-conservation gate), `hair_melanin_ladder` (three grooms differing only in eumelanin concentration, brightness must decrease monotonically), `hair_backlit_tt` (light behind the groom, exercising the TT rim regime and the documented NEE-transmissive-hemisphere limitation), `hair_styled` (comb/clump/curl/frizz grooming showcase through the `hair_geometry` parser), and `fur_medulla` (Phase 3 — three backlit grooms differing only in `medulla_ratio` 0 / 0.5 / 0.9, showing the Yan 2017 medulla turning crisp backlit strands into a soft saturated glow). `tests/HairRenderTest.cpp` renders equivalent scenes in-process for numeric assertions; these files are the human-renderable and CST-golden-covered companions
