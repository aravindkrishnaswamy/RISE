...
> **2026-07-24 scene-audit correction:** historical claims below that
> `bdpt_veach_egg` exercised the MLT Pel consumer were mislabeled; the scene
> uses the BDPT rasterizer. The live baseline harnesses now point Gate F at
> `FeatureBased/MLT/mlt_veach_egg.RISEscene`.

---

## TL;DR

> **STATUS — 2026-06-02: the integrator refactor (Phases 0/1/2a/2b/2c) is COMPLETE.** PT (Phase 2b), VCM (Phase 2a), and BDPT (Phase 2c, finished with family F3b) are all single-source-of-truth across Pel/NM — every paired Pel/NM method is now one `*Impl<Tag>` template behind thin forwarders. 116/116 tests, all baselines within noise, both toolchains warning-free, every family adversarially reviewed 0 P1 / 0 P2. Optional follow-ons (Phase 2d shared connection primitive, the Phase-4 dead-member cleanup, the PT-spectral AOV wiring, the deferred SA-MIS migration) are listed under "Phase 2d/3/4" below — none block "refactor complete". The narrative below is the original Phase-0→2a session report, preserved; per-phase outcomes are appended under "Phase-by-Phase Status".
...
### Phase 2c — BDPT templatization (COMPLETE, 2026-06-02 — all 5 families landed; F2 + F3 complete)
...
- **Part 5 / Family F3b — `EvaluateAllStrategies{,NM}` + the `EvalEmitterRadiance<Tag>` fold (DONE).** The (s,t) strategy-enumeration driver (Pel ~243 + NM ~103 ln) collapsed into `EvaluateAllStrategiesImpl<Tag>` (anon-ns free function taking `const BDPTIntegrator& self` + the 11 `protected` OpenPGL guiding-state members threaded as params under `#ifdef RISE_ENABLE_OPENPGL`) + **1 new helper** `DispatchConnectAndEvaluate<Tag>` (→ the public `ConnectAndEvaluate{,NM}` members, which own `pLightSampler`) + reuse of F3a's `ConnectionResultFor<Tag>` + 2 forwarders. **6 `if constexpr`** (all Pel-only — the NM method was the strict subset): the OpenPGL complete-path RIS prologue, the `useComplete` init, loop-B's `cr.s`/`cr.t` post-set, the zero-exitance value-type split (`ComputeDirectLighting`/`amount`/`throughput` vs `ComputeDirectLightingNM`/`leNM`/`throughputNM`), and the 2 Pel-only training records (the shared `RecordGuidingTrainingLightPath` runs for both). The original Pel `#ifdef OPENPGL if(useComplete){A}else#endif{B}` was restructured to `if constexpr(is_pel){if(useComplete){A}} … if(!useComplete){B}` (logically equivalent; NM always takes B). **Completed the F3a-seeded emitter fold**: F3a's `SurfaceEmitterRadiance<Tag>` renamed `EvalEmitterRadiance<Tag>` (single source); the protected `EvalEmitterRadianceNM` **member removed** (its 2 callers in `RecomputeSubpathThroughputNM` rerouted to `EvalEmitterRadiance<NMTag>`) + its `.h` decl deleted. **`.h` reduced by 7 lines** — the one ABI-safe member removal (protected, non-virtual, zero external callers; the chip-sanctioned reduction). Net `BDPTIntegrator.cpp` **−9** (6,275→6,266). Gates: clean from-scratch `make` (0 warnings, dual `Impl<PelTag>`+`Impl<NMTag>` in-TU → discarded-branch well-formedness proven; OpenPGL/`pSampler` params lexically referenced under `if constexpr(is_pel)` → no `-Wunused`) + Xcode RISE-GUI arm64 **BUILD SUCCEEDED** (0 source warnings); 116/116 (`BDPTStrategyBalanceTest`, `EnvLightBalanceTest` 80/0 lax, VCM oracles, `VertexColorRoundtripTest`, `SobolDimensionBudgetTest` unchanged); all 16 connection baselines within noise (`cornellbox_bdpt` **0.0000% bit-identical** at the Pel-locked checkpoint; the 2 above-floor scenes are documented noisy single-λ spectral — `vertex_colors_quad_bdpt_spectral` 0.5614% is INSIDE the binary's own 3-render spread 0.5779%); **strict env-MIS oracle** failure count is run-to-run MC jitter {5,5,6,6} that never exceeds pre's 6 (the flipping sub-check is the noisiest spectral topology); VCM no-touch + the historically misclassified BDPT Pel row `bdpt_veach_egg` 0.0092% (not MLT coverage) + MLT **NM** `hwss_mlt_spectral_cornellbox` sane; 3-reviewer adversarial review (read-only **Explore** agents, no `spawn_task`) **0 P1 / 0 P2 / 0 P2-to-fix** (Pel-locked-before-NM sequencing meant no behavioural defect surfaced). **Phase-4 dead-member cleanup (`IsVisible`/`EvalBSDFAtVertex`/`EvalPdfAtVertex`) deliberately left** (each needs a further `.h` edit; F3b removed only the fold's `EvalEmitterRadianceNM`). Full ledger: [PRE_PHASE1_STATUS.md](PRE_PHASE1_STATUS.md) → "Phase 2c F3b outcome".

**Phase 2c total**: `BDPTIntegrator.cpp` **~8,200 → 6,266 (≈ −1,934)**; `.h` net −7. Every BDPT Pel/NM method pair is now a single `*Impl<Tag>` template. **The integrator refactor (Phases 0/1/2a/2b/2c) is COMPLETE** — PT, VCM, BDPT are all single-source-of-truth across Pel/NM.

### Phase 2d/3/4 — not started (the integrator refactor proper is COMPLETE; these are optional follow-ons)
