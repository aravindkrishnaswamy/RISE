...
>   `VolumeSpectralCoefficientsTest` (19 assertions): curve-driven coefficients, chromatic
>   `Tr(650) ≫ Tr(550)`, pure-absorber `σ_s=0`, wavelength-independent luminance fallback,
>   a 400k-sample `SampleDistanceNM` scatter-probability match to `1−exp(−σ_t(λ)·maxDist)`,
>   and a negative-curve non-negativity-clamp case (`Tr ≤ 1`).
