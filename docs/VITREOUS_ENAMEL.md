...
>   the 360–830 nm sampler extremes (no extrapolation blow-up). Two authoring guards: curve
>   output is clamped to non-negative in the spectral path (a negative control point would
>   otherwise give `σ_t<0` ⇒ `Tr>1` energy gain), and a curve needs **≥2 control points** — a
>   single-point `IFunction1D` evaluates to 1.0 everywhere (the `PiecewiseLinearFunction`
