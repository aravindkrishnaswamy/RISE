...
2. **◑ Partially grounded (transaction atomicity).** The clone capability #1 proved is the foundation for the **working-copy / staging** model — drag on a scratch copy, build the transaction off-graph, publish only on success (so no double-apply, nothing redoable-after-reject). **What remains to build:** the **restore/publish path** (swap the working copy in + rebuild TLAS/`LightSampler`) and routing drag through the working copy. Not yet implemented; the natural next step after #1.
