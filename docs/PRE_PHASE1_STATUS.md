...
| Misclassified Gate-F row (BDPT scene) | `bdpt_veach_egg` | 0.0100% (0.0179%) |
...
- **Historical Gate-F row (misclassified)**: `bdpt_veach_egg` 0.0100% used `BDPTRasterizer`; it did not exercise the MLT consumer.
- **Gate 7 — adversarial review**: see ledger.
...
| Misclassified Gate-F row (BDPT scene) | `bdpt_veach_egg` | 0.0022% (0.0117%) | under floor |
...
- **Historical Gate-F row (misclassified)**: `bdpt_veach_egg` 0.0022% used `BDPTRasterizer`; it did not exercise the MLT consumer.
- **Gate 7 — adversarial review**: see ledger.
...
| Misclassified Gate-F row (BDPT scene) | `bdpt_veach_egg` | 0.0015% (0.0017%) | under floor |
...
- **Historical Gate-F row (misclassified)**: `bdpt_veach_egg` 0.0015% used `BDPTRasterizer`; it did not exercise MLT's `EvaluateAllStrategies` consumer.
...
- **Gate 3 — zero behaviour change** (post-Δ vs `pre_f3b`, [scripts/bdpt_connection_baselines.sh](../scripts/bdpt_connection_baselines.sh) 16-scene manifest; final post-NM values):
  ...
  | Misclassified Gate-F row (BDPT scene) | `bdpt_veach_egg` | 0.0092% (0.0061%) | multi-trial noise |
...
- **Gate-F correction**: the Pel `bdpt_veach_egg` measurement used `BDPTRasterizer` and did not validate MLT. MLT **NM** `hwss_mlt_spectral_cornellbox` did exercise `MLTSpectralRasterizer` → `EvaluateAllStrategiesNM` → `Impl<NMTag>` and rendered sane (meanL 72.1, bootstrap normalization b=22166.9).
...
## Codex-review findings — FIXED (2026-06-02)

An independent Codex review of the just-completed integrator refactor surfaced **three INDEPENDENT correctness bugs** (all confirmed real).  Each is fixed as its own attributable change-set in the working tree so the user can review/commit them separately.  Landed easiest-to-verify first.  Adversarial review (read-only Explore agent) after all three: Findings 1, 2-core, 3 → **CORRECT**; it also surfaced one in-family **additional issue** (HWSS-SSS-mid-path), documented below for the user (NOT fixed inline, per scope rule).

### Finding 1 [P2] — escape-transmittance capped at 1e10, not infinity — FIXED

- **Root cause.** The env-ESCAPE transmittance / free-flight `maxDist` used `Scalar(1e10)` as a stand-in for infinity.  For an UNBOUNDED (global) medium with tiny σ_t (e.g. σ_a = 1e-12), `exp(-σ_t·1e10) = exp(-0.01) ≈ 0.99` instead of 0 — the environment LEAKS through where an infinite absorbing path must fully attenuate it.  BDPT's env-NEE connection ([BDPTIntegrator.cpp](../src/Library/Shaders/BDPTIntegrator.cpp) `ConnectAndEvaluate` s=1, ~line 3535) and `LightSampler` env-NEE ([LightSampler.cpp:1740](../src/Library/Lights/LightSampler.cpp)) already used `RISE_INFINITY` and documented exactly why; the escape sites had drifted.
- **Fix.** `Scalar(1e10)` → `RISE_INFINITY` at all **9 env-escape sites**:
  - PT ([PathTracingIntegrator.cpp](../src/Library/Shaders/PathTracingIntegrator.cpp)): per-bounce maxDist in `IntegrateRayTemplated` (~1465), RGB first-bounce (~3110) + scatter→escape twin (~3209), NM per-bounce (~3534), HWSS first-bounce (~4170) + scatter→escape twin (~4246).
  - BDPT ([BDPTIntegrator.cpp](../src/Library/Shaders/BDPTIntegrator.cpp)): eye-subpath free-flight (~1642), Path-B synthetic env-vertex escape-Tr (~1945, comment ~1937 updated), light-subpath free-flight (~5113).  VCM consumes the same throughput via the shared generator.
- **Safety** (`RISE_INFINITY` = `Scalar(1.7976931348623158e+308)` = DBL_MAX, a FINITE value — verified, not IEEE inf):
  - `HomogeneousMedium`: `exp(-σ_t·DBL_MAX)` overflows to +inf → 0 for σ_t>0; `0·DBL_MAX = 0` (no NaN) → `exp(0)=1` for σ_t=0.  Free-flight samples a finite t (~1/σ_t) for any physical σ_t.
  - `HeterogeneousMedium`: `SampleDistance` / `EvalTransmittance{,NM}` all route through `MajorantGrid::TraverseRay`, which clips to the finite grid AABB (`tExit = fmin(gridExit, maxDist)`) and is triple-bounded (nMaxCells=256, grid-index exit, `tCellExit≥tExit`) — terminates regardless of `maxDist`; no hang.
  - `EquiangularSampling`: `atan2(DBL_MAX,D) → π/2` (same as `atan2(1e10,D)`), `tan` finite for ξ<1, all divisions guarded.  No new failure mode vs the 1e10 it ran with.  (`LightSampler:1740` and BDPT `:3535` already pass `RISE_INFINITY` through these helpers in production.)
- **Repro** (new fixtures [`scenes/Tests/Volumes/env_unbounded_thin_{pt,bdpt,vcm}.RISEscene`](../scenes/Tests/Volumes/)): global homogeneous pure-absorption medium σ_a=1e-12 + uniform white env + a small self-emissive sphere anchor.  Background = env through infinitely-much absorber (physically → 0).
  - Pre-fix: background ≈ 0.98 (BRIGHT env leak), frame mean ≈ 1.176 (PT, BDPT, VCM all leak).
  - Post-fix: background **min = 0** (env fully absorbed), frame mean ≈ 0.283 (just the emitter); emitter preserved at 3.18 = 10/π.  All three integrators fixed.
  - The existing `env_bounded_fog_{pt,bdpt,vcm}` (bounded heterogeneous medium) image-mean is UNCHANGED to ≤ 0.1 % (grid clips both ways), within the measured thread-local-RNG run-to-run noise floor.
- **Gates:** clean warning-free `make` + Xcode RISE-GUI arm64; 116/116; new fixtures before→after as above; bounded-fog unchanged.

### Finding 2 [P3] — spectral (NM) PT drops SMS suppression state across the SSS recursion — FIXED (code-read; repro impractical)

- **Root cause.** The SMS emission-suppression flags (`smsPassedThroughSpecular` / `smsHadNonSpecularShading`) are carried across the BSSRDF (SSS) recursion boundary in RGB but NOT in NM.  Two coupled gaps:
  1. [PathTracingShaderOp.cpp](../src/Library/Shaders/PathTracingShaderOp.cpp) `PerformOperationNM` called `IntegrateFromHitNM(...)` ending at `glossyFilterWidth`, relying on the false/false defaults — the RGB `PerformOperation` forwards the two flags.
  2. The two BSSRDF continuations in `IntegrateFromHitTemplated<Tag>` ([PathTracingIntegrator.cpp](../src/Library/Shaders/PathTracingIntegrator.cpp) ~2061 + ~2218) POPULATED the flags only for Pel (`if constexpr (Traits::is_pel)`); the NM `else` set `bsdfTimesCos` + optimal-MIS (a runtime no-op — optimal-MIS is Pel-only) and left the flags at default false/false.
  So a spectral camera→…→SSS(emerge diffuse)→glass→light path did NOT suppress the BSDF-hit emission already covered by SMS → double-count.  Same family as the 2026-05-31 fix (commit `4d4db993`, asymmetry #1/#3).
- **Fix (both coupled parts):**
  1. `PerformOperationNM` now forwards `rs.smsPassedThroughSpecular, rs.smsHadNonSpecularShading` like the RGB twin.  (`IntegrateFromHitNM` already had the two params with `false` defaults — no signature change.)
  2. Both BSSRDF sites set the two flags UNCONDITIONALLY (both tags); the NM-only `bsdfTimesCos`/optimal-MIS bookkeeping is gated under `if constexpr (Traits::is_nm)`.
- **Pel UNCHANGED (verified by `git diff`):** the Pel execution path sets the same two flags before and after and skips the NM block in both — byte-equivalent.  RGB renders are bit-identical (NM-only fix).
- **Repro — attempted, fell back to code-read (documented difficulty).** Built a spectral PT+SMS scene with a Donner-Jensen-skin SSS sphere + glass + emitter so a camera→SSS→glass→light path exists.  The SSS surface is under-lit through glass (NEE through a delta refractor fails; the SSS-emergence→glass→emitter chain is low-probability) and the resulting double-count is a rare firefly-level effect swamped by SMS MC noise: pre/post-fix image means agreed to 0.1 %, not isolatable above noise on the constructed geometry.  Per the agreed stop-rule, removed the non-triggering scene and rely on the code-read argument: with both parts applied, NM's SMS-flag handling is now IDENTICAL to the proven-correct Pel path, and the suppression logic lives in the SHARED `IntegrateFromHitTemplated<Tag>` body — once the flags match, the behavior matches.  Safety net: the existing [`sms_through_glass_emitter_pt_sms.RISEscene`](../scenes/Tests/Spectral/sms_through_glass_emitter_pt_sms.RISEscene) spectral-SMS regression + the 116-test suite.
- **ADDITIONAL ISSUE FOUND (documented for the user — NOT fixed, per scope rule).**  The HWSS hero-bundle path has the SAME flag-drop in ONE site: [PathTracingIntegrator.cpp](../src/Library/Shaders/PathTracingIntegrator.cpp) `IntegrateFromHitHWSS`, the **SSS mid-path delegation** (~line 3742-3747) calls `IntegrateFromHitNM(...)` ending at `glossyFilterWidth` (→ false/false), whereas the ADJACENT no-BSDF/glass mid-path delegation (~line 3719) correctly passes `false, true` with an explicit comment explaining why (the delegation is reached only after ≥1 non-specular SMS anchor).  By that same reasoning the SSS mid-path delegation should also pass `false, true`.  This is a one-line fix (append `, false, true` to the call at ~3742, mirroring ~3719); **no signature change needed** — `IntegrateFromHitNM` already takes the flags.  It is HWSS-mode + SSS + glass/specular + emitter only (rare), and it does NOT affect the NM or RGB paths fixed above.  Note: SPECTRAL_PARITY_AUDIT.md §2.3 previously claimed the "HWSS-mode double-count … FIXED" (true for the glass case at ~3719, via `4d4db993`); the SSS-mid-path case at ~3742 was missed.  Left for the user to decide (it is a 4th site beyond this finding's defined NM scope).
- **Gates:** clean build; 116/116; Pel bit-identical.

### Finding 3 [P2] — PT emitter-hit MIS used the joint env-vs-alias PMF in mixed HDRI+mesh scenes — FIXED

- **Architecture finding (determined the fix).** PT direct lighting ([LightSampler.cpp](../src/Library/Lights/LightSampler.cpp) `EvaluateDirectLighting`) samples mesh + delta lights from the alias table INDEPENDENTLY (`aliasTable.Sample`, `pdfAlias = aliasTable.Pdf`, **alias-only**, ~line 1501) and handles the environment via a SEPARATE env-NEE strategy (~line 1700, *"independent of the light-table NEE … its own MIS against BSDF"*).  Env-NEE samples env directions; it CANNOT sample a mesh emitter.  So a mesh-emitter BSDF-hit competes for MIS only with the alias-NEE technique → its competitor pdf must be **alias-only**.  The NEE side already uses alias-only (`p_light = pdfAlias·dist²/(area·cosLight)`, ~line 1652).
- **Root cause.** The 2026-05-29 continuous-PMF work made `CachedPdfSelectLuminary` (PT's BSDF-hit competitor query, used at [PathTracingIntegrator.cpp:1878 RGB / 3804 NM](../src/Library/Shaders/PathTracingIntegrator.cpp)) return the JOINT `(1-cachedEnvSelectProb)`-scaled pmf.  PT's NEE sample-side stayed alias-only, so the two PT techniques disagreed by `(1-envSelectProb)`: `p_nee` on the BSDF-hit side (~line 1887) was too small → `w_bsdf` too large → mesh-emitter hits OVERWEIGHTED whenever an HDRI coexists with mesh emitters.
- **Option chosen: (b) — make the PT competitor query alias-only.**  `CachedPdfSelectLuminary` is PT-EXCLUSIVE (only callers are PT 1878+3804; BDPT/VCM use `PdfSelectLuminary`/`SampleLight`, the joint-roll path) and its documented contract (the `EvaluateDirectLighting` MIS comment, *"CachedPdfSelectLuminary returns the same alias-table PDF on the BSDF-hit side"*) was always alias-only.  The continuous-PMF change erroneously added the joint factor.  Fix: removed the `aliasShare` factor — it now returns alias-only `aliasTable.Pdf` / `pLightBVH->Pdf`.  **Option (a)** (make PT's NEE roll env-vs-alias jointly) was REJECTED — it would require restructuring PT's two independent NEE strategies (alias-NEE + env-NEE) into a single joint selection (a large PT-NEE change) for no benefit, since the two independent strategies are already self-consistent.  BDPT/VCM are untouched (they correctly rely on the joint pmf via `SampleLight`).
- **Repro** (new fixtures [`scenes/Tests/Lights/env_plus_mesh_emitter_{pt,bdpt,vcm}.RISEscene`](../scenes/Tests/Lights/)): a SEALED Cornell box lit ONLY by an interior mesh emitter, plus a very bright GLOBAL env (`radiance_scale 1000`) blocked by the walls — it never lights the interior; its sole effect is `cachedEnvSelectProb → ~1` (the max-bug regime).
  - PT pre-fix mean = **0.206** (1.64× too bright).
  - PT post-fix mean = **0.127** ≈ BDPT reference **0.125** (within 1.2 % MC noise).  The mesh-emitter overcount is gone.  (VCM is unreliable on this scene — its own env+mesh residual + a max-154 firefly — BDPT is the clean reference.)
  - **env-only / mesh-only UNCHANGED by construction:** mesh-only → `cachedEnvSelectProb = 0` → `aliasShare = 1.0` → `1.0·Pdf == Pdf` bit-exact; env-only → no mesh emitter to look up → competitor query not exercised.
- **Gates:** clean build; 116/116; **`EnvLightBalanceTest` lax {0.30,0.35,1.50} HOLDS 80/80**; strict {0.10,0.30,1.00} **unchanged at 78/80 pre & post** (measured both ways by temporarily flipping the test tolerance, reverted — the 2 residuals are VCM-env+mesh + a BDPT/VCM spectral-env-only, neither touched by this PT fix); env+mesh repro before→after as above.

### Attribution (for separate commits)
- **Finding 1**: [PathTracingIntegrator.cpp](../src/Library/Shaders/PathTracingIntegrator.cpp) (escape-Tr maxDist region: 1465/3110/3209/3534/4170/4246), [BDPTIntegrator.cpp](../src/Library/Shaders/BDPTIntegrator.cpp) (1642/1937-comment/1945/5113), + `env_unbounded_thin_*`.
- **Finding 2**: [PathTracingShaderOp.cpp](../src/Library/Shaders/PathTracingShaderOp.cpp) (`PerformOperationNM` forward), [PathTracingIntegrator.cpp](../src/Library/Shaders/PathTracingIntegrator.cpp) (the two BSSRDF `if constexpr` sites ~2061/2218).
- **Finding 3**: [LightSampler.cpp](../src/Library/Lights/LightSampler.cpp) (`CachedPdfSelectLuminary`), + `env_plus_mesh_emitter_*`.

Findings 1 and 2 share `PathTracingIntegrator.cpp` but in **disjoint regions** (escape-Tr maxDist vs the BSSRDF SSS continuation), so `git add -p` cleanly separates them.
