...
`docs/PROXIMITY_SHOWCASES.md` specifies three composed showcase scenes for
`proximity(r)` and `interior(r)` — Phase 1 shipped machinery with two
minimal fixture scenes (`plank_closeup`, `weathered_workbench`) and Phase 3
finished the mesh/CSG/signed-half machinery with no beauty render an author
can open; the showcases fill that gap. Each scene's stations are predicted
from the geometry before composition and re-derived by a harness test
against the tracked scene, never hard-coded.
