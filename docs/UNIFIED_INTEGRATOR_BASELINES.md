...
**`Inf⚠` UPDATE (2026-06-03, K=16 re-measure with the FLOAT-EXR writer):** BDPT's `Inf⚠` was an **FP16-EXR artifact** → finite on re-measure (mean σ² ≈ 1.10e3, lum 1.096). **VCM's `Inf⚠` is GENUINE** — a real degenerate-pdf firefly (actual `Inf` in **9/16** FLOAT-EXR renders, not guard-catchable), an **open** integrator bug. See §7/§9 and [INTEGRATOR_BUGFIX_FINDINGS.md](INTEGRATOR_BUGFIX_FINDINGS.md) §1.
...
- **`glass_pavilion` (Inf), `spectral_caustic` (477%), `torus_chain` (89%), `diamond_teapot` (69%), `pool_caustics` (48%)** — caustic / refractive / dispersive. Even VCM (the winner) is noisy. These point at **Specular Polynomials** and **per-wavelength VCM photons** (spectral_caustic). **`glass_pavilion` `Inf` UPDATE (2026-06-03, K=16 re-measure — SPLIT into two causes):** **BDPT** = an FP16 EXR-write overflow of finite fireflies → **FIXED** at the writer layer (`bpp 32` → 32-bit FLOAT EXR); re-measures finite (`mean σ²` 1.10e3, `lum` 1.096). **VCM** = a **GENUINE degenerate-pdf `Inf` firefly — a real integrator bug, OPEN**: the FLOAT EXR (max ~3.4e38) still records actual `Inf` in **9/16** renders, NOT caught by the `isfinite` guards (an A/B of `-fno-finite-math-only` shows the same Inf rate, 6/10 vs 8/10). The chip's earlier "8 renders → inf=0" was a sampling fluke. See [INTEGRATOR_BUGFIX_FINDINGS.md](INTEGRATOR_BUGFIX_FINDINGS.md) §1. So glass_pavilion is BOTH a high-noise target AND carries a real VCM firefly to fix (degenerate-pdf guard at its source).
