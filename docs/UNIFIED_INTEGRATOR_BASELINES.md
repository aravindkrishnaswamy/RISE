...
- **`glass_pavilion` (Inf), `spectral_caustic` (477%), `torus_chain` (89%), `diamond_teapot` (69%), `pool_caustics` (48%)** — caustic / refractive / dispersive. Even VCM (the winner) is noisy, and `glass_pavilion` produces literal `Inf` fireflies in BDPT/VCM. These point at **Specular Polynomials**, **per-wavelength VCM photons** (spectral_caustic), and a **degenerate-pdf firefly guard** in VCM/BDPT caustic connections (glass_pavilion).
- **`sculptors_studio` (211%)** — spot-light-dominated: PT fireflies catastrophically (σ/μ 5373%), BDPT renders **near-black** (lum 0.0023 — a real BDPT delta-light failure), only VCM is well-behaved. A distinct 'delta-light BDPT' failure mode.
...
4. **Highest-leverage scenes (§7) are all caustic/SDS/spectral-caustic** — where a *new* technique (Specular Polynomials, per-wavelength VCM photons, ReSTIR-PT) would pay off, since even the best current integrator is noisy or Inf there. Two smaller by-product findings are flagged for the user (not fixed, per scope): a **degenerate-pdf firefly** in VCM/BDPT caustic connections (`glass_pavilion` Inf) and a **BDPT delta-light near-black failure** (`sculptors_studio`).
