...
- **Threshold-gated eye- and light-subpath branching** — `vcm_pel_rasterizer` reads `branching_threshold` from `StabilityConfig` (default `0.5`).  When `BDPTIntegrator::GenerateEyeSubpath` / `GenerateLightSubpath` encounters a multi-lobe **pure-delta** vertex (e.g. dielectric Fresnel reflect+refract) early in the walk (before any non-delta shading), with surviving normalized throughput above the threshold, it spawns one continuation per scattered ray (`beta *= kray_i` deterministic split, no `/selectProb`).  Each branch is a complete subpath with the shared prefix duplicated.  `VCMPelRasterizer::IntegratePixelRGB` iterates per branch, calling `ConvertEyeSubpath` + `EvaluateS0` + `EvaluateNEE` + `EvaluateMerges` once per eye-branch and `EvaluateInteriorConnections` per `(eye × light)` pair; `SplatLightSubpathToCamera` sits outside the eye loop and iterates per light-branch.  Sobol stream base offsets per branch (light branches base 10000, eye branches base 20000) keep branches decorrelated.  See [src/Library/Shaders/BDPTIntegrator.cpp](../src/Library/Shaders/BDPTIntegrator.cpp) around the do-while loops in `GenerateEyeSubpath` / `GenerateLightSubpath`.  Per-branch consumers are live for `BDPTPelRasterizer`, `BDPTSpectralRasterizer`, `VCMPelRasterizer`, `VCMSpectralRasterizer`, and the `VCMRasterizerBase` photon-store build; `MLTRasterizer` / `MLTSpectralRasterizer` force `branchingThresholdOverride = 1.0` because the Markov-chain proposal measure assumes a single subpath.  BDPT consumers apply `eb==0` / `lb==0` gates for the strategies that read only shared-prefix endpoints (`t==1` splat, `s==0`/`s==1` emission and NEE) to avoid N× accumulation; other `(s,t)` strategies are per-pair dependent and correctly sum across branches.
