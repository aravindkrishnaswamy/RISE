...
- **Surface-only merging.**  Medium scatter vertices propagate the geometric update (using `sigma_t_scalar` in place of `|cos|`) so dVCM/dVC/dVM are approximately correct at post-media surface vertices, but medium vertices themselves are not stored.  Connections skip MEDIUM-type endpoints.  **Connection + escape transmittance through participating media is now media-aware and consistent with PT/BDPT.**  Connection-Tr at three of four connection-strategy sites (NEE / interior / splat-to-camera) reuses `BDPTIntegrator::EvalConnectionTransmittance{,NM}` directly since Pre-Phase-1 Piece 2 (2026-05-31).  The fourth strategy — s=0 *escape* (eye/camera ray exits the scene through a medium to env-IBL) — is now Tr-attenuated too, via the **coordinated escape-Tr fix** (2026-05-31, [PRE_PHASE1_STATUS.md](PRE_PHASE1_STATUS.md) "Coordinated escape-Tr fix"): the transmittance is applied in the shared `GenerateEyeSubpath{,NM}` at the synthetic env-vertex push (`vEnv.throughput *= Tr`, `maxDist=1e10` matching PT), so VCM's `EvaluateS0Impl` env-escape inherits it with no VCM-side change.  (For surface direct hits the eye-walk's throughput already folds in Tr along the final segment, so s=0 surface emitters are untouched — applying it again would double-attenuate.)  Result: on the bounded-medium env-IBL fixture `env_bounded_fog_{pt,bdpt,vcm}` (pure absorption) **PT ≈ BDPT ≈ VCM to within ±5 % (VCM +0.1 % of PT)** — a cross-integrator agreement property.  Volume-aware MERGING is still out of scope.  **Known residual (separate workstream, NOT the escape-Tr fix):** with in-scattering (σ_s > 0), VCM under-counts single-scatter of env light at medium vertices (~31 % vs PT) — a VCM volumetric-MIS gap (medium-vertex env connection / env-S0 ↔ env-NEE partition, IMPROVEMENTS.md §12 family); the pure-absorption fixture isolates and validates the escape-Tr fix independently of it.
