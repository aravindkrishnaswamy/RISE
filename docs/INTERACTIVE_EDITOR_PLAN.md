...
- **Suppression-aware restart after production.** Desktop controllers restart through `startSuppressingInitialRender`; Android passes `suppressFirstFrame=true` into the atomic start operation so its newly-created sink is latched before the render thread starts. The production image stays visible until the user actually starts dragging. (§19.6)
...
| Viewport activation | **Always-on once a scene is loaded** (no "Interact" toggle). Production render disables viewport interaction and stops the interactive controller; on completion, the controller restarts with its initial render suppressed so the production image remains visible until the user drags. Desktop uses `startSuppressingInitialRender`; Android latches `suppressFirstFrame` while constructing the fresh sink, before its render thread starts. (Decision made post-Phase-3, replaces an earlier "Interact mode toggle" design.) |
| Preview-frame dispatch contract | Sink dispatches **every** `OutputImage` call to the UI, including partial buffers from cancelled passes. The post-production restart is the only exception: desktop controllers suppress the initial render at source, while Android's fresh sink drops exactly its first frame through a flag latched by `startViewport` before the worker starts. (Earlier design dropped cancelled passes; that proved to throttle the viewport during motion — see §19.6.) |
...
2. **Suppression-aware restart** *(kept, race closed)*. Used after a production render so the production image stays visible until the user actually starts dragging. macOS and Windows restart the existing controller through `startSuppressingInitialRender` / `StartSuppressingInitialRender`, suppressing production at the source. Android reconstructs its sink on every stop/start, so `startViewport(suppressFirstFrame=true)` latches the fresh sink's atomic one-shot before `SceneEditController_Start` launches the worker. There is no standalone post-start Android suppression API: such a call could arrive after a fast first pass and is therefore inherently racy.
