...
- Render flow: interaction is disabled, the controller's canonical scene time is captured and the interactive render is stopped off the UI thread, then the production rasterizer runs. On completion, desktop restarts the controller through `startSuppressingInitialRender`; Android calls `startViewport(suppressFirstFrame=true)`, which latches the new sink before starting its worker.
