...
- **Suppression-aware restart after production.** All three platforms suppress the controller's synthetic initial render at source. Android passes `suppressFirstFrame=true` into its atomic start operation, which selects `StartSuppressingInitialRender` before the render thread starts. The production image stays visible until the next real interaction, while an immediate edit remains a distinct pending render. (§19.6)
...
| Viewport activation | **Always-on once a scene is loaded** (no "Interact" toggle). Production render disables viewport interaction and stops the interactive controller; on completion, every platform restarts the controller with its synthetic initial render suppressed so the production image remains visible until the next real interaction. Android carries that intent as `suppressFirstFrame` into the atomic start operation. (Decision made post-Phase-3, replaces an earlier "Interact mode toggle" design.) |
| Preview-frame dispatch contract | Sink dispatches **every** `OutputImage` call to the UI, including partial buffers from cancelled passes. Post-production suppression happens before rendering, in `SceneEditController::Start(true)`, so a sink never drops an indistinguishable first user-edit frame. (Earlier design dropped cancelled passes; that proved to throttle the viewport during motion — see §19.6.) |
...
- Render flow: interaction is disabled, the controller's canonical scene time is captured, and the interactive render is stopped before the production rasterizer runs. Android combines time capture and stop under one blocking lifecycle transaction on `Dispatchers.IO`; the desktop hosts currently call their synchronous bridge stop from the UI render action. On completion, desktop restarts through `startSuppressingInitialRender`; Android calls `startViewport(suppressFirstFrame=true)`, which selects the same source-level suppressed admission before starting its worker. Because Android reconstructs its controller after each production render, a controller with no accepted time scrub preserves the ViewModel's current-time fallback; once a scrub is accepted, the controller's undo/redo-aware time is authoritative.
...
### 19.6 Sink dispatch policy (cancel guard reversed; suppression-aware restart retained)
...
1. **Cancel guard** *(added then removed)*. The rasterizer's `FlushToOutputs` fires unconditionally even when the dispatcher returned early for a cancel. The initial concern: post-production, the bridge restart would dispatch a half-rendered preview buffer that flashed over the just-completed production image. The first fix consulted `SceneEditController::IsCancelRequested()` (promoted from `protected` to `public`) and dropped the dispatch when true.
   ...
   **Reversal**: in practice, during a fast drag every pointer move trips the cancel flag, so almost every in-flight render is cancelled mid-pass. With the guard active the sink dropped all of them, and the user only ever saw the post-pause refinement passes (§19.7) — visually indistinguishable from a throttled viewport. The user feedback was direct: *"For interactive there should be no throttling updates to the screen."*
   ...
   Final policy: the sink dispatches **every** `OutputImage` call. Center-out tile order makes partial buffers visually usable (the centre of the image fills first, edges later). The post-production-flash concern that motivated the cancel guard is covered by the suppression-aware controller restart below — there is no remaining case where dropping cancelled frames was load-bearing.

   The promoted `SceneEditController::IsCancelRequested()` accessor stays public for the agent/session coordinators that poll cancellation. Obsolete sink controller fields and their cancel-guard wiring have been removed from all three platform adapters.

2. **Suppression-aware restart** *(kept, race closed)*. Used after a production render so the production image stays visible until the next real interaction. macOS and Windows restart the existing controller through `startSuppressingInitialRender` / `StartSuppressingInitialRender`. Android reconstructs the controller on every stop/start and selects `RISE_API_SceneEditController_StartSuppressingInitialRender` when `suppressFirstFrame=true`. Suppression therefore applies to the controller's synthetic initial admission, not an arbitrary first sink output: an edit arriving immediately after start remains pending and renders normally. There is no standalone post-start Android suppression API.
