...
> **⚠ ERRATUM (2026-07-02, post-CST-cutover).** The Model-B P5 CST cutover (completed 2026-07-01) delivered most of what this spec calls the "parse → IR" seam: **`Cst::ParseToCst(const std::string&)` is a string-input, construction-free parse** to a CST `Document`, and entity construction happens only in the separate **`DeriveToJob`** step ([../../src/Library/Cst/Cst.cpp](../../src/Library/Cst/Cst.cpp)) — so "the parser builds as it parses / no parse-only seam" (§1.1) and "there is no string-input parser" are **no longer true at the parse layer** (they remain true of the *derive* layer, where `Finalize` → `pJob.Add*` still runs — the throwaway-`Job` tier below still applies to construction-level checks). Also deleted since this spec was written: `AsciiSceneParser.cpp` and `ISceneParser::ParseAndLoadScene` (Slice 6c — the descriptor-driven chunk parsers live in `IAsciiChunkParser.h` / `ChunkParserRegistry.cpp`, driven by `DeriveToJob`), and the round-trip-save span types `SourceSpanIndex.h` `RawTokenSpan`/`ParameterSpan` (Slice 6d — the byte-splice save engine is gone; the CST `Document` itself is the span/IR substrate now). An F5 slice-0 `validate` over `ParseToCst` has shipped in `src/Library/Agent/`. The two-tier design (IR-level gutter + sandboxed throwaway `Job`) remains the right shape — the IR tier should be **the CST Document**, not a new IR.

**Status:** **CORE VALIDATION SHIPPED; HARDENING PLAN PARTIAL.**
`AgentSession::ValidateText` performs CST parse plus throwaway-`Job` derive
without mutating the live scene and backs the agent `validate` verb. The
broader editor-gutter, resource-jail, limit, and out-of-process hardening in
this document remains design work. This doc supersedes the original capturing
`ValidationJob` sketch in [MCP_TOOL_SURFACE.md](MCP_TOOL_SURFACE.md) §6.
...
### 1.1 The parser builds as it parses (no parse-only seam) — **superseded at the parse layer (see erratum)**

> **Post-cutover:** `Cst::ParseToCst(const std::string&)` IS the parse-only seam
> (construction-free, string-input); `ISceneParser::ParseAndLoadScene` was deleted
> in Slice 6c-3c. What survives of this section: the **derive** layer
> (`DeriveToJob` → descriptor dispatch → `Finalize` → `pJob.Add*`) is still
> build-on-finalize, so construction-level checks still need the throwaway-`Job`
> tier.

Pre-cutover: `ISceneParser::ParseAndLoadScene(IJob&)` either succeeded while **mutating** the `IJob`, or returned `false` with errors sent to `GlobalLog()` — there was no structured report and no dry-run mode. One level down, the descriptor architecture is explicitly *build-on-finalize* (still true of the derive layer today; the dispatch lives in `ChunkParserRegistry.cpp`):

```cpp
// IAsciiChunkParser (descriptor dispatch; pre-cutover home was AsciiSceneParser.cpp)
...
1. **String input.** Already exists post-CST-cutover: `Cst::ParseToCst(const std::string&)` parses a string with no temp file (the pre-cutover ask — a `ParseString` entry on the deleted `AsciiSceneParser` — is moot).
...
> **Post-cutover note:** the CST `Document` (`Cst::ParseToCst`) **is** this IR —
> a lossless, construction-free tree of chunks/params/commands that `SerializeCst`
> round-trips byte-exactly. Build the validate tier over the Document rather than
> introducing the parallel `SceneIR` below; the sketch is retained as the design
> intent. (Its `RawTokenSpan`/`SourceSpanIndex` types were deleted in Slice 6d;
> scene version is now `RISE ASCII SCENE 7`.)
...
**[Superseded post-CST-cutover]** The pre-cutover sketch reused the round-trip-save span types (`RawTokenSpan` / `ParameterSpan`, `SourceSpanIndex.h`) — deleted in Slice 6d with the byte-splice engine. And the "missing in-memory text-backed structure" is missing no longer: the `Job` retains the canonical CST `Document`, whose text face is `Cst::SerializeCst(doc)` — which also satisfies the "in-memory scene-text accessor" need ([MCP_TOOL_SURFACE.md](MCP_TOOL_SURFACE.md) §3.1). Validation and save now share the Document itself as the one vocabulary.
...
The only value validation the descriptor dispatcher performs is a kind `switch` (pre-cutover home `AsciiSceneParser.cpp:725-742`; the dispatch now lives in `ChunkParserRegistry.cpp` / `IAsciiChunkParser.h`): for `Double`/`DoubleVec3`/`DoubleVec4`/`DoubleMat4`/`UInt` it runs `AllTokensAreFiniteNumbers` — "every whitespace-separated token is a finite number, no `nan`/`inf`." Everything else falls to `default: break` (no check). Consequences:
...
- **`parser_error` (`errorClass:"parser"` — the parser rejects):** `SYNTAX_ERROR`, `SCENE_VERSION`, `UNKNOWN_CHUNK`, `UNKNOWN_PARAMETER`, `TYPE_MISMATCH` *(a non-finite numeric token in a `Double`/`UInt`/`Vec`/`Mat` slot — the one value check the dispatcher enforces — now in `ChunkParserRegistry.cpp`)*, `MISSING_REQUIRED`, `DUPLICATE_NAME`. The runtime-only Tier-2/`full`-mode codes `UNRESOLVED_REFERENCE`, `WRONG_PIPE` *(new — the §1.4 `ResolveOrDiagnoseScalar` color-vs-scalar failure)*, and `FILE_NOT_FOUND` are also `parser_error`s **when reached** (the parser rejects these too, just later, in `Finalize`/`Job`) — they are the §5.4-whitelisted checks a static schema can't express. This is the bucket the conformance test (§5.4) pins to the parser's reject verdict.
...
- **Tests.**
  ...
  - **Correctness invariant:** validating then loading the *same* text yields the *same* accept/reject as the real load path (`Cst::ParseToCst` + `DeriveToJob`; parser-faithfulness, the §1 goal — `ParseAndLoadScene` was deleted in Slice 6c-3c); the integrator/parser remains byte-identical for non-validation paths (the `apply IR` refactor is behavior-neutral).
...
- Ground truth: [CURRENT_STATE_AUDIT.md](CURRENT_STATE_AUDIT.md) (§4 pipe-not-in-descriptor still stands; its §12 "no side-effect-free parse / `Job` retains no text" is superseded post-CST-cutover — see that doc's erratum)
...
- Parser & descriptor architecture: [../../src/Library/Parsers/README.md](../../src/Library/Parsers/README.md), [../../src/Library/Parsers/ChunkDescriptor.h](../../src/Library/Parsers/ChunkDescriptor.h), [../../src/Library/Parsers/ChunkParserRegistry.cpp](../../src/Library/Parsers/ChunkParserRegistry.cpp) + [../../src/Library/Parsers/IAsciiChunkParser.h](../../src/Library/Parsers/IAsciiChunkParser.h) (descriptor dispatch + build-on-finalize; `AsciiSceneParser.cpp` was deleted in Slice 6c)
...
- CST parse/derive seam (the parse-only entry): [../../src/Library/Cst/Cst.h](../../src/Library/Cst/Cst.h) (`ParseToCst`, `DeriveToJob`, `SerializeCst`; the pre-cutover `ISceneParser::ParseAndLoadScene` was removed — see [../../src/Library/Interfaces/ISceneParser.h](../../src/Library/Interfaces/ISceneParser.h) header note)
- The decisive runtime-state check: [../../src/Library/Job.cpp](../../src/Library/Job.cpp) (`ResolveOrDiagnoseScalar` `:2718-2777`)
- Span/IR substrate: the CST `Document` ([../../src/Library/Cst/Cst.h](../../src/Library/Cst/Cst.h)); the pre-cutover `SourceSpanIndex.h` span types (`RawTokenSpan`/`ParameterSpan`) were deleted in Slice 6d
