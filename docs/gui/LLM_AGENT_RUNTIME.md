...
**Ground truth:** every "today" claim is reconciled to [CURRENT_STATE_AUDIT.md](CURRENT_STATE_AUDIT.md) (code-verified 2026-06-19 — **see its 2026-07-02 post-CST-cutover erratum**: the byte-splice "structured save" it describes was deleted in Slice 6d; save is now a whole-Document `Cst::SerializeCst` of the Job's retained CST Document). The bottom line for this doc is unchanged and stronger: **scene save ships** (property/transform/camera-create/camera-remove edits persist via the retained Document); only non-camera creation persistence (per-family Document insert ops) is the gap.
...
The **MVP requires no *new* engine work** and depends on no *unbuilt* save path. Scene save **already ships** — post-CST-cutover (2026-07-01) it is a whole-Document serialize of the Job's retained CST Document (`SaveEngine.cpp`, `Cst::SerializeCst`; the byte-splice "structured save" of the pre-cutover [CURRENT_STATE_AUDIT.md](CURRENT_STATE_AUDIT.md) §1 was deleted — see that doc's erratum), so AI transform/property/camera edits persist today; the only persistence gap is **non-camera creation** (per-family Document insert ops, [ENTITY_CREATION.md](ENTITY_CREATION.md)). And like a human hand-editor, the agent can always fall back to rewriting scene text wholesale and reloading — `read scene + read grammar + propose diff + validate + reload + render + look` is a complete, powerful loop on infrastructure that exists today (`loadAsciiScene` + `rasterize` + the progressive framebuffer the GUI already polls via `pollProductionVFS`).
...
|---|---|---|---|
| **A0** | Agent server: read scene + grammar + framebuffer + `validate` + render-control | (sibling doc) — runtime consumes its catalog as `ToolSpec[]` | mostly exists; text-loop needs no *new* save work (post-cutover, edits into the retained CST Document persist via the whole-Document save — see CURRENT_STATE_AUDIT's erratum) |
...
A1 is the high-leverage MVP and is **buildable on today's engine** (load/rasterize/poll-framebuffer + the existing controller) with no dependency on an unbuilt save path — scene save already ships (whole-Document serialize of the retained CST Document), and text-as-truth lets the agent rewrite-and-reload for the rest.
...
| [CURRENT_STATE_AUDIT.md](CURRENT_STATE_AUDIT.md) | ground truth for "scene save ships; only non-camera creation is the gap" (see its post-cutover erratum; pre-cutover §1). |
