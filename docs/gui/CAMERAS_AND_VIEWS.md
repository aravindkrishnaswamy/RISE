...
**Dependents / soft-blocks (rewritten 2026-07-02, post-CST-cutover):** Camera **creation, deletion, rename, transform, and property** persistence all ship today — but via the **CST Document path**, not the byte-splice engine this doc's body still cites. Every scene loads via `Cst::ParseToCst` + `DeriveToJob` with the `Job` retaining the canonical CST `Document`; camera edits route into it (`Job::ApplyCstParamEdit`, `ApplyCstInsertCameraChunk`, `ApplyCstRemoveCameraChunk` — the last works on *file-authored* cameras too); `SaveEngine::Save` serializes the whole Document (`Cst::SerializeCst`). The Mode A/B + Phase B/C engine, `tests/SaveEngineTest.cpp`, and its per-edit Refused classes were **deleted in Model-B P5 Slice 6d (2026-07-01)** — see the erratum atop [CURRENT_STATE_AUDIT.md](CURRENT_STATE_AUDIT.md). So promote-to-camera (B1), stamp (B3), B4a camera-list operations (including delete of a file-authored camera — the old "needs `> remove camera` tombstone" refusal is obsolete), and the camera-property half of bind-to-time-range (B4) persist **today**. **One** save soft-block genuinely remains: **B4b's `animation`-chunk *extension*** (camera cuts) is a scene-language + Document-edit-op addition that does not exist yet. Non-camera *creation* persistence is owned by [ENTITY_CREATION.md](ENTITY_CREATION.md) (whose tombstone design is likewise premised on the deleted engine — see its banner); transaction/epoch semantics by [TRANSACTION_MODEL.md](TRANSACTION_MODEL.md). See §3.1, §7.1. **Body caveat:** references below to Mode A/B, Phase B/C re-emit, the managed block, `SaveEngine.cpp:1373`, and `SaveEngineTest.cpp` describe the deleted engine; read them as "routes into the retained CST Document and persists via whole-Document save".
...
2. **Persist:** the new camera's chunk is inserted into the Job's retained CST Document (`Job::ApplyCstInsertCameraChunk` → `Cst::DocInsertItem`) and persists via the whole-Document save (`SaveEngine::Save` → `Cst::SerializeCst`; the byte-splice engine of [../ROUND_TRIP_SAVE_PLAN.md](../ROUND_TRIP_SAVE_PLAN.md) is history — see its header). A promoted camera emits a real `pinhole_camera` / `thinlens_camera` chunk with a `name`. **This persists today.** There is **no** "won't survive reload" caveat for a promoted camera. As a *transaction*, promote is an `AddCamera` commit (the transaction object + commit path are [TRANSACTION_MODEL.md](TRANSACTION_MODEL.md) §4; the `AddCamera` creation op is [ENTITY_CREATION.md](ENTITY_CREATION.md) §4) — undoable and attributed (`origin = InApp`) like any edit. **Promote/Stamp is the ONLY B1 action that mutates a camera** (restore does not — §3.2); it bumps the scene revision by exactly one and publishes one snapshot per [TRANSACTION_MODEL.md](TRANSACTION_MODEL.md) §4.2.
...
- **Stamp = promote the transient pose to a scene camera.** "Set Camera to View" applies the `ViewportPose` onto a chosen camera. This is the §3.4 promote path, sourced from the `ViewportPose` instead of a Named View:
  - **Stamp onto the active camera** (overwrite) = a property-edit transaction on the existing camera (the `CameraCommon` setters, as a single committed transaction; see [TRANSACTION_MODEL.md](TRANSACTION_MODEL.md) §4 — `origin = InApp`, one undo unit). Persists today via a camera-property edit into the retained CST Document (whole-Document save; see the preamble).
  - **Stamp into a new camera** = an `AddCamera` transaction seeded from the `ViewportPose` — exactly the `AddCameraFromPose(name, pose)` entry point §3.4 already needs (entity creation is owned by [ENTITY_CREATION.md](ENTITY_CREATION.md) §4; epoch/precondition by [TRANSACTION_MODEL.md](TRANSACTION_MODEL.md)). Persists today via the camera-chunk insert into the retained CST Document (`ApplyCstInsertCameraChunk`; see the preamble).
...
**Scene-language vs UI for B3:** the transient `ViewportPose` and free-fly navigation are **UI/session state only** — never written to the scene, no chunk. The single scene write is the **stamp**, which is a camera transaction (overwrite = property edit into the retained CST Document; new camera = `AddCamera` → `ApplyCstInsertCameraChunk`) — both **persist today** via the whole-Document save. No new chunk.
...
- **Actions:** activate (exists), clone (exists), rename, delete, "new from current view" (= B3 stamp into a new camera). **Persistence (updated 2026-07-02, post-CST-cutover):** camera **delete now persists uniformly** — `Job::ApplyCstRemoveCameraChunk` removes the camera's chunk from the retained CST Document (file-authored cameras included), and the whole-Document save writes the result; the pre-cutover "needs a `> remove camera` tombstone" refusal chain in the paragraph below is **obsolete** (the tombstone design itself was premised on the deleted byte-splice engine — see [ENTITY_CREATION.md](ENTITY_CREATION.md)'s banner). **Rename** remains the honest caveat: a rename must rewrite the declaration token *and every reference site* in the Document ([ENTITY_CREATION.md](ENTITY_CREATION.md) §6.3); surface whatever the rename op refuses.
...
| **B3** | Fly-then-stamp (transient `ViewportPose` + "Set Camera to View") | `ViewportPose` (UI state) + free-fly nav + **render-camera override (§5.5)** + `AddCameraFromPose` + stamp-onto-active (overwrite) | toolbar/menu action + lock-to-view toggle | writes existing camera chunk (stamp only) | **none for stamp persistence** — stamp persists today (property edit / camera-chunk insert into the retained CST Document, whole-Document save); the render-camera override is additive engine work (§5.5), not a save soft-block |
| **B1b** | Promote to scene camera (durable) | reuse B3 `AddCameraFromPose` | menu action | existing camera chunk emitted into the CST Document | **none** — created-camera persists today (`ApplyCstInsertCameraChunk`; fidelity covered by `tests/CstSaveFidelityTest.cpp`) |
| **B4a** | Camera list panel (thumbnails, activate/clone/rename/delete) | thumbnail cache over `ICameraManager`; reference-safe delete ([ENTITY_CREATION.md](ENTITY_CREATION.md) §5) | list widget | rename/delete edit the retained CST Document | **delete: none** — `ApplyCstRemoveCameraChunk` removes the chunk (file-authored included) and the whole-Document save persists it (the pre-cutover "needs `> remove camera` tombstone" refusal is obsolete). **rename:** must rewrite declaration + every reference site in the Document (ENTITY_CREATION §6.3) — surface any refusal |
| **B4b** | Bind camera to time-range (camera cuts) | `CameraTrack` + dispatcher hook + non-overlap validation (§6.2.1) | timeline cut-region UI | **extend `animation` chunk** | **`animation`-chunk descriptor extension + CST Document edit op for cut entries** (the one remaining save soft-block; once in the Document, whole-Document save persists cuts for free) |
...
6. **Round-trip dependency sequencing — narrowed further post-CST-cutover (2026-07-02):** camera *creation, property,* **and delete** persistence are **already built** — B1b promote, B3 stamp (overwrite *and* new-camera), and B4a delete (file-authored included, via `Job::ApplyCstRemoveCameraChunk`) persist today through the retained-CST-Document + whole-Document-save path (see the preamble). The old two-item blocker list (file-authored-camera delete needing a `> remove camera` tombstone; the managed-block re-emit relaxation) described the byte-splice engine deleted in Slice 6d and is obsolete. **One** save dependency remains:
   - **B4b camera-cut persistence** still needs the `animation`-chunk descriptor extension plus a CST Document edit op for cut entries (owned by [ENTITY_CREATION.md](ENTITY_CREATION.md) §7's successor work). Land B4b's in-memory cut model first; once cuts live in the retained Document, the whole-Document save persists them for free.
...
- **Tests.**
  ...
  - *B1b/B3 promote persistence:* a promoted / stamped-new camera survives **save→reload→resave byte-identically** on the second save — extend the CST save-fidelity coverage (`tests/CstSaveFidelityTest.cpp`; the pre-cutover `SaveEngineTest.cpp` tests were deleted in Slice 6d). Stamp-onto-active persists via a property edit into the retained CST Document.
  ...
  - *B4a delete persistence (§8 / §6.1, rewritten post-CST-cutover):* deleting a camera — created **or** file-authored — removes its chunk from the retained CST Document (`ApplyCstRemoveCameraChunk`) and the next save writes a file without it; a reload then shows the camera gone. Invariant: the delete is an explicit, undoable transaction, and the serialized file's remaining bytes are minimally diffed (no unrelated reformatting).
...
- **Migration.** **No scene-format change** for B1–B3 (named views are sidecar UI state; promote/stamp emit *existing* camera chunks). The **one** scene-language touch is **B4b's `animation`-chunk extension** for camera cuts (one descriptor addition + `Finalize` read + a CST Document edit op for cut entries) — backward-compatible (older scenes parse unchanged; the cut entry is optional). Post-CST-cutover there is no separate "save-engine capability" dependency: once an entity or entry lives in the retained CST Document, the whole-Document save persists it (B4a file-authored-camera delete already works this way via `ApplyCstRemoveCameraChunk`; the old tombstone / `> remove camera` prerequisites are obsolete). ABI: all additive C exports + new bridge methods/structs, per the **`abi-preserving-api-evolution`** skill (`.claude/skills/`) — no virtuals on frozen interfaces, no signature changes (§9). The **render-camera override (§5.5)** is an additive interactive-render parameter (a new overload or a render-time scene view) — no scene-format change, no signature change to existing rasterizer entry points.
- **Rollback.** Each sub-phase is independently feature-flaggable and default-on once its tests pass; with a flag off, the GUI falls back to today's behavior (active-camera navigation, clone-only camera creation) without touching any `.RISEscene`. B4b camera cuts degrade safely: until the `animation`-chunk extension + Document edit op land, cuts are in-memory session state that a save must not silently drop — surface that loudly (the never-silently-lose-work philosophy) — so disabling cuts cannot corrupt a file. Lock-to-view defaults **OFF** (§5.4); turning the whole transient-pose model off reverts B3 to overwrite-active-only (the render-camera override §5.5 is simply not engaged, and the interactive pass reads `Scene::pActiveCamera` as today).
