...
- **A4 Region / ROI render.** Modifier+drag a box in the viewport; only the box refines (huge iteration speedup). A persistent "REGION" badge while active; regions **auto-clear before any production render** (the Blender leak-into-final footgun is explicitly designed against). Under the MLT integrator regions are ignored by the engine — the UI must say so rather than pretend.
