...
|---|---|---|
| **Light** | Easy | Self-contained chunk; all params are scalars/vec3/painter-names already in `Inspect`. **CORRECTION (2026-09-02): the color caveat below is stale.** `LightColorPelToSRGB` does not exist in the current `SaveEngine.cpp`, and implementing it as originally written would double-encode. Since 2026-09-02 lights read AND store `color` as linear Rec.709 (same convention as a painter's `color`; see [SCENE_CONVENTIONS.md](../SCENE_CONVENTIONS.md) §4). A re-emit must write the engine's linear RISEPel triple verbatim and add no `colorspace` line (or, if one is wanted for clarity, `colorspace Rec709RGB_Linear`) — never sRGB-encode it. The camera path already proves the round-trip. |
