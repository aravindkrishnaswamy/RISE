...
> **⚠ ERRATUM (2026-07-02, post-CST-cutover).** This spec's code-verified citations predate the Model-B P5 CST cutover (completed 2026-07-01). What changed: the byte-splice save engine (Mode A/B, `SourceSpanIndex` / `OverrideSpanIndex`, `tests/SaveEngineTest.cpp`) was **deleted** (Slice 6d); every scene now loads via `Cst::ParseToCst` + `DeriveToJob` and **the `Job` retains the canonical CST `Document`** — so this doc's "the Job does not retain source text" premise (§3.1) is now the **opposite** of reality (the retained Document *is* the in-memory source of truth; its text face is `Cst::SerializeCst(doc)`). `SaveEngine::Save` (now ~281 lines) serializes that whole Document, NoOps on byte-equality, and **still refuses on the external-modification mtime/size guard** — the guard survives, but it lives at `SaveEngine.cpp` ~223-247 against `FileIdentity` (`src/Library/SceneEditor/FileIdentity.h`), not `SourceSpanIndex.h:133-140`. The doc's core model (single-writer, snapshots, `(UUID, revision)` identity, refuse-don't-merge reconciliation, `SaveResult` four-state) is unaffected and if anything *simpler* to build now; stale citations are corrected inline where load-bearing.
>
> **⚠ SUPERSEDED (2026-07-06) — §7 "AI L1 staging," §8 "Attribution storage," and §10 "show me the code diff" describe a `SceneTransaction`/`TxnMeta`/`Commit(txn)`/`RenderTransactionDiff`/`CheckDiskSync`/`ResolveAndSave` abstraction that was NEVER BUILT.** Grep across `src/` confirms zero hits for `SceneTransaction`, `TxnMeta`, `RenderTransactionDiff`, `CheckDiskSync`, or `ResolveAndSave`. This is aspirational design that the Secure-MCP agentic-approval work (slices 2, 5a–5c, landed 2026-06 through 2026-07) superseded with a narrower, concrete mechanism. **What shipped instead:**
> - **An in-memory `AgentProposal` queue on the controller**, not a general transaction type: `SceneEditController::StageProposal` / `ListProposals` / `ResolveProposal`, guarding an `AgentProposal` struct (`src/Library/SceneEditor/SceneEditController.h`), all under the controller's existing `mMutex`. The queue is **per-controller-lifetime only** — nothing is persisted to disk, and it does not survive a reload/document swap.
> - **Mutations replay through three narrow, existing verbs**, not a generic `SceneEdit` list inside a `SceneTransaction`: `SceneEditController::ApplyAgentParamEdit` / `ApplyAgentInsertChunk` / `ApplyAgentRemoveChunk` (same file). A staged `AgentProposal` records which of the three it will replay (`AgentProposalKind::ParamEdit` / `InsertChunk` / `RemoveChunk`) plus its arguments; there is no arbitrary composite-edit-list transaction object.
> - **Authority, not autonomy-mapped staging, decides propose-vs-commit**: `AgentAuthority::{Owner,External}` (`src/Library/Agent/AgentSession.h`) — `Owner` sessions (the local GUI) commit directly; `External` sessions always stage a proposal and can never resolve it (including their own). Layered on top, `AgentAutonomy::{Read,Propose,Commit}` (`src/Library/Agent/AgentRpc.h`) gates which wire verbs a session may dispatch at all. Neither enum matches this doc's L0–L3 staging table (§7.3) — that table was never implemented.
> - **Conflict detection is the real `(uuid, revision)` optimistic-concurrency pair**, not a `SceneTransaction.baseDocument` diff: `Cst::CstHeadVersion` (`src/Library/Cst/Cst.h`) — `uuid` names the retained CST head (fresh per load), `revision` bumps iff the retained Document changed. `ResolveProposal` re-checks the proposal's captured base version against the live head atomically inside the same `mMutex` critical section that applies the edit — there is no separate `RenderTransactionDiff`/diff-and-rebase step; a stale proposal is rejected, not re-diffed.
> - **The wire surface is `list_proposals` / `resolve_proposal`**, dispatched through `AgentRpc.cpp` — `list_proposals` is on the read-safe allowlist (dispatchable even under `AgentAutonomy::Read`); `resolve_proposal` is deliberately Commit-only (an `External`/Propose-autonomy caller is refused with a named error, not silently escalated).
>
> **Deliberately NOT built** (do not assume these exist): no `SceneTransaction`/`TxnMeta`/`CommitResult` value types; no published-snapshot `AcquireSnapshot()`/`DocumentId` seam (§3.4/§3.5.4 are still aspirational, per their own inline "NOT YET IMPLEMENTED" markers); no `RenderTransactionDiff` unified-diff renderer; no `CheckDiskSync`/`ResolveAndSave`/`FileSyncState`/`ReconcileChoice` reconcile-prompt surface (§9's promote-refusal-to-prompt UX is undesigned-in-code — `SaveEngine`'s external-modification refusal from §9.1 is real, but nothing above it); no proposal persistence across a reload or process restart; no S6-style per-proposal scope/limit enforcement. The aspirational §7/§8/§10 prose below is kept in place as design history — it documents *why* the shipped shape looks the way it does — but a reader implementing against this doc should follow the shipped symbols above, not the pseudocode in §7/§8/§10.

**Status:** **MIXED IMPLEMENTATION AND HISTORICAL DESIGN.** The retained CST
document, `(uuid, revision)` head version, shared undo path, owner/external
authority, and proposal conflict checks have shipped. The general
`SceneTransaction`, published-snapshot API, attribution model, and file
reconciliation UX did not; follow the supersession banner above and current
symbols rather than the aspirational pseudocode. This document remains the
design owner for work that has not shipped.
...
9. **External-file reconciliation refuses; reload is a document swap, not an undoable transaction.** `SaveEngine` already refuses to save when the on-disk file changed since load (mtime+size guard, `SaveEngine.cpp` ~223-247, against `FileIdentity` — `src/Library/SceneEditor/FileIdentity.h`). This spec *promotes* that hard refusal into a **reload-or-Save-As prompt** surfaced before save and on window-focus — never a silent merge or clobber of hand-edits (finding #6). A reload is reclassified as a **document-level swap** that destroys the controller + its history and constructs a new one with a fresh UUID — it is *not* an in-history undoable transaction (re-serialize-from-introspection cannot reproduce unsupported chunks / `>` commands / macros / `FOR` loops / construction-only state; and a re-parse has no history slot to pop). A truly undoable reload needs a real document snapshot and is out of v1 scope (§9.3, §11.4, §13).
...
| **Durable checkpoint** | The on-disk `.RISEscene`. Updated only by an explicit save (whole-Document `Cst::SerializeCst`). | `SaveEngine::Save` (`SaveEngine.cpp:178`) |
...
| **Reconciliation (file)** | Resolving divergence between the live document and a `.RISEscene` changed on disk under us — **refuse with reload-or-Save-As**, never merge/clobber. | promotes `FileIdentity` guard (`SaveEngine.cpp` ~223-247; `FileIdentity.h`) |
...
- **Durable = disk.** Only `RequestSave` (`SceneEditController.h:483`) advances it. **[Corrected post-CST-cutover]** The `Job` **does** retain the scene's source of truth in memory — the canonical CST `Document` (its text face is `Cst::SerializeCst(doc)`); the pre-cutover "byte-offset `SourceSpanIndex` only, no retained text" claim is obsolete (see the erratum). "The file" and "the live state" remain two stores that can diverge (disk can change under us), so §9 reconciliation is still mandatory — but a save is now a straightforward whole-Document serialize rather than a splice against re-read file bytes.
...
> **⚠ SUPERSEDED (2026-06-20, 5th-review remediation) — see §13a of [GUI_ROADMAP](../GUI_ROADMAP.md) for ground truth.** This entire §3.5–§3.7 narrative is HISTORICAL: the snapshot/restore primitive was built (#2a/#2b) then **DEMOTED to EXPERIMENTAL**, and the editor's transaction **rollback now uses inverse-edit undo, NOT snapshot-restore** (`SceneEditTransactionTest` 85/0). Every "18/18", "52/0", "pose-only camera", "restore/swap NOT yet built", and "no new source file" phrasing below is stale spike-era text, retained for design rationale only.
>
> **Status (A-era spike): PROTOTYPED & TESTED.** A real spike landed on branch `feature/gui-snapshot-prototype`. The snapshot mechanism in this section — clone-the-mutable-wrapper-state, addref-the-immutable-leaves — is implemented and proven independent of later live mutation by [`tests/SceneSnapshotTest.cpp`](../../tests/SceneSnapshotTest.cpp) (**18/18 pass**, including a negative control). The *publication* layer (RCU-style published-pointer, `AcquireSnapshot()`, and the **restore/swap** path) is NOT yet built — see §3.5.4 and the prototype-scope-vs-production-gaps note (§3.7). The v1 paper design (a thin `IReference` handle that *only* addrefs resources, with no clone) is **superseded**: addref preserves lifetime, not state, and the editor mutates the very instances it would have addref'd, so a pure-addref snapshot would observe the mutation. The spike chose the clone instead, and the test proves the choice.
>
...
> **⚠ SUPERSEDED — see the top-of-file notice.** The `SceneTransaction`/staging design below was not built. What shipped is the narrower `AgentProposal` queue (`SceneEditController::StageProposal`/`ListProposals`/`ResolveProposal`) plus `AgentAuthority::{Owner,External}` (`src/Library/Agent/AgentSession.h`) and `AgentAutonomy::{Read,Propose,Commit}` (`src/Library/Agent/AgentRpc.h`) — no L0–L3 autonomy-mapped staging table exists in code.
...
> **⚠ SUPERSEDED — see the top-of-file notice.** The `TxnMeta` deque below does not exist. The shipped `AgentProposal` (`SceneEditController.h`) carries its own origin/kind/target fields inline and is resolved (approved/rejected) via `ResolveProposal`, not popped off a parallel undo-attribution deque.
...
`SaveEngine` **already detects** external modification: at save it `stat()`s the source and compares `st_size` + `st_mtime`(+nsec) against the `FileIdentity` captured at CST-load (`SaveEngine.cpp` ~223-247; `FileIdentity` in `src/Library/SceneEditor/FileIdentity.h`; identity captured/refreshed via `Job::RefreshCstLoadFileIdentity`). On mismatch it returns `Status::Refused` with a clear message and **leaves the file untouched**; the guard applies only to an in-place save of the loaded path (a Save-As to a different path is exempt). Post-save it re-baselines the stored identity to the just-written file. This external-clobber guard is the foundation — the spec turns its *hard refusal* into a *user choice*. *(Pre-cutover this guard protected byte-splice offsets and lived beside `SourceSpanIndex`; post-cutover it protects against overwriting external hand-edits with the in-memory Document.)*
...
`ResolveAndSave`:
- `SaveAsNewPath` → the safe default offered first. Writes the live edits to a **different** path (a whole-Document serialize of the retained CST Document); the original (externally-edited) file is untouched — the hand-edits at the original path are never at risk.
- `OverwriteDiscardingDisk` → only after the user explicitly confirms "discard the version on disk." **[Simplified post-CST-cutover]** The pre-cutover dilemma here (no retained source text → either lossy re-serialize-from-introspection or partial-refusal) is **gone**: the `Job` retains the canonical CST Document, and `Cst::SerializeCst(doc)` re-emits the live document's own text faithfully (comments, formatting, `let`s intact). `OverwriteDiscardingDisk` is therefore exactly that serialize written over the external version — honest, lossless with respect to the in-memory document, and loudly confirmed. No introspection re-serialize is involved.
...
`SaveEngine` already distinguishes Save-As: the external-modification guard applies **only when writing back to the loaded path** (`SaveEngine.cpp` ~223-247 — the guard checks `cstLoadIdent.filePath == filePath`); a Save-As to a *different* path serializes the retained Document to the new target unguarded, then re-baselines the `FileIdentity` to it. *(The pre-cutover `isSaveAs` flag and cross-file managed-block refusals were deleted with the byte-splice engine.)* Reconciliation respects this: any `SaveEngine` refusal surfaces through the same reconcile UI rather than dropping content. `SaveAsNewPath` (§9.2) is exactly this path, offered as the safe response to a disk-changed file.

---

## 10. "Show me the code" diff derives from a transaction

> **⚠ SUPERSEDED — see the top-of-file notice.** `SceneEditController::RenderTransactionDiff` was never built; there is no shared unified-diff renderer. A staged `AgentProposal` is shown to the owner via `ListProposals`'s own fields (kind/target/params), not a generated `.RISEscene` text diff.
...
> **`std::string SceneEditController::RenderTransactionDiff(const SceneTransaction&) const`** — produces a unified `.RISEscene` text diff for the transaction. **[Mechanism updated post-CST-cutover]** Implementation serializes the CST Document *as it is at the base snapshot* vs *as it would be after the transaction's edits* (`Cst::SerializeCst` on both, buffer-only, no disk write) and diffs the two texts — `SerializeCst`'s minimal-diff property makes the diff naturally surgical. *(The pre-cutover plan reused the byte-splice engine's per-entity chunk renderers, e.g. `RenderCreatedCameraChunk` — deleted in Slice 6d.)*
...
A transaction whose edits all net to no-op (e.g. drag-then-undo within the bracket) returns `NoOp`, does **not** advance the revision, **does not publish a new snapshot**, and pushes nothing to history. This mirrors `SaveEngine`'s NoOp byte-identity result (`SaveEngine.h:46`; the drag→undo→NoOp-save behavior is covered by the CST save-fidelity tests post-cutover). Readers continue to observe the prior published snapshot.
...
- **Tests.**
  ...
  - `ExternalFileReconcileTest`: build on the shipped external-mod guard (`SaveEngine.cpp` ~223-247; the pre-cutover `SaveEngineTest.cpp` was deleted in Slice 6d — write fresh coverage or extend `CstSaveFidelityTest.cpp`) — `CheckDiskSync` returns `DiskChanged` after a touch; an in-place `RequestSave` is **refused** (not a silent overwrite); `ResolveAndSave(SaveAsNewPath)` succeeds and **leaves the original disk file byte-identical**; `ResolveAndSave(ReloadFromDisk)` performs the document swap (new UUID, empty history — assert the controller identity changed and `UndoDepth()==0`); `ResolveAndSave(OverwriteDiscardingDisk)` writes exactly `Cst::SerializeCst` of the live Document over the external version, only after the explicit confirm; **no path clobbers/merges/auto-rewrites without the explicit choice** (F1, finding #6).
  - **Correctness invariant** (engine-touching discipline): committing then immediately undoing a transaction yields a scene **byte-identical on save** to the pre-transaction save (re-uses `SaveEngine`'s NoOp byte-identity outcome). No render/RMSE change — the transaction layer is pure orchestration, integrators are byte-identical.
