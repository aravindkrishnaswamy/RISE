...
`IRasterizer::RasterizeScene` already takes `const Rect* pRect` ([IRasterizer.h:72](../../src/Library/Interfaces/IRasterizer.h)), and `Job::RasterizeRegion` wraps it (builds the `Rect`, forwards it). A4 stores an optional inclusive film-pixel rectangle on the controller and passes it only into full-resolution interactive `RasterizeScene` calls. Ordinary production callbacks continue to call `Job::Rasterize` / `RasterizeAnimationUsingOptions` and never consult the viewport region. The explicit regional-final command captures the same bounds in the shell before the coordinated worker starts, then calls `Job::RasterizeRegion` inside that coordinator slot. The persistent framebuffer ([INTERACTIVE_EDITOR_PLAN.md](../INTERACTIVE_EDITOR_PLAN.md) §19.8) means pixels outside the interactive region survive between passes — exactly the "refine the box, leave the rest" behavior.
...
- **Tests.**
  ...
  - *A4 region:* the rect clips a pixel-integrator render (interior pixels match a full render within MC noise; edge pixels differ), survives an ordinary full-frame production dispatch, and is forwarded only by the explicit Render Active Region action; an explicit test asserts a region request under MLT renders **full-frame** (the documented no-op, [MLTRasterizer.cpp:1226](../../src/Library/Rendering/MLTRasterizer.cpp)).
