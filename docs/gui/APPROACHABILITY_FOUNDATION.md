...
| A4 | Region / ROI render | No (the `pRect` param already exists) | Ordinary finals ignore it; explicit regional-final action | 1 |
...
| [Blender Manual: View Regions](https://docs.blender.org/manual/en/latest/editors/3dview/navigate/regions.html) | *"If you define a render region while in Camera View, it will apply not just to the viewport, but also to the final render."* This is the **region-leaks-into-final footgun**. → A4 makes ordinary finals structurally full-frame and exposes a separately named regional-final action. |
...
| Region/ROI: film-loop restriction + explicit regional final (A4) | **all of it** — interactive region state plus coordinated `RasterizeRegion` | Draw Region action/shortcut, editable overlay, Render Active Region command |
...
- **Shared C++:** a new `SceneEditController::ApplyAssetToTarget(assetRef, targetKind, targetName, modifiers)` that routes through `SceneEditor::Apply` (Principle 6 — one mutation path). It resolves an asset-library entry (§9) to the scene-language mutation: material → rebind the object's material chunk; HDRI → **swap** the environment/global radiance map. **⚠ "Round-trippable" is overstated — the assignment is NOT yet round-trippable as a structured edit** ([CURRENT_STATE_AUDIT.md §1, §7](CURRENT_STATE_AUDIT.md); [../GUI_ROADMAP.md](../GUI_ROADMAP.md) §13a #7). Two reasons, both **partly TO-BUILD**:
  - **A library material is a *fragment* that usually has to be *created*, not just rebound.** Applying it generally means **creating** the material chunk (and its painter chunks) the object then binds to — and **entity-creation persistence is camera-only today** (post-CST-cutover the mechanism is a chunk insert into the retained CST Document — `Job::ApplyCstInsertCameraChunk` is the only insert op; the only creation op is `CloneActiveCamera`; the pre-cutover "Phase C" framing is history, see [CURRENT_STATE_AUDIT.md](CURRENT_STATE_AUDIT.md)'s erratum). Material/painter creation is the **ENTITY_CREATION** work ([ENTITY_CREATION.md](ENTITY_CREATION.md)), and painter-property round-trip in particular is TO-BUILD ([MATERIAL_EDITOR.md](MATERIAL_EDITOR.md) §2.1). So a freshly-assigned library material persists only once material/painter **creation** (a per-family Document insert op) lands; until then it is a live-viewport edit that does not survive save/reload.
...
### Interaction
The idle affordance is an action, **Draw Region**, not the noun "REGION." Clicking it enters a one-shot draw mode: the cursor becomes a crosshair and an in-canvas prompt says **"Drag to choose an area · Esc to cancel."** The implementation may call this state "armed," but the product UI must not expose that state-machine term. An expert shortcut may enter the same mode; it must be advertised only after it is actually bound on every desktop shell.

Dragging a rectangle restricts full-resolution interactive refinement to that box (the highest sample-efficiency-per-effort IPR feature, [Arnold ARV](https://help.autodesk.com/cloudhelp/ENU/AR-Maya/files/am-Arnold_for_Maya_User_Guide/rendering/arnold_for_maya_rendering_am_Arnold_RenderView_Window_html)). Coarse preview-ladder passes still refresh the whole frame, so the status must distinguish **"Preparing full-frame preview"** from **"Refining region"** rather than making the initial full-frame work look like a failed region. The overlay persists, lightly de-emphasizes the area outside the box, and reports the region's pixel dimensions and percentage of the film. The active rectangle is direct-manipulation state: drag inside to move; use any of four edge or four corner handles to resize without handle-crossing jumps; or choose **Redraw Render Region** and drag a replacement. Escape and a visible, separately positioned close affordance clear it. Move/resize/clear controls expose semantic accessibility actions with pointer hit targets larger than their visual marks.

### The footgun and its fix
The [Blender region-leaks-into-final behavior](https://docs.blender.org/manual/en/latest/editors/3dview/navigate/regions.html) — *"a render region… will apply not just to the viewport, but also to the final render"* — is the documented #1 footgun. **RISE's rule: the ordinary Production Render and Render Animation actions are always full-frame.** They temporarily suspend (but do not destroy) the viewport region while production owns the scene, then leave it available when interactive work resumes.

**Render Active Region** is a separate, deliberate still-render action, enabled only when a valid active region exists and the production rasterizer honors regions. It captures that rectangle before entering the production coordinator and passes the captured inclusive film-pixel bounds to `Job::RasterizeRegion`; it never changes what the ordinary Production Render action does. Animation remains full-frame. The UI must label a regional final unmistakably in its command and render status, and the persistent overlay keeps the chosen bounds visible. This explicit command replaces the earlier deferred idea of "promote region to final."

### Mapping onto existing machinery (almost free)
`IRasterizer::RasterizeScene` already takes `const Rect* pRect` ([IRasterizer.h:72](../../src/Library/Interfaces/IRasterizer.h)), and `Job::RasterizeRegion` wraps it (validates and clips the inclusive `Rect`, then forwards it). A4 stores an optional inclusive film-pixel rectangle on the controller and passes it only into full-resolution interactive `RasterizeScene` calls. Ordinary production callbacks continue to call `Job::Rasterize` / `RasterizeAnimationUsingOptions` and never consult the viewport region. The explicit regional-final command captures the same bounds in the shell before the coordinated worker starts, then calls `Job::RasterizeRegion` inside that coordinator slot. Final reconstruction and denoising are region-aware too: filtered-film resolve writes only the selected pixels, while OIDN crops beauty and guide buffers to the box and copies only that result back. The persistent framebuffer ([INTERACTIVE_EDITOR_PLAN.md](../INTERACTIVE_EDITOR_PLAN.md) §19.8) means pixels outside the interactive region survive between passes — exactly the "refine the box, leave the rest" behavior.
...
### What ships where
- **Shared C++:** `SceneEditController::SetInteractiveRegion` / `ClearInteractiveRegion`, stable region state across production coordination, and threading the rect into full-resolution interactive `RasterizeScene` calls. The region is already expressed in full-resolution image-pixel space; coarse preview passes intentionally remain full-frame, so there is no scaled region to forward. The C ABI is appended in [RISE_API.h](../../src/Library/RISE_API.h).
- **Platform:** the Draw Region action and expert shortcut, crosshair + in-canvas instruction, editable overlay and measurements, and the explicit Render Active Region command. Both desktop shells capture the same inclusive controller bounds and invoke their existing coordinated `RasterizeRegion` path; neither may bypass the render coordinator.

### Android tier
**Tier B.** Two-finger or long-press-drag to draw the region; ordinary finals remain full-frame and a regional final stays explicit. Lower priority than A1–A3 on mobile.

### Non-goals
No multiple simultaneous regions; no per-region integrator override (that is the auto-router-heatmap territory of Direction D). A region never silently affects an ordinary final (§12 spirit); a regional final happens only through the explicitly named command.
...
| **1c** | A4 region/ROI render + explicit regional final | `pRect` param and `RasterizeRegion` entry point exist; shell/UI wiring | low–medium (coord trap) |
...
- Region render silently affecting an ordinary final — ordinary final callbacks must never consult viewport-region state; only Render Active Region may call `RasterizeRegion`.
...
- **Tests.**
  ...
  - *A4 region:* the rect clips a pixel-integrator render, invalid/out-of-film bounds fail safely, filtered-film resolve and OIDN leave sentinel pixels outside the rectangle unchanged, the selection survives both synchronous and asynchronous ordinary full-frame production dispatches, and only the explicit Render Active Region action forwards it; an explicit test asserts a region request under MLT renders **full-frame** (the documented no-op, [MLTRasterizer.cpp:1226](../../src/Library/Rendering/MLTRasterizer.cpp)).
