...
| Scene save (`SaveEngine`, `DirtyTracker`; post-CST-cutover a whole-Document `Cst::SerializeCst` of the Job's retained CST Document — the byte-splice engine + span indices were deleted in Slice 6d) | `src/Library/SceneEditor/SaveEngine.cpp` (~281 lines) | Byte-exact serialization must be identical everywhere. |
