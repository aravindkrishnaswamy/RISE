...
> **⚠ Painter editing is TO-BUILD — not shipped ([../GUI_ROADMAP.md](../GUI_ROADMAP.md) §13a #7).** The §2 framing "C1 is the existing rows given a panel, *no new serialization*" holds **only for a material's own slots** (scalar literals, enums), whose edits route into the retained CST Document and persist via the whole-Document save. It does **NOT** hold for the **Color swatch + picker** row above, which edits a *painter* chunk: `Painter` is **not** an `EntityCategory` in `DirtyTracker` and is not introspected. Making the colour picker (and any painter-value edit) persist therefore needs **three net-new pieces, all TO-BUILD** (post-cutover the fourth — save re-emit — is free once the edit op routes into the Document):
...
> 2. **Painter edit operations** — `SceneEdit` ops to set a painter's value, routed into the retained CST Document (`ApplyCstParamEdit` on the painter chunk); no `Set*PainterProperty` op exists today.
> 3. **Painter dirty tracking** — add `Painter` to `DirtyTracker`'s `EntityCategory` so an edited painter flags the Save button.
>
...
- **Serialize the graph AS the chunks it already lowers to.** Creating a node = emitting its `*_material` / `*_painter` chunk with a generated unique name; wiring an edge = writing that name into the consumer slot. **[Mechanism updated post-CST-cutover]** The engine that persists this is the retained CST Document + whole-Document save (`Cst::SerializeCst`, minimal-diff — comments / whitespace preserved; the byte-splice engine of [../ROUND_TRIP_SAVE_PLAN.md](../ROUND_TRIP_SAVE_PLAN.md) is history). **What is already shipped vs. what is genuinely new:** edits to a **material entity's** *existing* slot values route into the retained Document and persist today — the same path C1's per-slot `setProperty` uses. **But `Painter` is not yet an editable/introspected family**, so editing a *painter node's* value does **not** persist; painter-property round-trip is **TO-BUILD** (the pieces in the §2.1 note: `PainterIntrospection` + painter edit ops routed into the Document + painter dirty tracking). The other **genuinely new** work is graph-**topology** serialization: **inserting brand-new `*_material`/`*_painter` chunks into the Document** (`Cst::DocInsertItem`, the `ApplyCstInsertCameraChunk` pattern) and rewriting reference tokens for re-wired edges. **C2 Phase B therefore depends on graph-topology Document ops + reference-rewrite + the TO-BUILD painter-property path, NOT on "round-trip save landing"** (the material-entity half has landed). Where a topology edit can't yet be expressed as a surgical chunk insert/reference-rename, the graph editor falls back to rewriting the affected material's **ownership closure** — defined precisely in §3.7a — never to "the material block," because arbitrary scenes have no contiguous block boundary (graph chunks may be scattered, shared between materials, or `FOR`/`DEFINE`-generated). When that closure is ambiguous, the editor **REFUSES** the topology edit with a clear diagnostic rather than guessing a block to clobber.
...
Aligns with [../GUI_ROADMAP.md](../GUI_ROADMAP.md) §11: C1 + spectral-widget subset = roadmap **Phase 3**; C2 + C3 = roadmap **Phase 4** (depends on new graph-**topology** serialization **plus** the TO-BUILD painter-property path — material-*entity* property round-trip already ships (via the retained CST Document post-cutover), but `Painter` is **not** covered: [../GUI_ROADMAP.md](../GUI_ROADMAP.md) §13a #7).
...
- Round-trip persistence (C2 Phase B depends on **topology** Document ops **plus** the TO-BUILD painter-property path — **material-entity** property edits persist via the retained CST Document, but `Painter` is not covered: [../GUI_ROADMAP.md](../GUI_ROADMAP.md) §13a #7): [../ROUND_TRIP_SAVE_PLAN.md](../ROUND_TRIP_SAVE_PLAN.md) (**historical** — its engine was deleted in Slice 6d; see its header).
