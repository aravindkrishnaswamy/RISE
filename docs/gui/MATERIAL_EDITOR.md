...
| **Color swatch** + picker | `Reference` → Painter (string name) | Detect a uniform color painter; show a swatch that opens the **spectral color picker** ([SPECTRAL_DIFFERENTIATORS.md](SPECTRAL_DIFFERENTIATORS.md) D1). Inline RGB literal stays editable as text. **Persistence SHIPPED (doc 88 S4)** — this row *edits a painter chunk*, and the painter chunk's own single-occurrence value now routes into the retained CST Document exactly like a material slot; see the note below. A **range-slider** row field is still TO-BUILD (doc 88 S4b/P5.3). |
...
>
> **Still TO-BUILD (doc 88 S4b/P5.3):** occurrence-addressed editing of a painter's REPEATABLE parameters — a ramp's `stop` lines, an expression painter's `param`/`def` lines. These surface as read-only rows today (`"<param>[<i>]"`, one per occurrence) because the shared CST edit route and the agent/undo chain are fixed at occurrence 0 by documented invariant (`SceneEdit` carries no occurrence field); making occurrence *N* editable means threading `occ` through the capture/restore pairing, scoped as its own follow-up. Also still open, and not painter-specific: the **range-slider** row field (§2.2's `range`/`softRange` descriptor hint) and **thumbnails** (§4).
...
**Material-entity** *and* **painter-node** *property* round-trip are **both already shipped** (post-cutover: the edit routes into the retained CST Document and the whole-Document save persists it; the pre-cutover "SaveEngine Phase-B re-emit" mechanism was deleted in Slice 6d; the painter half shipped in doc 88 S4, `Painter` now a tracked `EntityCategory`, §2.1 note) — editing a *material* or *painter* node's own SINGLE-OCCURRENCE slot value (a scalar literal, an enum, a painter's `expr`/`color`/numeric params) persists today. **One caveat narrows this:** a painter's REPEATABLE parameters (ramp `stop` lines, expression `param`/`def` lines) stay read-only until occurrence-addressed editing lands (doc 88 S4b/P5.3). Graph **topology** is the larger genuinely-new work regardless: there is no general material-*graph* serialization that survives a save/reload of brand-new nodes and re-wired **edges** (which painter feeds which slot of which material). Two layers:
...
Aligns with [../GUI_ROADMAP.md](../GUI_ROADMAP.md) §11: C1 + spectral-widget subset = roadmap **Phase 3**; C2 + C3 = roadmap **Phase 4** (depends on new graph-**topology** serialization **plus** the still-open occurrence-addressed repeatable-painter-parameter path, doc 88 S4b/P5.3 — material-*entity* and painter single-occurrence property round-trip both already ship via the retained CST Document).

| Phase | Deliverable | Depends on |
|---|---|---|
| **C1.0** | Promote Material panel mode to first-class; sliders (descriptor `range`), swatches, presets, `fresnel_mode` disclosure | Descriptor infra (shipped); thin-film introspection (shipped). **Painter swatch *editing* persistence SHIPPED (doc 88 S4)** (§2.1 note: `PainterIntrospection` + edit ops + `Painter` dirty tracking + whole-Document save). The **range-slider** row field and occurrence-addressed repeatable-parameter editing remain open (doc 88 S4b/P5.3). |
...
| **C2.B** | Graph **topology** serialization (new chunks + edge rewrite, §3.7/§3.8); **occurrence-addressed painter repeatable-parameter round-trip (TO-BUILD: ramp `stop` / expression `param`/`def` occurrence editing, doc 88 S4b/P5.3 — single-occurrence painter round-trip SHIPPED doc 88 S4, §2.1 note)**; cycle/orphan/copy-vs-link enforcement (§3.8); connection-legality validator; layout sidecar | Graph-topology emit + reference-rewrite + the still-open occurrence-addressed repeatable-parameter path (round-trip save's **material-entity** AND painter single-occurrence property re-emit both already ship — audit §1, doc 88 S4; **NOT** blocked on "Phase 0 save landing"); reference-safe deletion ([ENTITY_CREATION.md](ENTITY_CREATION.md) §5) |
...
- Round-trip persistence (C2 Phase B depends on **topology** Document ops **plus** the still-open occurrence-addressed repeatable-painter-parameter path, doc 88 S4b/P5.3 — **material-entity** and painter single-occurrence property edits both persist via the retained CST Document today): [../ROUND_TRIP_SAVE_PLAN.md](../ROUND_TRIP_SAVE_PLAN.md) (**historical** — its engine was deleted in Slice 6d; see its header).
