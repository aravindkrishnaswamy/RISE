...
- **Slice 1 — load via the CST (flagged, additive).** A new `Job::LoadAsciiSceneViaCst` reads the file -> `ParseToCst` -> retains the `Document` ->
  ...
  SWITCH (63 P1) is a second D2 consumer: `Job::RederiveCstWithVariant` does the §"D2" full re-derive on each pick (~2 ms..~38 ms over ~1k–16k chunks measured; sub-ms for the tens-to-hundreds-of-chunks hero range), so the heavier-than-Animation switch is bounded, not a footgun.)
