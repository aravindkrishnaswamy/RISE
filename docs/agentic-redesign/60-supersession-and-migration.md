...
> 6's D45–D51** *amend* earlier decisions and win where they conflict; **round 6 also supersedes the
> legacy `docs/gui/RENDER_COORDINATOR.md` + `AI_SECURITY_MODEL.md`** where they conflict — pinned
> renders survive a head change per D47, and commit is semantic-only per D51). The
...
> plus renderConfig + cameraOverride + samplingSeed — *the round-4 axes; round-5 D41/D42
> below refine the asset axis to a content digest and the config/camera axes to
> `effectiveRenderConfigHash` + `viewCameraStateHash`*), compared by **DAG ancestry /
...
> **comment/token-aware** (parse the CST / token stream, never a raw line grep). The
> **round-5** changes folded in here tighten the *runtime* semantics and **correct the
> locked charter**: status carries **both requested and published stamps** with
> `status:ok` ⟺ full-stamp equality on every axis (**D38** amends D13/D29);
> derivation has a **bounded synchronous semantic phase** (lex→parse→CST→bind→reference-
> resolution→typecheck, used by `propose_patch` precommit and rename) in *front of* the
> **async expensive phase** (realize/TLAS/`prepare`/render), the same code, not a second
> resolver (**D39** amends D34/D35/D5); the seed makes **`prepare` deterministic**
> (so the `PreparedArtifact` cache is sound) while the **final render is honestly
> reproducible-within-MC-tolerance, NOT bit-identical** — bit-identical per-pixel/sample
> streams + deterministic reductions/denoise across all renderers is a **named future
> prerequisite** (**D40** amends D33); assets bind by the **content digest of the exact
> loaded buffer** (load-once-and-hash or revalidate-post-load+retry), closing the
> path-hash↔reopen TOCTOU, and the `DerivedStamp` asset axis becomes a **content digest**,
> not a session generation (**D41** amends D5/D17/D29); the `PreparedStamp` carries a
> normalized **`ResolveEffectiveRenderConfig(DerivedScene,request)`** (scene-authored ← request ←
> defaults ← auto-resolution) **+ a view-camera-state pose hash** (not a raw request +
> `CameraId`) (**D42** amends D29/D22); the render arbiter has **two job classes** —
> latest-wins **preview** jobs (newer head cancels) vs stamp-pinned explicit **render**
> jobs (a newer head does NOT cancel) (**D43** amends D34); and **D44 corrects the locked
...
flagged; round 2's **D11–D20**, round 3's **D21–D28**, round 4's **D29–D37**, and round
5's **D38–D44** amend several earlier decisions and win where they conflict (round 4's
**D37** *corrects* a round-3 census error in D27; round 5's **D44** *corrects* the locked
charter's L5/INV-5 identity model, and **D40** honestly weakens the render-determinism
claim). The remaining `10`–`50` facet docs are being
...
- **Managers are name-keyed with identity serials.** `GenericManager<T>`
  ...
  **monotonic per-name serial** (`m_nNextSerial`, `GetItemSerial`) so a remove+re-add
  ...
  "did the instance behind this name change" hook. (This name-keyed serial is the
  Model-A *substrate*; under Model B identity becomes the immutable **`NodeId`** —
  charter L5/INV-5 corrected by **D44**, lineage `NodeId` vs name-path addressing —
  with these serials kept as the underlying store, no longer the identity, §4.4.)
...
| **`DerivedScene` (config-independent)** | NEW (derivation engine) | **NEW (D11/D12/D21/D22/D29/D30/D31/D41)** | `DerivedScene = f(CST, AssetManifest, t)`: immutable, sealed; owns realized/tessellated geometry, materials, lights-as-emitters, **TLAS** (full-rebuild-v1, D24). **Config-independent.** **Identified by a full `DerivedStamp = { cstVersion, assetDigest, animationName, shutterInterval }` (D29 + D41)** — not the CST version alone (t=0 vs t=1, or pre/post asset change, must not collide). **Per D41 the asset axis is a content digest of the actually-loaded buffer** (load-once-and-hash or revalidate-post-load+retry, closing the path-hash↔reopen TOCTOU — R17), a reproducible content-addressed identity, **not** the round-4 session-local `assetManifestGen` (which survives only as a fast in-process change signal). Cache lookups match the **full stamp by equality**, and staleness-vs-head is the **cstVersion axis only via DAG ancestry, never numeric `<`**. The **derivation cache lives on the `DerivedArtifact` keyed by this stamp (D30), NOT on the immutable `Version`** (`Version = { greenRoot, identityRoot, metadata }`) — one Version → many artifacts. **Time is a per-frame derivation input (D21)**, and for motion blur the "time" axis is a **shutter *interval*** with animated quantities baked as immutable `at(τ)` functions/samples (AnimatedTransform-style) the renderer reads **read-only** + a **motion BVH** (**gated: v1 single-time / no motion blur, D31**) — see the animation row. Reverse-dependency-closure COW (D11). | 2 |
| **`PreparedRenderState = prepare(DerivedScene, EffectiveRenderConfig)`** | NEW (derivation engine) | **NEW (D22/D29/D30/D32/D33/D34/D40/D42/D43)** | The **config-dependent** layer: light samplers, **photon maps**, integrator-specific structures. The config input is a **normalized `EffectiveRenderConfig = ResolveEffectiveRenderConfig(DerivedScene, request)` (D42)** — a deterministic merge **scene-authored ← request overrides ← defaults ← auto-resolution** (the resolved integrator/resolution) **+ a sampling seed / RNG-stream identity (D33)**. **Identified by `PreparedStamp = DerivedStamp + { effectiveRenderConfigHash, viewCameraStateHash, samplingSeed }` (D29 + D42)** — D42 supersedes the round-4 `{ renderConfig, cameraOverride, samplingSeed }`: the config axis is the **content hash of the normalized EffectiveRenderConfig** and the camera axis is a **view-camera-state pose hash** (the ephemeral viewport is a pose, not a `CameraId`). The cache lives on the stamp-keyed **`PreparedArtifact` (D30)**. **`prepare` is deterministic (D33):** all stochastic prep (photon tracing) uses the seed, not `rand()`, so it is a pure function of its key → cacheable **and** deterministic. **But the final RENDER is only reproducible-within-MC-tolerance, NOT bit-identical (D40)** — per-worker + global RNGs, tile/splat reduction, and denoise are non-deterministic; bit-identical streams/reductions/denoise across all renderers is a **named future prerequisite** (R18). **Built via a non-mutating `prepare(const DerivedScene&, PreparedRenderStateBuilder&)` (D32)** — `BuildPendingPhotonMaps` + light-sampler construction are refactored off "mutate the Scene" (named prerequisite). Immutable + sealed (D12); the render loop adopts one at a **pass boundary**. A render-time integrator override re-runs only `prepare`, **not** the scene derivation. `prepare`/render run as **cancellable phases of the render arbiter, off the edit thread (D34)**, in **two job classes — latest-wins previews (newer head cancels) vs stamp-pinned explicit renders (a newer head does NOT cancel), D43**. | 2 |
...
| Component | Fate | Why |
|---|---|---|
| `src/Library/Agent/` (MCP server, LLM runtime, providers, credential store) | **NEW (no supersession)** | Does not exist; built on the CST from the start (Facet 5). Model B is the cleanest substrate (read CST/text, propose patch, validate→derive→render). Per **D13/D38** the session publishes a coherent status `{ headVersion, requestedDerivedStamp, requestedPreparedStamp, publishedDerivedStamp, publishedPreparedStamp, status, diagnostics }` — **both requested and published stamps, with `status:ok` ⟺ full-stamp equality on every axis (D38)**: `read_document` is stamped with `headVersion`, `read_graph`/`render` with the published stamps (which may lag or be last-good), and a patch's optimistic-concurrency precondition is checked against `headVersion`. **`validate`/`propose_patch` precommit run the bounded synchronous semantic phase (D39)**, and an explicit "render this" is a **stamp-pinned render job (D43)** (not cancelled by a newer head). |
...
### 4.4 Identity stability (INV-5, L5 — resolved by D9, refined by D14/D15/D25/D26/D30/D35/D36, charter corrected by D44)
...
first-class.) **D44 closes the last gap by correcting the *locked* charter itself:** L5
and INV-5 had read "name-path is the stable identity currency" (now corrected),
which contradicted this section (and D9/D15/D26/D36). D44 rewrites **L5** so identity is the immutable **`NodeId`**
(lineage; survives rename + reparse; in each Version's `identityRoot`) and **name-path**
is the **addressing** scheme, and **INV-5** so durable references (selection, agent refs,
UI bindings, undo lineage) key on the **`NodeId`**, not the name-path — the only round
that edits the locked charter, and a *correction* to make the foundation consistent with
the decisions this section already follows. All identity references in this facet's
inventory (§2.3, §2.11) and cross-facet rows (§5) use NodeId-as-identity / name-path-as-
...
The open questions this section previously carried are **closed** by review rounds 1–5;
...
- **Derived/prepared identity + caching + threading → resolved by D29/D30/D32/D33/D34
  (round-5 D41/D42 refine the stamp axes).** Derived state is keyed by a full
  **`DerivedStamp` / `PreparedStamp`** — round-4 axes (cstVersion + assetManifestGen +
  animationName + shutterInterval, plus renderConfig + cameraOverride + samplingSeed),
  with **D41/D42 refining** the asset axis to a **content digest of the loaded buffer**
  and the config/camera axes to **`effectiveRenderConfigHash` + `viewCameraStateHash`**
  (see the round-5 bullet below) — compared by **DAG ancestry / equality, never numeric `<`** (**D29**);
  ...
  **async + cancellable on the render arbiter, off the edit thread** (**D34**, the source
  ...
  R16.
- **Runtime semantics + the locked charter → resolved by D38/D39/D40/D41/D42/D43/D44
  (round 5).** Round 5 added no new fork — it made the runtime precise and corrected the
  charter: **status carries requested AND published stamps** with `ok` ⟺ full-stamp
  equality (**D38**, R21); **derivation has a bounded synchronous semantic phase**
  (validate/rename) in front of the async expensive phase, the same code, not a second
  resolver (**D39**); **the seed makes `prepare` deterministic but the render is
  reproducible-within-MC-tolerance, NOT bit-identical** — bit-identical streams +
  reductions + denoise across all renderers is a **named future prerequisite** (**D40**,
  R18); **assets bind by the content digest of the actually-loaded buffer** (load-once or
  revalidate+retry), closing the path-hash↔reopen TOCTOU, and the `DerivedStamp` asset
  axis is a **content digest**, not a session generation (**D41**, R17); the
  `PreparedStamp` carries a normalized **`ResolveEffectiveRenderConfig`** + a
  **view-camera pose hash**, not a raw request + `CameraId` (**D42**, R20); the arbiter
  has **two job classes — latest-wins previews vs stamp-pinned explicit renders**
  (**D43**, R19); and **the locked charter L5/INV-5 is corrected** so `NodeId` is lineage
  identity and name-path is addressing (**D44**, §4.4). All are honest
  *prerequisites/risks/corrections*, not forks. §6.2 (R17–R21), §4.4.

---

## 5. Cross-facet dependencies & assumptions

This facet reconciles 1–5. The cross-facet questions are now **pinned by D1–D44**;
...
| **Facet 2 (derivation)** | CST→Scene is incremental/memoized/deterministic; the memo key = the **derivation key** (trivia-INsensitive typed values + child structure, **D15**) + **traced**-input versions, formalized as a full **`DerivedStamp = { cstVersion, assetDigest, animationName, shutterInterval }`** (the asset axis is a **content digest of the loaded buffer, D41**, not the round-4 session `assetManifestGen`) matched **by equality**, with staleness-vs-head on the **cstVersion axis via DAG ancestry, never numeric `<`** (**D29**); emits `Add*` in topological order from traced edges (§4.3); reuses the `IJob` apply layer, `DirtyScope` taxonomy, and realize/TLAS seam; publishes **reverse-dependency-closure COW** snapshots (**D11**) **built → sealed → swapped at a PASS boundary** (**D12**). **Two derivation layers (D22):** the config-**independent** `DerivedScene` owns geometry/TLAS (**full-rebuild v1, D24**); `PreparedRenderState = prepare(DerivedScene, EffectiveRenderConfig)` owns **light-samplers + photon-maps**, keyed by **`PreparedStamp = DerivedStamp + { effectiveRenderConfigHash, viewCameraStateHash, samplingSeed }`** (**D29 + D42** — D42 supersedes the round-4 `{ renderConfig, cameraOverride, samplingSeed }`: the config axis is the hash of a **normalized `ResolveEffectiveRenderConfig(DerivedScene, request)`** = scene-authored ← request ← defaults ← auto-resolution, and the camera axis is a **view-camera-state pose hash**, not a `CameraId`). **Derivation itself splits into two phases (D39):** a **bounded synchronous semantic phase** (lex→parse→CST→bind-to-descriptor→reference-resolution→typecheck; edit-thread-OK; what `propose_patch` precommit and rename use) *in front of* the **async expensive phase** (realize/TLAS/`prepare`/render) — the same code, so it is **not** a second resolver (D35's no-drift holds). **The `DerivedStamp` asset axis is a content digest of the actually-loaded buffer (D41)**, not a session generation. **The memo/dependency cache lives on a stamp-keyed `DerivedArtifact`/`PreparedArtifact`, NOT on the immutable `Version = { greenRoot, identityRoot, metadata }` (D30).** `prepare()` reads the sealed scene through **non-mutating** input APIs into a `PreparedRenderStateBuilder` — `BuildPendingPhotonMaps` + light-sampler construction are a **named refactor prerequisite** off "mutate the Scene" (**D32**); **`prepare` is deterministic** via a `RenderConfig` sampling seed (no `rand()`) in the `PreparedStamp` (**D33**) — **but the final render is only reproducible-within-MC-tolerance, not bit-identical (D40); bit-identical streams/reductions/denoise across all renderers is a named future**; and the whole derive→seal→prepare→seal→render chain runs **async + cancellable on the render arbiter, off the edit thread** (**D34**) with **two job classes — latest-wins previews vs stamp-pinned explicit renders (D43)**. Render-populated caches (irradiance/accum) are **render-local mutable** (D21); **motion blur** is preserved via a **time-INTERVAL** immutable `DerivedScene` + motion BVH (**gated: v1 single-time, D31**). The derivation cache has an explicit edge lifecycle (**D20**). **The O(closure)/O(log N) headline is gated on persistent immutable containers (D23)** — v1 may copy-on-snapshot at O(N) with honest claims. History = CST-only; re-derive uses current asset bytes (**D28**). | **D1, D4, D5, D11, D12, D15, D17, D20, D21, D22, D23, D24, D28, D29, D30, D31, D32, D33, D34, D39, D40, D41, D42, D43** |
| **Facet 3 (edit model)** | Undo/redo = CST version-DAG; a gesture = one undo unit; the `SceneEditor` mutation surface re-points onto the CST; invariant chain + manager-resolution survive as the apply path; **lineage identity is `NodeId`** (immutable; survives rename + reparse; in each Version's persistent identity side-map, D15/D26) and **name-path is the *addressing* scheme** (resolves to a NodeId within a version; changes on rename) — **D44 corrects the locked charter L5/INV-5 to this exact split** (durable references key on NodeId, not name-path), making the foundation consistent with D9/D15/D26/D36; the `Version` is **`{ greenRoot, identityRoot, metadata }`** — CST + occurrence identity only, cache off-Version on artifacts (**D30**); **both roots `{greenRoot, identityRoot}` propagate through edits (the `GestureBuffer`/working head), and `NodeId` is stored in `Widget`/`ViewNode`/`EditIntent`/selection — not name-path (D36)**; **rename rewrites referrers from traced `ReferenceUse` records (D14), not `referenceCategories`, against a head-stamped trace using the ONE derivation resolver — concretely the bounded synchronous semantic phase derived to head (D39), no second tracing pass — refused if head can't be derived (D25/D35)**; the session exposes **both `headVersion` and `derivedVersion`** (D13), and status carries **both requested and published stamps with `ok` ⟺ full-stamp equality (D38)**; deletion-persistence grows to all families. | **D1, D9, D13, D14, D15, D25, D26, D30, D35, D36, D38, D39, D44** |
| **Facet 4 (dynamic UI)** | UI is a pure function of CST + descriptors; widgets bind to **`NodeId`** (the stable lineage **identity**, D15/**D44**) addressed by **name-path** (addressing, not identity — D44); the 5 descriptor-driven introspection files rebind to CST nodes; hand-built accordions retire; the C-ABI boundary gains CST/text entry points (and surfaces `headVersion` vs the published stamps with `ok` ⟺ full-stamp equality, D13/**D38**). | **D9, D13, D15, D38, D44** |
| **Facet 5 (agent)** | `src/Library/Agent/` is net-new on the CST (read/propose-patch/validate→derive→render); GUI is "just another agent"; the `docs/gui/` agent specs survive as text-level. **`propose_patch` precommit + `validate` run the bounded synchronous semantic phase (D39)** (lex→parse→CST→bind→reference-resolution→typecheck — no asset I/O, no realization), not the full async job; an explicit agent "render this" is a **stamp-pinned render job (D43)** (a newer head does not cancel it), distinct from latest-wins viewport previews; status the agent polls carries **requested vs published stamps (D38)**. | **D38, D39, D43** (refine L2) |

**No conflict with D1–D44 (or L1–L7) remains in this facet.** The former hardest
fork — O1 (canonical form) — is closed by **D2**, so `SaveEngine`'s fate is no longer
contingent; rounds 3–5 added no new fork, only honest scoping and two **factual
corrections**: animation/irradiance kept and re-expressed (D21); motion blur preserved
as a time-interval scene, gated (D31); the DerivedScene/PreparedRenderState split
(D22) keyed by full stamps (D29) — now an `effectiveRenderConfigHash` + view-pose hash
(D42) — with the cache on artifacts not the Version (D30); a non-mutating, async-on-the-
arbiter `prepare` that is deterministic while the render is honestly
reproducible-within-tolerance not bit-identical (D32/D33/**D40**); two arbiter job
classes — latest-wins previews vs stamp-pinned renders (**D43**); a bounded synchronous
semantic phase in front of the async job (**D39**); assets bound by content digest of
the loaded buffer (**D41**); status carrying requested vs published stamps (**D38**); the
...
(D36); the full `>` set migrated/retired (D27); CST-only history (D28); **D37's
correction that the active corpus has ZERO `> modify` (the seven are commented-out) and
the migrator must be comment/token-aware**; and **D44's correction of the locked charter
L5/INV-5 so NodeId is lineage identity and name-path is addressing**. The only remaining
empirical unknown is the corpus reference-graph acyclicity check (a P0 scan, §4.3), which
is a verification step, not a design decision.
...
| **R4** | **Agent-edit safety** — an agent (or GUI-as-agent) commits a CST edit that is syntactically valid but semantically destructive (deletes a referenced node, writes an invalid material), and undo/version-history is the only firewall. | **Med** | **High** | the **bounded sync semantic phase** (parse/resolve/typecheck, D39) gates every agent patch *before* commit — but **commit is semantic-only (D51)**: asset-load/realize/prepare/render run async *after* commit, so a committed head can be a **broken-but-valid CST** (surfaced as `error` phase + diagnostics, never silent corruption), and a caller needing the stronger guarantee uses the opt-in `awaitFullValidation` mode (so the legacy `AI_SECURITY_MODEL.md` "full precommit gating" promise is **superseded** by D51); reference-safe deletion (block/cascade, from `ENTITY_CREATION.md`) at the CST level; CST version history makes every agent action atomically revertable; the `AI_SECURITY_MODEL.md` *threat model* (injection/SSRF/secrets/jail) still carries over. CST diffs make agent actions diff-reviewable (a Model B *advantage*). |
...
| **R11** | **Asset-change invalidation missed (D5/D17/D41)** — a referenced texture/mesh/spectral/glTF changes on disk without the CST or filename changing; a clean derive then disagrees with a cache hit (stale render, or a migrated scene that silently diverges from its v6 original). **Bytes can change with `(size,mtime)` unchanged**, so mtime alone is not deterministic. | **Med** | **Med-High** | **D5+D17+D41**: derivation input is `(CST, AssetManifest)`; asset identity = **`(size,mtime)` fast prefilter → content hash** (authoritative; memo keys use the content hash). **Per D41 the hash MUST be the content digest of the exact bytes the loader actually consumed**, not of a separately-stat'd path — see **R17** for the path-hash↔reopen TOCTOU. The fingerprint is **traced** (D4) into the memo keys of the nodes that consumed it, so a change invalidates exactly those nodes. A file watcher (or re-stat on focus/render) drives invalidation. Output paths excluded. Gate **G5** exercises this on the first slice (§D18 phase 6's asset-backed node). |
...
| **R16** | **`prepare()` is mutating, non-deterministic, and would freeze the edit thread (D32/D33/D34)** — photon maps are `Scene`-owned and `BuildPendingPhotonMaps` **mutates** pending flags/maps/gather params (`Scene.cpp:750`); light samplers are `RayCaster`-owned; photon tracers seed RNGs with `rand()` (`RandomNumbers.h:32`) so the same `(scene, config)` yields different maps (not cacheable/reproducible); and photon-map construction takes seconds + all cores, so running it synchronously on the UI/agent edit thread would freeze it. | **Med** | **Med-High** | **Three coordinated fixes, named as prerequisites:** **D32 —** refactor `BuildPendingPhotonMaps` + light-sampler construction from "mutate the Scene" to a **non-mutating `build(const DerivedScene&, PreparedRenderStateBuilder&)`** (read the sealed scene through const APIs, write a separate builder, then seal); **D33 —** `RenderConfig` carries a **sampling seed / RNG-stream identity** used by all stochastic prep instead of `rand()`, and the seed is part of the `PreparedStamp` (D29) so **`prepare` is a pure function of its key → cacheable AND deterministic** (same `PreparedStamp` → same photon maps; this is all the cache soundness needs). **NB (D40): the seed makes `prepare` deterministic, but the *final render* is only reproducible-within-MC-tolerance, NOT bit-identical** — the earlier "deterministic renders" claim is weakened accordingly (see **R18**); **D34 —** derive→seal→prepare→seal→render run as **cancellable phases of the render arbiter, off the edit thread** (the edit thread only commits a cheap CST Version; a newer head cancels and restarts at the new stamp — but see **R19**: only *preview* jobs are latest-wins, pinned renders are not cancelled by a newer head). These are real machinery work, not a fork; they gate the `PreparedRenderState` layer (R2's latency story depends on D34). §2.2. |
| **R17** | **Asset path-hash has a TOCTOU vs the loader's reopen (D41)** — the manifest hashes a *path*, but loaders **reopen that path later** (`TriangleMeshLoaderPLY.cpp:795`), so a file can change between the hash and the load and the artifact is stamped with the **wrong identity** (a non-reproducible cache hit). A session-local generation counter is also not a reproducible identity. | **Low-Med** | **Med-High** | **D41: bind asset bytes by the content digest of the actually-loaded buffer**, not a separately-stat'd path. Two acceptable shapes: **load-and-hash one buffer** (read the file once, hash *that* buffer, hand the same buffer to the loader) or **revalidate after load** (re-hash the loaded bytes vs the manifest; on mismatch, retry/refuse). The **`DerivedStamp` asset axis is a content digest** (per-asset content hashes), a **content-addressed identity** suitable for reproducibility — the session generation counter survives only as a fast in-process change *signal*, never as the stamp axis. Pairs with R11 (invalidation) and R12 (the save-side atomic-rename guard, which is the write-side analogue of the same read-once discipline). **D46: a composite asset** (e.g. glTF, which pulls in external buffers + textures, `GLTFSceneImporter.cpp:1849`) identifies by the content digest of its **whole transitive byte closure** — a direct-path digest would miss those buffers — and a **pinned render job pins the closure** so a queued render reproduces regardless of later disk changes. §2.4. |
| **R18** | **"Diffable / deterministic renders" is over-claimed — the render is NOT bit-identical (D40)** — renderers use per-worker independently-seeded RNGs **plus** `GlobalRNG()` (`RasterizeDispatchers.h:146`, `PixelBasedRasterizerHelper.cpp:897`), and tile assignment, splat reduction, and denoise are also non-deterministic; a single seed does **not** yield byte-identical images, so a literal byte-diff of two renders of the "same" stamp is misleading. | **Med** | **Med** | **D40 (honest weakening): `prepare` IS made deterministic by the seed** (cache-sound, R16); **the final render is reproducible *within MC tolerance*** (same scene/config/seed → the same converged image up to Monte-Carlo noise), so the git-native review story is **review-by-image, not byte-diff**. **Bit-identical rendering** — deterministic per-pixel/per-sample RNG streams keyed by pixel+sample, deterministic splat reduction, deterministic denoise, **across every renderer** — is a **named future prerequisite**, not v1 (sized alongside the persistent-container work, R13). Document the claim as "reproducible-within-tolerance" everywhere the diffable-render thesis appears. §2.2, §4.6. |
| **R19** | **One cancel-on-newer-head policy silently destroys a requested final render (D43)** — D34 cancels every in-flight render when a newer head arrives; but the agent surface allows explicit pinned final/export renders, and an unrelated edit landing mid-render would throw that work away. | **Low-Med** | **Med** | **D43: two arbiter job classes.** **Preview jobs are latest-wins** — interactive viewport previews track head, a newer head cancels the in-flight preview (D34's policy; ephemeral, cheap, never pinned). **Pinned render jobs are stamp-pinned** — an explicit "render *this* stamp" (final/export) pins to its `requestedPreparedStamp`; a newer head does **not** cancel it (only its requester can — D47, superseding the legacy `RENDER_COORDINATOR.md` revision-stale-drop). **D48 (single render slot):** RISE's single-render invariant holds — exactly one render at a time, so while a pinned render owns the slot **previews SUSPEND/queue (not "alongside")**, the newest pending preview running when it frees. **D50:** pinned renders are a **`RenderJobId`-keyed set** with per-job phase + **targeted stop/pause/resume** (the single requested/published-stamp surface describes the *one* preview). R2/R16's "newer head cancels in-flight work" applies to **previews only**. §2.2. |
| **R20** | **No defined merge for render config; config can't resolve from CST alone; `CameraId` can't name the live viewport pose (D42/D45)** — scene-authored rasterizer/integrator settings, request overrides, defaults, and auto-resolution (e.g. `auto_rasterizer`, which **inspects the assembled scene and may run a probe render**, `AutoRasterizer.cpp:333`) have no specified precedence and **cannot be resolved from the CST alone**; and a `CameraId` cannot identify the continuously-changing ephemeral viewport camera. | **Med** | **Med** | **D42/D45: `ResolveEffectiveRenderConfig(DerivedScene, request) → EffectiveRenderConfig` is a named build item, run AFTER `DerivedScene` exists** (so auto-routing can inspect the scene + probe-render; the probe takes the one render slot briefly) — a deterministic merge **scene-authored ← request overrides ← defaults ← auto-resolution** (the resolved integrator/resolution); the **normalized result + its content hash** go in the `PreparedStamp`, not the raw request. The viewport camera is an **ephemeral pose, not a `CameraId`**: stamp a **content hash / generation of the complete view-camera state** (pose + lens). So `PreparedStamp = DerivedStamp + { effectiveRenderConfigHash, viewCameraStateHash, samplingSeed }` (supersedes the round-4 `{ renderConfig, cameraOverride, samplingSeed }`). §2.2, §5 (Facet 2 row). |
| **R21** | **Status can't show what the arbiter is *trying* to produce (D38)** — the round-4 status surface published only what the arbiter *produced*; when time/assets/camera/config change without changing `headVersion`, a client can't tell whether it is up to date. | **Low** | **Med** | **D38: status exposes BOTH requested and published stamps** — `{ headVersion, requestedDerivedStamp, requestedPreparedStamp, publishedDerivedStamp, publishedPreparedStamp, status, diagnostics }`, with the requested stamps set when **any** input axis changes (not just the CST head). **`status:ok` requires full-stamp equality** (`published == requested` on every axis — cstVersion, assetDigest, animation, shutter, effective-config, view-camera, seed); otherwise `deriving|preparing|rendering`. **D49: `ok`/done ALSO requires `phase==complete`** — the PreparedRenderState is published *before* rendering starts, so stamp-equality alone is true throughout the render; the `phase` (idle/deriving/routing/preparing/rendering/complete/error) is what attests the output exists. Builds on the D13 `headVersion`/`derivedVersion` split (§5, Facet 3 row). |
...
NEW(D11/D12/D22/D29) · `DerivedStamp` = `{ cstVersion, assetDigest, animationName, shutterInterval }`,
asset axis = **content digest of the loaded buffer** (load-once/revalidate+retry, closing
path-hash↔reopen TOCTOU; not session `assetManifestGen`) NEW(D41) · `DerivedStamp`/`PreparedStamp` =
full derived/prepared identity, compared by DAG ancestry/equality not `<` NEW(D29) ·
`DirtyTracker` DELETE ·
...
identity side-map (lineage **identity**, D15/D26; charter L5/INV-5 corrected to NodeId-identity
vs name-path-addressing, D44), name-path addressing (D9/D15/D44), stored in
...
prepare(scene, EffectiveRenderConfig), config-dependent (light-samplers/photon-maps), keyed by
`PreparedStamp = DerivedStamp + { effectiveRenderConfigHash, viewCameraStateHash, samplingSeed }`
(D42 supersedes round-4 `{renderConfig,cameraOverride,samplingSeed}`), non-mutating +
deterministic-`prepare` + async-on-arbiter NEW(D22/D29/D32/D33/D34/D42) · **render is
reproducible-within-MC-tolerance, NOT bit-identical; bit-identical = named future** (D40) ·
`ResolveEffectiveRenderConfig(DerivedScene,request)` = scene-authored←request←defaults←auto-resolution,
normalized+hashed into the stamp; view-camera = pose hash not `CameraId` NEW(D42) ·
`prepare`/derive run async + cancellable on the render arbiter, off the edit thread, in TWO
job classes — latest-wins previews vs stamp-pinned explicit renders NEW(D34/D43) ·
status carries requested AND published stamps, `ok` ⟺ full-stamp equality NEW(D38) ·
derivation = bounded SYNC semantic phase (validate/rename) + async expensive phase, same code
NEW(D39) · `RawTokenCapture` trivia seeds green tree (index not retained) · `ReferenceUse`
...
D34) · `Reference.h` UNCHANGED · `RenderConfig`→`EffectiveRenderConfig` (resolved+normalized+hashed,
D42) carries sampling seed/RNG-stream identity (deterministic `prepare`, render only
reproducible-within-tolerance D40) NEW(D33/D40/D42) · rename head-stamped trace via the ONE derivation
