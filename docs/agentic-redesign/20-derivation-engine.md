...
> §2.6/§7 note conformance.) **Round 5 makes the two-phase split, determinism honesty,
> asset identity, config resolution, and cancellation precise:** **D39** — derivation
> splits into a **bounded SYNCHRONOUS semantic phase** (lex → parse → CST →
> bind-to-descriptor → **reference resolution / traced `ReferenceUse`** → type/pipe/
> typecheck; no asset I/O beyond identity, no realization; deterministic, edit-thread-OK)
> and the **asynchronous expensive phase** (realize/tessellate, TLAS, `prepare`, render —
> D34's arbiter job); the semantic phase **is the front of the async job** (same code, no
> second resolver — D35's no-drift holds), and is what `propose_patch` precommit and
> rename (D35) use synchronously (this *refines* the round-4 "all derivation is async"
> wording everywhere below); **D40** — the seed makes **`prepare` (photon maps)
> deterministic** (so the `PreparedArtifact` cache is sound), but the **final RENDER is
> NOT bit-identical** — it is **reproducible within Monte-Carlo tolerance** (per-worker
> RNGs + `GlobalRNG()` + non-deterministic tile assignment / splat reduction / denoise);
> **bit-identical rendering** (deterministic per-pixel/per-sample streams + deterministic
> reductions/denoise across *every* renderer) is a **named future option**, not v1 (this
> weakens D33's "reproducible render" claim); **D41** — the asset's identity is the
> **content digest of the exact bytes the loader consumed** (load-and-hash one buffer, OR
> revalidate-post-load + retry — the path-hash has a TOCTOU vs the loader's reopen); the
...
> so **`PreparedStamp = DerivedStamp + { effectiveRenderConfigHash, viewCameraStateHash,
> samplingSeed }`** (replaces D29/D22's raw `renderConfig` + `cameraOverride: CameraId`);
> **D43** — the arbiter has **two job classes**: **preview jobs (latest-wins** — a newer
> head cancels the in-flight preview, D34's policy) and **pinned render jobs (stamp-pinned**
> — an explicit "render *this* stamp" is pinned to its `requestedPreparedStamp`; a newer
> head does **NOT** cancel it, it runs to completion / is queued), so D34's "newer head
> cancels" applies to **previews only**; **D38** — derivation/arbiter status carries
> **requested vs published** stamps and **`status:ok` ⟺ full-stamp equality** (the requested
> stamps are set when *any* input axis changes, not just the CST head); and **D44** — the
> *locked* charter L5/INV-5 is corrected so the foundation matches D9/D15/D26/D36 (the
...
> prepare(DerivedScene, EffectiveRenderConfig)`** (D22/D42) — both made *pure*,
> *deterministic*, *incremental*, and *memoized* (the *layers* are deterministic; the
> seeded `prepare` makes the photon maps reproducible — D33 — while the *render* off them
> is reproducible only within MC tolerance, not bit-identical — D40), each producing an
> **immutable sealed snapshot via copy-on-write** (D1/D12); a **bounded synchronous
> semantic phase** (validate/rename) fronting the async expensive phase (D39); the
> **traced** CST→engine dependency graph (D4); memo
...
DerivedStamp     = { cstVersion, assetDigest, animationName, shutterInterval }        // identifies a DerivedScene (D29; asset axis is a CONTENT digest — D41)
DerivedArtifact  = { derivedStamp, derivedScene, derivationCache }   // the cache lives HERE (D30)
PreparedStamp    = DerivedStamp + { effectiveRenderConfigHash, viewCameraStateHash, samplingSeed }  // identifies a PreparedRenderState (D29/D33/D42)
...
digest / animation) and each of those many `PreparedArtifact`s (one per
`EffectiveRenderConfig` / view-camera state / seed) — one Version → many artifacts (D30).
**Cache lookups match the full stamp by equality** (D29). The only **ordered** comparison is
...
axes (`assetDigest`, `shutterInterval`, `effectiveRenderConfigHash`, `viewCameraStateHash`,
…) are equality-matched, not ordered (D29/D41/D42). The layered model:
...
prepare : (DerivedScene, EffectiveRenderConfig)  →  PreparedRenderState   (D22; EffectiveRenderConfig carries the seed — D33; resolved + hashed — D42)
...
prepare : (DerivedScene, DerivedScene', EffectiveRenderConfig, PrepareCache)
              →  (PreparedRenderState', PrepareCache')      (incremental — INV-3; cache held on the PreparedArtifact — D30)
```

**Derivation runs in two phases (D39): a bounded SYNCHRONOUS semantic phase + the
asynchronous expensive phase.** `derive` is *not* monolithically async (the round-4
"all derivation runs async on the arbiter" — D34 — is refined here):

- **Synchronous semantic phase (bounded, deterministic, edit-thread-OK — D39):** lex →
  parse → CST → bind-to-descriptor → **reference resolution (traced `ReferenceUse`,
  §2.2/§2.6)** → type / pipe / typecheck. Output: a validated CST + the reference graph
  + structured diagnostics (§5.1). It does **no asset I/O beyond identity** (it may read
  the AssetManifest's `(size, mtime)` prefilter / cached digest, but loads no asset
  bytes) and performs **no realization**. It is what `propose_patch` precommit and
  **rename (D35)** invoke *synchronously* on the edit thread. **Scope (D39):** it
  resolves references to **CST-declared** name-paths; references *into* asset-expanded
  sub-entities (e.g. a glTF import's children) need the async phase and are **out of v1
  cross-reference scope**.
- **Asynchronous expensive phase (the arbiter's cancellable job — D34/D39):**
  realize / tessellate (this is where asset *bytes* load — §2.9), the **full TLAS
  rebuild** (D24), then `prepare` (light samplers + photon maps) and render.

The semantic phase **is the front of the same async job** (it runs first, then the
async phase continues from its validated CST) — so it is **not a second resolver**, and
D35's "one resolution implementation / no drift" holds: a synchronous `propose_patch` /
rename runs *exactly* the semantic phase the arbiter's job begins with (§2.2/§2.8). This
is the precise sense in which `derive` is "deterministic, incremental, memoized" *and*
parts of it are safely synchronous.
...
**`PreparedRenderState = prepare(DerivedScene, EffectiveRenderConfig)` (config-DEPENDENT
...
photon-consuming integrators), and any integrator-specific structures. **The config input
...
The **sampling seed / RNG-stream id** (D33) is part of the resolved config. The viewport
camera is an **ephemeral pose** (continuously changing), so it is identified by a
**content hash of the complete view-camera state** (pose + lens — `viewCameraStateHash`,
D42), **not** a `CameraId` (a `CameraId` cannot name a pose that mutates every frame).
The `PreparedStamp`'s two non-`DerivedStamp` config axes are therefore exactly
`effectiveRenderConfigHash` + `viewCameraStateHash` (plus the seed — D29/D42). Because
the split is clean, the **render-time integrator override re-runs only `prepare`, never
scene derivation**: the geometry/materials/TLAS in `DerivedScene` are reused by reference
and only the samplers/photon maps are rebuilt for the new (re-resolved)
`EffectiveRenderConfig`. The light samplers and photon maps are RayCaster-owned today
...
from the `EffectiveRenderConfig` seed / RNG-stream id, **not** `rand()` (photon tracers
seed with `rand()` today — `RandomNumbers.h:32` — so the same `(DerivedScene,
EffectiveRenderConfig)` currently yields *different* photon maps). With the seed in
`PreparedStamp`, `prepare` is a pure function of its key → **cacheable and reproducible**:
the same `PreparedStamp` yields the **same photon maps** (this is exactly what the
`PreparedArtifact` cache soundness needs — D40). **The final RENDER, however, is NOT
bit-identical (D40, honest weakening).** Renderers use per-worker independently-seeded
RNGs + `GlobalRNG()` (`RasterizeDispatchers.h:146`,
`PixelBasedRasterizerHelper.cpp:897`), and tile assignment, splat reduction, and denoise
also vary — so a single seed does **not** yield a byte-identical image. The render is
**reproducible within Monte-Carlo tolerance** (same scene / config / seed → the same
converged image up to MC noise), so the git-native "diffable renders" thesis weakens to
**review-by-image, not byte-diff**. **Bit-identical rendering** — deterministic
per-pixel / per-sample RNG streams keyed by pixel+sample, deterministic splat reduction,
deterministic denoise, *across every renderer* — is a **named future option, not v1**
(§6.8).
...
Determinism (INV-2) means: each *derived layer's* result is a pure function of its
inputs' *content* — `DerivedScene` of `(CST, manifest, t, animationName)` (its
`DerivedStamp`, whose asset axis is a **content digest** — D41), `PreparedRenderState`
of `(DerivedScene, EffectiveRenderConfig)` **including the sampling seed** (its
`PreparedStamp` — so even stochastic photon prep is reproducible, D33) — independent of
node *visit order*. **This determinism is the layers', not the render's:** `prepare` is
bit-reproducible (D33), but the **render** off a given `PreparedRenderState` is only
reproducible **within MC tolerance** (D40, above) — bit-identical rendering is a named
future (§6.8). §4 audits where today's `Job` violates the *layers'* determinism and
what must change.
...
**records the edge it traversed** (the reactive-build / "tracing" model). **Which
phase records which edge follows the D39 split:** the **name-ref / value / expression
edges** (the `ReferenceUse` records, §2.6) are recorded by the **synchronous semantic
phase** (reference resolution + typecheck, no asset bytes loaded — D39); the **asset
edges** (the `assetDependents` index) are recorded by the **asynchronous realize phase**,
which is the only phase that loads asset bytes (§2.9). So `propose_patch` precommit and
rename — which run only the sync semantic phase — see the full name-ref graph but not the
asset edges (which need the async phase; D39 scope). Nodes and the edges that carry
invalidation:
...
```
   ...
   RealizedGeometry
        │
        ▼ (world bbox)
      TLAS  ──────────────►  PhotonMaps (prepare — only if EffectiveRenderConfig rasterizer ConsumesScenePhotonMaps, D22)
...
**`PreparedRenderState`** built by `prepare(DerivedScene, EffectiveRenderConfig)` (the
config input is the resolved + hashed `EffectiveRenderConfig` — D42, §2.1).
...
| L5 *light topology* | `LightSampler` + `EnvironmentSampler` | `RebuildLightSamplers` (`prepare`) | light add/remove, emissive-material bind/unbind, exitance edit, **spatial change of an emissive object**, env map replace/scale, **OR the `EffectiveRenderConfig` light-sampling strategy changes** |
| L6 *photon maps* | caustic/global/shadow/translucent maps | `BuildPendingPhotonMaps` (`prepare`) | any L2/L3/L5 change **and** the active rasterizer (an `EffectiveRenderConfig` input) consumes them |
...
built by `prepare(DerivedScene, EffectiveRenderConfig)`, not by `derive`, because the
light-sampling strategy and the photon-consuming gate are `EffectiveRenderConfig` inputs
(D42, §2.1).
...
key(entity) = H( derivation_key(node),                       // typed values + child STRUCTURE, NO trivia (D15)
                 ...
                 { (asset_path, content_digest)  for each TRACED asset input } )  // D5/D17/D41 — digest of the LOADED bytes
```
...
digest / `EffectiveRenderConfig` / view-camera / seed). A `Version` commits *before* async derivation
...
```cpp
// Held on the DerivedArtifact, keyed by its DerivedStamp = {cstVersion, assetDigest,
//   animationName, shutterInterval} (D29/D30; assetDigest is a CONTENT digest — D41) — NOT on the immutable Version.
...
struct DerivationCache {
    ...
    // since they depend on EffectiveRenderConfig (the light-sampling strategy / photon gate).
};

// Held on the PreparedArtifact, keyed by its PreparedStamp = DerivedStamp +
//   {effectiveRenderConfigHash, viewCameraStateHash, samplingSeed} (D29/D30/D33/D42). Same D23 gating:
...
Given `(CST, CST', AssetManifest, t, cache)`. **Steps 1–2 are the bounded SYNCHRONOUS
semantic phase (D39)** (key recompute = the trivia-insensitive derivation key + the
*name-ref / expr* traced inputs; no asset bytes loaded); **steps 3–4 — and step 1b's
asset-driven seeding — are the asynchronous expensive phase** (the realize phase loads
asset bytes, §2.9). The split is *which phase*, not *which code*: the sync phase is the
front of the same arbiter job (§2.8):
...
1. **Seed the dirty set** from three sources: (a) the **CST diff** CST→CST′ at node
   ...
   **asset content-digest change** (D5/D41 — the digest of the bytes the loader
   actually consumed, not a path-hash) → seed every consumer via `assetDependents`. The
   manifest delta is produced by the watcher / re-stat (§2.9); this seeding belongs to
   the **async realize phase** (it is the only phase that touches asset bytes — D39);
   (c) a **change of the frame's time interval** (`shutterInterval`, a
...
4. **Build into a mutable builder, run the config-independent phase B on it, *seal*
   ...
   **This whole step 4 is the asynchronous expensive phase of the render arbiter's job,
   off the edit thread (D34/D39)** — the edit thread only committed the cheap CST
   `Version` and (for `propose_patch`/rename) may have run the bounded **synchronous
   semantic phase** of steps 1–2 inline (D39); the arbiter re-runs that semantic phase as
   the *front* of its job and continues into this expensive phase. If a newer head arrives
   mid-`derive`/`prepare`/render, **whether the in-flight phases cancel depends on the job
   class (D43): a preview job is latest-wins (a newer head cancels + restarts at the new
   stamp); a pinned render job is stamp-pinned (a newer head does NOT cancel it — it runs
   to completion / is queued)** (§2.8). The COW re-derive produces a **mutable `DerivedSceneBuilder`** (a closure-copy view
   ...
   **fully rebuilds the TLAS** (v1 — D24; no path-copy). Then **`seal()` →** an
   ...
   (config-independent — D22). **`prepare(DerivedScene, EffectiveRenderConfig)` then builds
   ...
   the **photon maps** iff pending + the active rasterizer (an `EffectiveRenderConfig`
   input) consumes them — with all stochastic prep drawing from the `EffectiveRenderConfig`
   **sampling seed** rather than `rand()`, so the prepared photon maps are reproducible
   (D33; the *render* off them is reproducible only within MC tolerance — D40); it **seals** an
...
- **`CanTessellate` / parse-refusal:** a `displaced_geometry` over a
  ...
  CST node (Facet 5 surfaces it). `CanTessellate` is a **semantic check** (no asset bytes
  needed), so it is checked in the **bounded synchronous semantic phase** (D39) — at
  derive time, not realize time — so the error is reported before any render is attempted
  (and before the async realize phase runs).
- **Photon-pass gating (config-dependent — `prepare`):** L6 is part of the
  **`PreparedRenderState`** *iff* the active rasterizer's `ConsumesScenePhotonMaps()`
  ...
  The active rasterizer is an **`EffectiveRenderConfig` input** to `prepare` (the resolved
  rasterizer selection — D42). Changing it from `pixelpel_rasterizer` (consumes) to
  `pathtracing_pel_rasterizer` (doesn't) **removes L6 from the prepared state** — no
  photon shoot, and any pending photon dirtiness is irrelevant — and re-runs **only
  `prepare`** (the `DerivedScene` is unchanged and reused by reference — D22). Changing
  back re-adds L6 and marks it pending. This makes the existing runtime gate a
  *prepare-layer membership* decision driven by `EffectiveRenderConfig` plus the traced
  photon dirtiness.

### 2.8 Threading: a bounded SYNC semantic phase + the async (derive→seal→prepare→seal→render) phase as arbiter jobs; pass-boundary swap, not parking; previews latest-wins, pinned renders stamp-pinned (D1, D12, D22, D34, D38, D39, D43)

**The edit/agent thread does only two bounded things: commit a CST `Version` (D34), and
— for `propose_patch` precommit and rename (D35) — run the bounded SYNCHRONOUS semantic
phase (D39).** That semantic phase (lex → parse → CST → bind-to-descriptor → reference
resolution / traced `ReferenceUse` → type/pipe/typecheck) is **deterministic and
edit-thread-OK**: it loads **no asset bytes** and does **no realization**, so it cannot
freeze the UI. **Everything expensive — realize/tessellate (loads asset bytes) → full
TLAS rebuild → `prepare` (samplers/photons) → render — runs as the ASYNCHRONOUS phase of
the render arbiter's job, off the edit thread** (D34/D39). This is non-negotiable because
photon-map construction and a full TLAS rebuild take seconds and all cores; running them
synchronously on the UI/agent thread would freeze it (and would contradict the
single-render-arbiter promise). **The semantic phase is the *front* of the same arbiter
job** (the arbiter re-runs it, then continues into the expensive phase), so there is no
second resolver and D35's no-drift holds (D39). The arbiter picks up each **debounced CST
head (O2)** — both a **commit** *and* a **gesture preview of the uncommitted head** (D1,
§5.3) — at its `DerivedStamp` (D29), runs the semantic phase, builds into a **mutable
`DerivedSceneBuilder`**, runs the **config-independent expensive phase B on the builder**
(realize/tessellate + TLAS), **seals** it to a **new immutable `DerivedScene`** value
(D12/D22); then **`prepare(DerivedScene, EffectiveRenderConfig)`** — reading the sealed
`DerivedScene` through **non-mutating const input APIs** and building into a
**`PreparedRenderStateBuilder`** (D32), seeded deterministically from the
`EffectiveRenderConfig` seed (D33) — builds and **seals** a **`PreparedRenderState`**
(light samplers + photon maps), and the session **publishes only the sealed
`PreparedRenderState` by an atomic pointer swap at a PASS boundary** (D12) — it does
**not** park the render thread for safety, and it **never mutates after publication**.

**Two arbiter job classes — cancellation differs (D43).** When a newer head arrives, what
happens to the in-flight job depends on its class:
- **Preview jobs — latest-wins (D43):** an interactive viewport preview tracks head; a
  newer head **cancels and restarts** the in-flight preview at the new stamp (D34's
  policy). Previews are ephemeral, cheap, never pinned. **This is the source of the
  head-vs-derived lag (D13/D29):** the arbiter is mid-derive/prepare/render on an *older*
  preview stamp while head has moved on.
...
- A commit/preview builds a new `DerivedScene` on the builder, seals it (sharing all
  ...
  samplers/photon maps by reference where the light topology and `EffectiveRenderConfig`
  are unchanged), and **atomically swaps the renderer's pointer at a PASS boundary** (never
...
- **Render-time integrator override re-runs only `prepare`** (D22): the same sealed
  ...
  the new (re-resolved) `EffectiveRenderConfig` and swapped in.
...
**Coherent version status — requested vs published, `ok` ⟺ full-stamp equality (D13,
refined by D29, completed by D38).** Because derive/prepare/render are **async,
cancellable arbiter phases (D34)** and may serve a last-good snapshot, the head version
*and any other input axis* (time, assets, effective config, view camera, seed) can differ
from what the arbiter has actually *published*; a single shared `documentId` would falsely
claim them equal. A surface that published only what the arbiter *produced* also couldn't
show what it is *trying* to produce when time/assets/camera/config change without changing
`headVersion` (D38). The session therefore publishes **one coherent status value** that
carries **both the requested and the published stamps**:
`{ headVersion, requestedDerivedStamp, requestedPreparedStamp, publishedDerivedStamp,
publishedPreparedStamp, status, diagnostics }`. **`status:ok` requires FULL-stamp equality
(D38):** `published == requested` on *every* axis — `cstVersion`, `assetDigest`,
`animationName`, `shutterInterval`, `effectiveRenderConfigHash`, `viewCameraStateHash`,
`samplingSeed`; otherwise `status ∈ {deriving, preparing, rendering}` (the phase in
flight). **The requested stamps are set when *any* input axis changes — not just the CST
head** (D38) — so an asset edit, a camera move, a config change, or a new frame all move
the requested stamp and drop `ok`. **`read_graph` / `render` / `derive_preview` are stamped
with the *published* `DerivedStamp` / `PreparedStamp`** (D29) — *what the scene actually
reflects* — whose `cstVersion` axis **may lag head** (or be last-good on error) and is
**never** claimed equal to head. The "is this rendered scene stale vs head?" check is on the
**`cstVersion` axis alone, by version-DAG ancestry** (rendered `cstVersion` is an
ancestor-or-equal of head's), **never numeric `<`** (the DAG branches; D29) — the other
stamp axes (`assetDigest`, `shutterInterval`, `effectiveRenderConfigHash`,
`viewCameraStateHash`, `samplingSeed`, …) are equality-matched, not ordered (D41/D42).
(`read_document` is stamped with `headVersion`, the CST truth — Facet 1/5; a patch's
optimistic-concurrency precondition is checked against `headVersion`.) `status` +
`diagnostics` explain any lag or failure so a consumer reading a lagging stamp knows why.
...
truth — they can be dropped and rebuilt from `(CST, AssetManifest, t)` +
`EffectiveRenderConfig` at any time; the two sealed layers are immutable values, not a
mutable mirror of the CST. (The *render-local* caches of §2.10 — irradiance cache, accumulation — are a
...
prefilter change (or whenever determinism is required) compute a *content digest*, which
...
**The digest is of the exact bytes the LOADER consumed (D41), not a separately-hashed
path.** A naive "stat the path, hash the path, then hand the path to the loader" scheme
has a **TOCTOU window**: loaders reopen the path later (e.g.
`TriangleMeshLoaderPLY.cpp:795`), and the file can change between the manifest's hash and
the loader's reopen — stamping the artifact with the *wrong* identity. D41 closes this two
ways (either is acceptable): **(a) load-and-hash one buffer** — read the file once, hash
*that* buffer, and hand the same buffer to the loader (one read, no reopen); or **(b)
revalidate after load** — re-hash the bytes the loader actually consumed against the
manifest entry and, on mismatch, **retry / refuse** (never silently accept a different
file). **The `DerivedStamp` asset axis is therefore a *content digest* (`assetDigest` —
the per-asset content hashes), a reproducible identity, NOT a session generation counter
(D29→D41).** A session-local generation counter is not a reproducible identity, so it
**remains only as a fast in-process change signal** (a cheap "something may have changed,
re-stat" trigger), never the stamp's recorded identity.

- **Asset reads are traced (D4); memo keys use the content digest (D17/D41).** When
  `derive`'s **async realize phase** (the only phase that loads bytes — D39) loads a file
  for a param, it records an edge from the consuming L1/L3 node to that manifest entry
  (the `assetDependents` index, §2.3), and the entry's **content digest of the loaded
  bytes** participates in the consumer's memo key (§2.3). A digest change therefore
...
| Phase-B reconciliation — prepare half (`RebuildLightSamplers`, `BuildPendingPhotonMaps`) | **Evolve (named prerequisite — D32/D33)** | Runs in `prepare` (→ `PreparedRenderState`, which **owns** the samplers/photon maps, moved in from the RayCaster — D22), but **cannot be reused unchanged**: `BuildPendingPhotonMaps` *mutates* the `Scene`'s pending flags/maps/gather params (`Scene.cpp:750`) and light samplers are `RayCaster`-owned — you can't build either by mutating a **sealed immutable** `DerivedScene`. Refactor both to `build(const DerivedScene&, PreparedRenderStateBuilder&)` reading through **non-mutating const input APIs** (D32), and make stochastic prep draw from the `EffectiveRenderConfig` **seed** (D42), not `rand()` (`RandomNumbers.h:32`), so `prepare` is deterministic/cacheable (D33) — note this makes only the **photon maps** reproducible; the final render is reproducible within MC tolerance, not bit-identical (D40). |
...
- If a commit is *catastrophic* (e.g. the active-rasterizer node — an
  `EffectiveRenderConfig` input to `prepare`, D22/D42 — is holed so no valid
  `PreparedRenderState` can be built),
...
4. **Light-sampler rebuild granularity (a `prepare`-layer concern — D22).** L5 is a
   ...
   scope for v1 (matches today), flag for a later perf pass. (An `EffectiveRenderConfig`
   light-sampling-strategy change also re-runs this, since the samplers are
   config-dependent — D22/D42.) **Prerequisite (D32/D33):** the rebuild must read the sealed
   ...
   stochastic prep that feeds it must draw from the `EffectiveRenderConfig` seed, not
   `rand()` — §2.1/§2.8.
...
7. **O2 working-assumption (debounced-commit).** §2.8 designs for debounced-commit.
   ...
   with structural sharing, derivable from `(CST, AssetManifest, t)`
   (+ `EffectiveRenderConfig` for the prepared layer) and droppable at will, not a mutable
   mirror of the CST that must be kept in sync. So the remaining deltas if synthesis chose
   60Hz incremental
...
8. **Render determinism is `prepare`-deep only; bit-identical rendering is a named future
   (D40).** The seed (D33) makes **`prepare` / the photon maps** bit-reproducible — that
   is exactly what the `PreparedArtifact` cache soundness needs (same `PreparedStamp` →
   same photon maps). The **final render is NOT bit-identical**: renderers use per-worker
   independently-seeded RNGs + `GlobalRNG()` (`RasterizeDispatchers.h:146`,
   `PixelBasedRasterizerHelper.cpp:897`), and tile assignment, splat reduction, and
   denoise also vary, so the same scene/config/seed yields the same converged image only
   **within Monte-Carlo tolerance** (review-by-image, not byte-diff — the git-native
   thesis weakens accordingly). **Named future option (not v1 — D40):** **bit-identical
   rendering** — deterministic per-pixel/per-sample RNG streams keyed by pixel+sample,
   deterministic splat reduction, and deterministic denoise, *across every renderer*. This
   is a large cross-renderer change and is explicitly out of v1 scope; it is the analogue
   of the TLAS-refit (D24) and motion-blur (D31) named gates. **Open:** is a single
   reference renderer's bit-identical mode (rather than all renderers) a useful
   intermediate milestone for regression diffing? Flag for a later pass.

9. **Cancellation policy is two-class (D43) — not a single "newer head cancels" rule.**
   Preview jobs are **latest-wins** (a newer head cancels + restarts — D34) while pinned
   render jobs are **stamp-pinned** (a newer head does not cancel; queued / run to
   completion). **Open:** the coordinator's queue policy for *multiple* pinned renders
   (FIFO vs priority vs coalescing identical stamps) and whether a pinned render may share
   a `DerivedArtifact`/`PreparedArtifact` already computed by a preview at the same stamp
   (it should — the artifacts are stamp-keyed; §2.3). Also: the bounded **synchronous
   semantic phase (D39)** runs on the edit thread for `propose_patch`/rename — its latency
   bound (lex→parse→bind→resolve→typecheck on a large CST) must stay well under an
   interactive budget; if a pathological scene blows it, the fallback is to run even the
   semantic phase async and report "validating" — flagged, not expected.
...
- **From Facet 1 (CST):** I assume (a) **immutable, process-stable per-occurrence
  ...
  D17/D41) — resolved identity + (size,mtime)-prefilter→**content-digest-of-the-loaded-
  bytes** fingerprint per referenced asset (D41: load-and-hash one buffer, or
  revalidate-post-load + retry — no path-hash TOCTOU) — is supplied as a derivation input
  (its **content digest is the `assetDigest` stamp axis — D29→D41**; the generation
  counter survives only as a fast in-process change signal), and **time** — a **shutter
  interval `[t0,t1]`**
...
- **To/with Facet 3 (edit model):** Facet 3 deletes `SceneEdit`/`EditHistory`/
  ...
  **Assumption:** Facet 3's "apply a CST edit" commits a cheap CST `Version` (D34); for
  `propose_patch` precommit and rename, Facet 3 may run my **bounded synchronous semantic
  phase** (lex→parse→bind→reference-resolution→typecheck — D39) on the edit thread (it is
  the *same* resolver the async job fronts with — D35/D39, no second path). The **render
  arbiter** then runs the **async expensive phase** of my
  `derive(CST, CST', AssetManifest, t, animationName, cache)` (D5/D21/D31), and a
  render/integrator change calls `prepare(DerivedScene, EffectiveRenderConfig)` (D22/D42,
  with the seed in `EffectiveRenderConfig` — D33) — i.e. Facet 3 owns *producing* the new
  CST + version history; I own *turning the diff into a new sealed immutable
  `DerivedScene`, then a `PreparedRenderState`*, **asynchronously on the arbiter as
  cancellable phases** (D34), with **cancellation by job class — previews latest-wins,
  pinned renders stamp-pinned (D43)**. The **derive→seal→prepare→seal→render /
  pass-boundary atomic-swap** seam (§2.8, D12/D22/D34) is shared mechanism; assume Facet
  3/4 drive the commit *and gesture previews of the uncommitted head* (D1, §5.3) and
  supply the **request** that `ResolveEffectiveRenderConfig` merges into the
  `EffectiveRenderConfig` (D42) (incl. the render-time integrator override, which re-runs
  only `prepare` — D22) plus the **ephemeral view-camera state** (hashed into the
  `PreparedStamp` — D42). Identity flows through edits: staged-edit state (`GestureBuffer`,
  working head) carries
...
- **To Facet 5 (agentic surface):** my structured per-node diagnostics (§5.1) are
  the "structured errors" the MCP `validate→derive→render` loop returns — where
  **`validate` is exactly the bounded synchronous semantic phase (D39)** (the agent gets
  binding/type/reference errors synchronously, before any expensive async derive/render).
  I assume the agent edits land as CST diffs through Facet 3, then trigger my derive; an
  explicit agent render is a **stamp-pinned** job (D43), not cancelled by a later head.
  The status the agent reads carries **requested vs published** stamps with `ok` ⟺
...
- **Decision-conformance check (D1–D37):** the design honors **D1** (immutable COW
  ...
  **refined to stamps + DAG-ancestry by D29; completed to requested-vs-published +
  full-stamp-`ok` by D38** — §2.8), **D14** (rename uses
  ...
  temp+fsync+revalidate+rename save; **refined by D41** to a content digest of the
  loaded buffer — §2.1/§2.9), **D18** (corrected first-slice fixture
  ...
  **TLAS**, config-INDEPENDENT; **`PreparedRenderState = prepare(DerivedScene,
  EffectiveRenderConfig)`** owns **light samplers + photon maps**, config-DEPENDENT; the
  config is an explicit third input — **resolved + hashed by `ResolveEffectiveRenderConfig`
  per D42**; a render-time integrator override re-runs only `prepare` —
  ...
  **D29** (full **`DerivedStamp = {cstVersion, assetManifestGen→assetDigest (D41),
  animationName, shutterInterval}`** / **`PreparedStamp = DerivedStamp +
  {renderConfig→effectiveRenderConfigHash (D42), cameraOverride→viewCameraStateHash (D42),
  ...
  **D33** (`prepare` **deterministic**: `EffectiveRenderConfig` carries a **sampling seed /
  RNG-stream id** used by stochastic prep instead of `rand()`; seed in `PreparedStamp` →
  **`prepare`/photon maps** pure/cacheable/reproducible; **weakened by D40** — the *render*
  is reproducible only within MC tolerance, bit-identical is a named future — §2.1/§2.4/§2.8),
  **D34** (derive→seal→prepare→seal→render
  ...
  the head-vs-derived lag; **refined by D39** (a bounded sync semantic phase fronts the
  async job) **and D43** (cancel only previews, not pinned renders) — §2.4/§2.8/§6.8/§6.9);
  and it references **D35** (rename reuses the
  ...
  **D36** (identity propagates through edits: `GestureBuffer`/working head carry
  ...
  pass, not a second derive client; the bounded sync semantic phase is the *front* of that
  one function, not a second resolver — D39), L3 (consumes post-expansion CST), L5
  (identity is first-class — realized as **NodeId** (lineage) + **name-path** (addressing)
  per D9/**D44**, homed in the D26 `identityRoot`), L6 (descriptor-as-schema;
  `referenceCategories` is a UI ref-picker hint, not a parallel edge schema). **No
  conflicts with D1–D44 or the locked decisions** — and **D44 brings the locked charter
  L5/INV-5 into line with this facet's long-standing NodeId-is-identity assumption**, so
  what was a latent charter-vs-facet tension is now resolved in favor of the decisions.
  Open decisions touched: O1 (designed for CST-canonical; the memo key's derivation-key +
  ...
  O2 (designed for debounced-commit; §6.7 argues to keep it, now as a latency
  preference since D1 is safe at either cadence).
...
the **asset-backed node** that makes **G5** (AssetManifest invalidation by the
**content digest of the loaded bytes** — D17/D41) and the external-file-conflict path
(D6/D17) testable. Facet 2's contributions to that fixture:
...
2. **Build the minimal memo cache + traced graph for these node kinds:** L0 values,
   ...
   **recorded by tracing the derivation** (D4 — *not* read from
   ...
   stamps are trivial: one CST version, one **asset digest** (D41), a single-time interval,
   one **`effectiveRenderConfigHash`** + **`viewCameraStateHash`** + seed (D42) — but the
   keying is the real round-4/5 shape.)
...
This slice stands up the cache, the **traced** graph build (the name-ref edges from the
**sync semantic phase** — D39), the diff→dirty→propagate loop, the
derive→seal→prepare→seal→render path **run as a bounded sync semantic phase + cancellable
async arbiter phases** (D12/D22/D34/D39; the slice's one explicit render is a pinned job —
D43), and both backends — the skeleton every other chunk type plugs into by
...
lights-as-emitters/**TLAS**); the second is `prepare(DerivedScene, EffectiveRenderConfig)`
(D22), which reads the sealed `DerivedScene` through **non-mutating const APIs** into a
**`PreparedRenderStateBuilder`** (D32) and draws stochastic prep from the
`EffectiveRenderConfig`'s **seed** (D33), producing a config-**dependent**
**`PreparedRenderState`** (light samplers + photon maps) — so a render-time integrator
override re-runs only `prepare`. (`EffectiveRenderConfig` is the deterministic merge of
scene-authored ← request override ← defaults ← auto-resolution, hashed into the stamp —
D42.)
...
pipeline is a **bounded synchronous semantic phase** (lex→parse→bind→reference-resolution→
typecheck — what `propose_patch`/rename/`validate` run on the edit thread) fronting the
**asynchronous expensive phases of the render arbiter, off the edit thread** (the edit
thread only commits a cheap CST `Version`; D34/D39); the async phases are cancellable
**by job class — a newer head cancels a preview but not a stamp-pinned render** (D43); the
renderer **atomically swaps to the sealed `PreparedRenderState` at a PASS boundary** — no
parking-for-safety; the old layers drain by refcount (D1, D12, D22). Status carries
**requested vs published** stamps with `ok` ⟺ full-stamp equality (D38). Caches *populated
...
`prepare`** that makes the **photon maps** reproducible (D33; the *render* off them is
reproducible only within **MC tolerance**, not bit-identical — D40, a named future; cache
= full-stamp equality, staleness = `cstVersion` DAG ancestry not `<` — D29; the asset axis
is a **content digest of the loaded bytes** — D41); incrementality comes from the
**artifact-scoped, stamp-keyed**
