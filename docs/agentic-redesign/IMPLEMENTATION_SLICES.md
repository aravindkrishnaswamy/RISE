...
8. **Measure a non-spatial edit AND a spatial edit; report TLAS time separately.**
   **Slices 0-5 substantively LANDED (21-stable-apply-and-resolver.md); the item-8 result is achieved.**
   ...
   resolution cannot drift *even in principle*).  Both slice-3 follow-ups (optional-slot removal AND CSG-operand re-point) LANDED in workstream #3 -- CSG re-point needed no object-rollback (the slot-precise preflight + AddCSGObject's resolve-first make a CSG re-point unable to fail, so objects-last + entity-only rollback stays sufficient).
