...
- **0d grammar (D-1 = fold-all)**: ratified. The migrator already constant-folds `$()`/DEFINE/FOR/`hal()` to
  ...
  scenes/colors.RISEscript` (an include-era file removed after the cutover)
  would fail the legacy v6 reader (a stray header = unknown chunk). The
  `RISE ASCII SCENE 7` header is
