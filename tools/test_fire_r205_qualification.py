    ...
    def test_compilation_source_closure_must_be_committed(self):
        with tempfile.TemporaryDirectory(prefix="rise-r205-source-closure-red-") as temporary:
            ...
            try:
                ...
                (root / "build/make/rise/RendererBuildIdentity.generated.h").write_text("// regenerated by pinned recipe\n")
                qualification.clean_source("HEAD")
                for mutation in ("vendor", "wildcard", "ignored_wildcard", "build_include"):
                    extra = (root / "build/make/rise/zlib.h" if mutation == "build_include" else
                             root / "tests/fire_production_trace" / (
