...
\t\tbool AcceptedManifoldTokenMatchesCurrentPayload() const;"""
    ...
    if name == "FireProductionTransport" and suffix == ".cpp":
        ...
        traced_compatible_stage = compatible_stage + (
            "\n\t\t\t\t\tFireProductionRoundoffTrace::SealDualStageAndReset(\n"
            "\t\t\t\t\t\tcomputed.auxiliaryFaceDensity[component],"
            "computed.momentum[component],component);")
        if text.count(compatible_stage) != 1:
            raise RuntimeError("compatible dual stage seam changed")
        text = text.replace(compatible_stage, traced_compatible_stage)
