...
namespace
{
		...
		)
	{
		...
		for( std::size_t i = 1; i < eyeVerts.size(); i++ )
		{
			...
			// delta lights — NEE is the only strategy that can sample
			// the path through a delta vertex.
			const Scalar wLight = ls.isDelta
				? Scalar( 0 )
				: bsdfDirPdfW / ( lightPickProb * directPdfW );
			Scalar wCamera = 0;
			if( !ls.isDelta && directPdfW > 0 && cosAtLight > 0 ) {
				// Same reasoning as wLight: for delta lights, the
				// path can't be re-generated by alternative light-
				// tracing or VM strategies that "land" on the delta
				// vertex.  wCamera = 0 keeps NEE at full weight.
				//
				// camFactor uses the SmallVCM-GEOMETRIC emissionPdfW
				// (no light-selection multiplier).  Δ6 bisect confirmed
				// this divide is a no-op in test scenes (lightPickProb
				// = ls.pdfSelect cancels symmetrically against dVC's
				// pdfSelect factor); kept for SmallVCM convention
				// consistency.
				const Scalar emissionPdfW_geom = ( lightPickProb > 0 ) ?
					( emissionPdfW / lightPickProb ) : Scalar( 0 );
				const Scalar camFactor =
					( emissionPdfW_geom * cosAtEye ) / ( directPdfW * cosAtLight );
				wCamera = camFactor * (
