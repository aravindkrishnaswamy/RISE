...
namespace RISE
{
		...
		namespace ChunkParsers
		{
			...
			struct OverrideObjectAsciiChunkParser : public IAsciiChunkParser
			{
				bool Finalize( const ParseStateBag& bag, IJob& pJob ) const override
				{
					...
					// arc-86 slice 1 fix F2: a GROUP MEMBER's transform stack already carries the
					// group's composed matrix G (GroupAsciiChunkParser pushes it, so the member's
					// final transform is `G * M`).  The two ABSOLUTE arms below route through
					// SetFinalTransformMatrix / ClearAllTransforms+PushTop, both of which CLEAR
					// m_transformstack -- silently discarding G and un-grouping the member with no
					// diagnostic at all.  The per-field arm only replaces component matrices, so the
					// stack (and G with it) survives and the override composes GROUP-LOCALLY.  The
					// two arms therefore had OPPOSITE semantics on a grouped member; refuse the
					// destructive pair rather than ship the silent difference.  (The lookup only
					// resolves when the group Finalized EARLIER in document order, which is exactly
					// when the clobber would happen: an override that PRECEDES the group is fine --
					// the group's later push composes on top of it, as intended.)  Scene-variant
					// bakes emit override_object, so this is the diagnostic a variant on a grouped
					// member needs; author the variant with position/orientation/scale instead.
					if( hasMatrix || hasQuaternion ) {
						char groupNames[512] = {0};
						if( pJob.GetGroupMembership( name.c_str(), nullptr, groupNames, (unsigned int)sizeof( groupNames ) ) ) {
							const std::string why = std::string( "override_object `" ) + name +
								"`: this object is a member of group `" + groupNames + "`, and the "
								"absolute `" + ( hasMatrix ? "matrix" : "quaternion" ) + "` arm "
								"REPLACES the object's whole transform stack -- which would silently "
								"DISCARD the group's transform and un-group the object. Use the "
								"per-field arm instead (`position` / `orientation` / `scale`): it "
								"replaces only the named component, so the group's transform still "
								"composes outside it (the override is interpreted GROUP-LOCALLY). "
								"To place the object in world space independently, remove it from "
								"the `group` chunk's `member` list first.";
							if( RISE::g_cstFinalizeDiagSink ) *RISE::g_cstFinalizeDiagSink = why;
							GlobalLog()->PrintEx( eLog_Error, "%s", why.c_str() );
							return false;
						}
					}
				...
				const ChunkDescriptor& Describe() const override
				{
					static const ChunkDescriptor d = []{
						...
						cd.description = "Apply transform overrides to an already-declared "
							...
							"chunk that created the target.  GROUP MEMBERS: if the target is "
							"a `member` of a `group` declared EARLIER in the document, only "
							"the per-field arm (`position` / `orientation` / `scale`) is "
							"accepted, and it is interpreted GROUP-LOCALLY — it replaces the "
							"named component of the object's OWN transform, and the group's "
							"transform still composes outside the result.  The absolute "
							"`matrix` and `quaternion` arms are REFUSED on a grouped member: "
							"they replace the object's entire transform stack, which would "
							"silently discard the group's transform.";
			...
			// docs/agentic-redesign/86-object-grouping.md Slice 1: `group` names member
			// objects and composes ONE transform into EACH of them at derive time.
			// Creates NO manager entity -- the side-index decision (§3 of the design):
			// the CST document itself is the group's index, so there is no ILight /
			// IObject / etc. to look up by the group's name, and no dynamic_cast is
			// needed to reach a member -- IObjectPriv already inherits ITransformable.
			//
			// DECLARE-MEMBERS-BEFORE-GROUP: PASS-2 (DeriveToJob) applies chunks in
			// document order and resolves references by immediate manager lookup --
			// same rule standard_shader's `shaderop` and file_rasterizeroutput already
			// live under.  A member not yet declared is an apply-time Finalize failure
			// (PASS-1 validates shape only; it cannot resolve references).
			//
			// COMPOSITION ORDER: PushBottomTransStack pushes to the BACK of the
			// transform-stack deque; FinalizeTransformations folds front-to-back as
			// `(*i) * m_mxFinalTrans`, so pushing the group matrix onto a member whose
			// stack is otherwise empty (the normal case -- Job::AddObject clears it on
			// every apply) yields exactly `groupMatrix * (P*O*St*S)`: the group OUTSIDE
			// the member's own transform.
			//
			// The push SIDE is invisible in that single-group-on-an-empty-stack case --
			// a one-element deque folds identically under push_front and push_back, so
			// PushTop would give the same answer there.  It only matters once the stack
			// holds TWO OR MORE entries: a second group over the same member, or a member
			// authored with `matrix` (SetFinalTransformMatrix leaves one stack entry the
			// group's push then joins).  In both, PushBottom makes the LATER push the
			// OUTER transform; PushTop would invert that.  GroupChunkTest's [two-groups]
			// and [matrix-member] cases pin exactly this with deliberately NON-COMMUTING
			// matrices (red-proven 2026-08-15: flipping this call to PushTopTransStack
			// fails both) -- an all-translation version of either case would not, because
			// translations commute.
			//
			// TWO GROUPS OVER ONE MEMBER is well-defined and ALLOWED (not detected as a
			// conflict): the transforms simply compose in document order, one push per
			// group, exactly like any other transform-stack consumer -- the later group
			// lands OUTSIDE the earlier one, so the effective total is
			// `G_last * ... * G_first`, which is exactly what the Job-level membership
			// side index (NoteGroupMembership, below) accumulates.
			//
			// INCREMENTAL DERIVE: see DeriveToJobIncremental's explicit
			// `case ChunkCategory::Group` in Cst.cpp -- re-applying a closure that
			// contains a `group` chunk (whether the group itself was edited, or a
			// member was edited and the group was pulled in as its dependent) always
			// falls back to a full derive, because FinalizeTransformations never clears
			// m_transformstack (only a member's OWN re-apply does, via
			// Job::AddObject's ClearAllTransforms) -- a partial re-apply would either
			// double-compose the edited member's group push, or -- if only the member
			// and its OWN edited group are in the closure but a SIBLING member is not
			// (the group's Finalize touches every listed member, not just the one that
			// triggered the closure) -- double-compose onto that untouched sibling.
			struct GroupAsciiChunkParser : public IAsciiChunkParser
			{
				static bool Reject( const std::string& why )
				{
					if( RISE::g_cstFinalizeDiagSink ) *RISE::g_cstFinalizeDiagSink = why;
					GlobalLog()->PrintEx( eLog_Error, "group:: %s", why.c_str() );
					return false;
				}

				// arc-86 slice 1 hardening (H1, 2026-08-15): reject a non-invertible group transform HERE,
				// at parse time, instead of leaving it to be caught later (and only on the GUI edit path)
				// by SceneEditor.cpp's commit-time `G * G^-1 == I` check.  Same verification, not a
				// determinant epsilon: Matrix4Ops::Inverse returns its INPUT UNCHANGED when det == 0
				// (MatricesOps.h), so a bare determinant test and this test are not equivalent -- verifying
				// the product against identity catches that returned-input case by construction, plus any
				// other numerical failure, with one check.  `!(<=)` also rejects NaN (SceneEditor.cpp's
				// GroupLocalTransformMatrix_ documents the same trick).
				//
				// `scale` is the only one of the group's three fields that CAN make this composition
				// (Translation * Rotation * Stretch) singular -- Translation and a pure Euler Rotation are
				// always invertible (det(R) == 1), so det(G's linear part) == sx*sy*sz.  That makes the
				// "which component" diagnostic cheap and exact: whichever scale axis is nearest zero.
				static bool GroupMatrixInvertible_( const Matrix4& G, const double scale[3], std::string& outWhyNot )
				{
					...
					for( int k = 0; k < 16; ++k ) {
						...
						if( !( std::fabs( d ) <= 1e-9 ) ) { ok = false; break; }   // NaN also fails `<=`
					}
					if( ok ) return true;
					// Cheap culprit diagnosis: the smallest-magnitude scale component, if any is near zero.
					int worst = 0;
					for( int k = 1; k < 3; ++k ) if( std::fabs( scale[k] ) < std::fabs( scale[worst] ) ) worst = k;
					const char* axis = worst == 0 ? "x" : ( worst == 1 ? "y" : "z" );
					if( std::fabs( scale[worst] ) < 1e-6 ) {
						char buf[160];
						std::snprintf( buf, sizeof( buf ), "its `scale` %s-component (%.9g) is zero or "
							"near-zero, which collapses the group's linear transform to a non-invertible "
							"plane/line/point", axis, scale[worst] );
						outWhyNot = buf;
					} else {
						outWhyNot = "the composed transform is not invertible for a reason other than a "
							"degenerate `scale` axis (none of x/y/z is near zero) -- check for extreme "
							"scale magnitudes that overflow/underflow in the inverse";
					}
					return false;
				}
				...
				{
					const std::string name = bag.GetString( "name", "noname" );
					const std::vector<std::string>& members = bag.GetRepeatable( "member" );

					if( members.empty() ) {
						return Reject( "group `" + name + "`: at least one `member` is required" );
					}

					// Document-level validation FIRST (no resolution needed): a group naming
					// itself, or listing the same member twice, is a scene-authoring error
					// regardless of whether either name happens to resolve to something.
					std::set<std::string> seen;
					for( const std::string& m : members ) {
						if( m == name ) {
							return Reject( "group `" + name + "`: member `" + m +
								"` names the group itself -- a group has no manager entity, "
								"so this can never resolve to the group; it is almost certainly "
								"a typo for a member object's name" );
						}
					...
					IJobPriv* priv = dynamic_cast<IJobPriv*>( &pJob );
					if( !priv ) {
						return Reject( "group: IJob does not expose IObjectManager (mock?)" );
					}
					IObjectManager* objs = priv->GetObjects();
					if( !objs ) {
						return Reject( "group `" + name + "`: object manager unavailable" );
					}

					// Resolve every member FIRST, before mutating any of them (review
					// P1.7 atomicity, same discipline as Job::AddObject / override_object):
					// a missing member fails the WHOLE group without having composed the
					// group transform onto any of its siblings.
					std::vector<IObjectPriv*> resolved;
					resolved.reserve( members.size() );
					for( const std::string& m : members ) {
						IObjectPriv* obj = objs->GetItem( m.c_str() );
						if( !obj ) {
							// NESTED GROUPS are out of scope for V1 (design doc §3 "Nesting depth":
							// single level, validated).  A group creates no manager entity, so a
							// member naming an already-declared GROUP looks exactly like a member
							// naming nothing -- and the generic message below would misattribute it
							// to the declare-members-before-the-group ordering rule, sending the
							// author off to reorder chunks that are already in the right order.
							// IsGroupDeclared is the side index that tells the two cases apart.
							if( pJob.IsGroupDeclared( m.c_str() ) ) {
								return Reject( "group `" + name + "`: member `" + m + "` names another "
									"`group`, and a group cannot contain a group in V1 -- `member` must "
									"name an OBJECT (standard_object / csg_object). Place the inner "
									"group's member objects in this group directly, or apply this "
									"group's transform to the inner group's own position/orientation/"
									"scale instead." );
							}
							return Reject( "group `" + name + "`: member `" + m + "` was not found. "
								"Chunk order matters -- every `member` must be declared with a "
								"standard_object/csg_object chunk BEFORE this `group` chunk, because "
								"DeriveToJob applies chunks in document order and resolves references "
								"by immediate manager lookup (a forward reference cannot resolve)." );
						}
						resolved.push_back( obj );
					}

					// Build the group matrix exactly as Job::AddObject composes a member's
					// OWN transform (position * orientation-from-Euler-degrees * stretch) --
					// see the struct comment above for why PushBottomTransStack then
					// FinalizeTransformations yields groupMatrix * memberMatrix.
					double pos[3]    = {0,0,0};
					double orient[3] = {0,0,0};
					double scale[3]  = {1.0,1.0,1.0};
					bag.GetVec3( "position", pos );
					...
					const Matrix4 groupMatrix =
						Matrix4Ops::Translation( Vector3( pos[0], pos[1], pos[2] ) ) *
						( Matrix4Ops::XRotation( orient[0] ) *
						  Matrix4Ops::YRotation( orient[1] ) *
						  Matrix4Ops::ZRotation( orient[2] ) ) *
						Matrix4Ops::Stretch( Vector3( scale[0], scale[1], scale[2] ) );

					// H1 (2026-08-15): refuse a non-invertible group transform HERE, before any member is
					// mutated -- same atomicity discipline as the member-resolution loop above.  Without
					// this, a degenerate group derives fine and the singularity is only caught later, at
					// GUI commit time, by SceneEditor.cpp's `G * G^-1 == I` check -- a path with no test
					// coverage and a much worse authoring experience (the error surfaces at drag time, on
					// an object far from the group chunk, instead of at the group chunk itself).
					{
						std::string whyNot;
						if( !GroupMatrixInvertible_( groupMatrix, scale, whyNot ) ) {
							return Reject( "group `" + name + "`: the composed transform (position * "
								"orientation * scale) is not invertible -- " + whyNot + ". A group's "
								"transform must be invertible because members' committed transforms are "
								"recovered by dividing it back out; fix the group's `scale`." );
						}
					}

					// The membership SIDE INDEX (design §3, made concrete by the 2026-08-15 fix
					// round).  A member's GetFinalTransformMatrix() from here on is `G * M`, and
					// the object carries NO back-pointer to its group, so every consumer that
					// ROUND-TRIPS or OVERWRITES a member's transform needs this index to divide
					// G back out (SceneEditor::CommitPendingCstObjectTransforms) or to refuse
					// (override_object's absolute-matrix arms).  Encoded column-major in the
					// Matrix4 `_00.._33` field order, matching AddObjectMatrix.
					const double groupMatrix16[16] = {
						groupMatrix._00, groupMatrix._01, groupMatrix._02, groupMatrix._03,
						groupMatrix._10, groupMatrix._11, groupMatrix._12, groupMatrix._13,
						groupMatrix._20, groupMatrix._21, groupMatrix._22, groupMatrix._23,
						groupMatrix._30, groupMatrix._31, groupMatrix._32, groupMatrix._33 };

					for( size_t i = 0; i < resolved.size(); ++i ) {
						resolved[i]->PushBottomTransStack( groupMatrix );
						resolved[i]->FinalizeTransformations();
						pJob.NoteGroupMembership( members[i].c_str(), name.c_str(), groupMatrix16 );
					}
					objs->InvalidateSpatialStructure();
					return true;
				}
				...
				const ChunkDescriptor& Describe() const override {
					static const ChunkDescriptor d = []{
						ChunkDescriptor cd;
						cd.keyword = "group"; cd.category = ChunkCategory::Group;
						cd.description = "Names member objects and composes ONE transform "
							"(position * orientation * scale, matching standard_object's own "
							"composition) into EACH of them at derive time, before the TLAS is "
							"built. Creates NO manager entity -- the scene document itself is "
							"the group's index; there is no way to reference a group by name "
							"from another chunk. Every `member` MUST already be declared (via a "
							"standard_object/csg_object chunk) BEFORE this `group` chunk -- "
							"chunks are resolved in document order. An object may be a member of "
							"more than one group; their transforms compose in document order, "
							"same as any other transform-stack consumer. Editing a `group` chunk, "
							"or editing anything whose incremental edit-closure includes one, "
							"always falls back to a full scene derive. "
							"A MEMBER NAMING THE GROUP ITSELF IS A HARD REJECT: a `member` entry equal "
							"to this group's own `name` is refused -- a group has no manager entity, so "
							"that can only ever mean an object that happens to share the group's name, "
							"ambiguous enough that the derive refuses rather than guess. This is the "
							"ONLY name-collision check `group` performs: a group's `name` matching an "
							"OBJECT's name elsewhere in the scene (e.g. `standard_object cart` alongside "
							"`group cart { member wheel }`) is NOT checked and IS accepted -- harmless "
							"today because a group creates no manager entity, so there is nothing for "
							"the shared name to collide with. (Tooling that mints a group and a "
							"representative object together should still give them distinct names for "
							"clarity, but the derive will not stop it.) "
							"A group cannot contain a group in V1 -- `member` must name an object. "
							"An `override_object` that FOLLOWS a group and targets one of its "
							"members may use only the per-field arm (position / orientation / "
							"scale), which composes GROUP-LOCALLY (the group's transform still "
							"lands outside it); its absolute `matrix` / `quaternion` arms are "
							"REFUSED, because they replace the member's whole transform stack and "
							"would silently discard the group's transform.";
						auto P = [&cd]() -> ParameterDescriptor& { cd.parameters.emplace_back(); return cd.parameters.back(); };
						{ auto& p = P(); p.name = "name";        p.kind = ValueKind::String;    p.description = "Unique name"; p.defaultValueHint = "noname"; }
						{ auto& p = P(); p.name = "member";      p.kind = ValueKind::Reference; p.referenceCategories = {ChunkCategory::Object}; p.repeatable = true; p.required = true; p.description = "Member object this group places (repeatable; at least one required; must be declared before this chunk)"; }
						{ auto& p = P(); p.name = "position";    p.kind = ValueKind::DoubleVec3;p.description = "World-space position composed into every member"; p.defaultValueHint = "0 0 0"; }
						{ auto& p = P(); p.name = "orientation"; p.kind = ValueKind::DoubleVec3;p.description = "Euler orientation (degrees) composed into every member"; p.defaultValueHint = "0 0 0"; }
						{ auto& p = P(); p.name = "scale";       p.kind = ValueKind::DoubleVec3;p.description = "Per-axis scale composed into every member"; p.defaultValueHint = "1 1 1"; }
						return cd;
					}();
					return d;
				}
	...
	std::vector<ChunkParserEntry> CreateAllChunkParsers()
	{
		...
		add( "group",                                 new GroupAsciiChunkParser() );
