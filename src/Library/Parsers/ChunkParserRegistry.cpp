			...
			struct ExpressionPainterAsciiChunkParser : public IAsciiChunkParser
			{
				...
				const ChunkDescriptor& Describe() const override {
					static const ChunkDescriptor d = []{
						...
						{ auto& p = P(); p.name = "def";   p.kind = ValueKind::String; p.repeatable = true; p.description = "Named sub-expression `<name> <expr>` (repeatable, in order); a let-binding referencing u, v, P, Po, N, fw, time, curv, curvR, the occlusion()/thickness() builtins, params, and earlier defs"; }
