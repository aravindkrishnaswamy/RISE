	...
	const IORStack& ) const
{
	...
	else
	{
		...
		wi = GeometricUtilities::CreateDiffuseVector( onb, ptrand );
		lobeType = ScatteredRay::eRayDiffuse;
	}

	const Scalar cosWi = Vector3Ops::Dot( wi, n );
	if( cosWi <= 0 || Vector3Ops::Dot( wi, geomN ) <= 0 ) {
	...
	s.pdf     = q;
	if( nm < 0 ) {
		s.kray = pBRDF->ValueWithParams( wi, ri, p ) * ( cosWi / q );
	} else {
		s.krayNM = pBRDF->ValueNMWithParams( wi, ri, nm, p ) * ( cosWi / q );
	}
