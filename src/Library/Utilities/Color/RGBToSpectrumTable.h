...
//    ROMM RGB triple.
//
//    The ROMM LUT data is BAKED INTO THE BINARY via
//    RGBToSpectrumTable_ROMMData.cpp (auto-generated from
//    extlib/jakob-hanika-luts/romm.coeff by
//    tools/GenerateROMMSpectrumLUTHeader.py).  The runtime no
//    longer reads any LUT file from disk — earlier revisions used
//    fopen() through the media-path locator, which silently failed
//    when the GUI was launched without RISE_MEDIA_PATH set,
//    leaving every spectral painter on the constant-0.5 fallback.
//
//    Lookup procedure:
