...
namespace RISE
{
	...
	class FireProductionFrozenSourcePacketSeal
	{
		...
		std::vector<float> beginningTemperatureK_;
	...
	public:
		...
		const std::vector<float>& BeginningTemperatureK() const
	...
	struct FireProductionScalarHeunFluxStage
	{
		...
		std::vector<float> physicalEnergyFluxWPerM2;
		std::array<std::vector<float>,3> physicalGasFluxKGPerM2S;
		std::array<std::vector<float>,3> advectiveGasLowFluxKGPerM2S;
		std::array<std::vector<float>,3> advectiveGasFluxDeltaKGPerM2S;
		std::string methaneRecordId;
		std::uint64_t attemptIdentity;
	...
	{
		std::array<std::size_t,3> packedFaceOffset;
		std::vector<float> donorAdvectiveFlux;
		...
		std::vector<float> advectiveFluxDelta;
		std::vector<float> mcMusclAdvectiveFlux;
		...
		std::vector<float> faceLogTemperature;
		std::vector<float> faceSensibleEnthalpyJPerKG;
		std::array<std::vector<float>,3> physicalGasFluxKGPerM2S;
		std::vector<float> lowCompositeFlux;
		std::vector<float> highCompositeFlux;
		std::uint32_t commandCommitCount;
		std::uint32_t interstageFullGridTransferCount;
		std::uint32_t terminalStagingCount;
		std::uint32_t branchObligationBitmap;
	...
	struct FireProductionResidentEOSCandidateComparatorRequest
	{
		...
		FireProductionResidentEOSCandidateComparatorRequest() :
			...
			producerPrecision(FireStateProducerPrecision::Binary32),candidateTimeStepS(0.0f),
	...
	{
		std::vector<float> tangentTargetPerS;
		std::vector<float> frozenSourceTargetPerS;
		std::vector<float> absoluteReferenceDiagnosticPerS;
		std::vector<float> monitoredAbsoluteReferenceTargetPerS;
		std::vector<float> assembledTargetPerS;
		//! Authenticated absolute same-unit radii around the device materializations.
		std::vector<float> tangentEnclosurePerS;
		std::vector<float> assembledEnclosurePerS;
		std::uint32_t commandCommitCount;
		std::uint32_t interstageFullGridTransferCount;
		std::uint32_t terminalStagingCount;
		std::uint32_t branchObligationBitmap;
