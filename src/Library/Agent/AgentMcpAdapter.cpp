			...
			JsonValue BuildToolsList( AgentAutonomy autonomy )
			{
				...
				// drift-class reason recorded on file_build_plan above.
				{
					...
					const std::string desc =
						...
						"the light palette with its grammar. At most 6 sources; a longer enumeration is "
						"cut to its first 6 and the result says so. For the model this is still ONE call "
						...
						"for special cases. A scene has a FREE BUDGET OF 2 zero-area lights; past "
						"it, each request to insert one is refused once with the count and the "
						"alternatives, and the identical request after that refusal is applied. "
						"shape_light, rect_light, an emissive object and hosek_wilkie_skylight are "
						"counted by nothing. ambient_light is NOT offered and is REFUSED on every "
