			...
			JsonValue BuildToolsList( AgentAutonomy autonomy )
			{
				...
				// read_viewport (Toolkit slice 1)
				{
					...
					tools.push_back( MakeTool( "read_viewport",
						...
						"control of layout or panes. When `available` is false there is no image. RETRIABLE "
						"reasons (they clear on their own -- one short retry is "
						...
						"scene is reopened; tell the user rather than retrying), \"output_provenance_unavailable\" "
						"(active fire media require a primary-plus-sidecar chain this viewport route cannot "
						"emit -- retrying or switching to the agent `render` route cannot fix it; use a "
						"provenance-capable file-output render or tell the user). available:false is a "
