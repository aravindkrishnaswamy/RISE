			...
			JsonValue BuildToolsList( AgentAutonomy autonomy )
			{
				...
				// recorded on file_build_plan above.
				{
					...
					const std::string desc =
						...
						"message says so. LEFT is a DRAFT (studio-preview shading: form and proportion, not "
						"appearance); RIGHT is the same object's MATERIALS under a fixed studio light rig that "
				...
				// render
				{
					...
					props.set( "quality", StringProp( "OPTIONAL, \"draft\" or \"production\" (default \"production\" -- today's exact behaviour). \"draft\" renders through a wholly SEPARATE, cheap studio-preview pipeline (the SAME fixed preview shader the GUI's live interactive editor uses) that IGNORES the scene's authored LIGHTING entirely -- a fixed synthetic studio rig stands in for it, so lighting, exposure, and colour balance are NOT representative. It DOES evaluate each material's diffuse albedo (geometry-only ambient occlusion times the BSDF's `albedo()`), so an expression-driven painter -- including one that queries `proximity`/`occlusion` -- DOES run, at 1 spp against the draft's fixed rig rather than the scene's real lights. Composition and camera framing are representative. NEVER judge materials/lighting/exposure/colour from a draft image -- use quality:\"production\" (or read_viewport) for that. A draft render CAPS samples at 4 regardless of the requested `samples` value. Check the result's `renderMode` field (\"production\"/\"draft\") to see which pipeline actually ran -- `integrator` always names the head's active PRODUCTION rasterizer regardless of `quality`, so it is NOT the field to check for this." ) );
