//////////////////////////////////////////////////////////////////////
//
//  Turbulence3DPainter.cpp - Implementation of a 3D turbulence
//  noise painter.  Uses absolute-value FBM for wispy, smoke-like
//  density patterns.
//
...
Turbulence3DPainter::Turbulence3DPainter(
...
RISEPel Turbulence3DPainter::GetColor( const RayIntersectionGeometric& ri ) const
{
	...
	// Turbulence output is already in [0, ~1] due to abs, clamp for safety
	if( d < 0.0 ) d = 0.0;
	if( d > 1.0 ) d = 1.0;
	return pColorInterp->InterpolateValues( a.GetColor(ri), b.GetColor(ri), d );
}

Scalar Turbulence3DPainter::GetColorNM( const RayIntersectionGeometric& ri, const Scalar nm ) const
{
	Scalar	d = pFunc->Evaluate( ri.ptIntersection.x*vScale.x+vShift.x, ri.ptIntersection.y*vScale.y+vShift.y, ri.ptIntersection.z*vScale.z+vShift.z );
	if( d < 0.0 ) d = 0.0;
	if( d > 1.0 ) d = 1.0;
...
IKeyframeParameter* Turbulence3DPainter::KeyframeFromParameters( const String& name, const String& value )
{
