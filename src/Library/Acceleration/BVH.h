...
namespace RISE
{
	...
	public:
		BVH( const TreeElementProcessor<Element>& ep_,
			...
			: hasFastFilter( false ),
			  ...
			  cfg( cfg_ )
		{
			// Tier 1 §1: pick builder.  Plain SAH (cfg.buildSBVH=false)
			// uses the in-place idx[] partition that was the Phase 1
			// path; SBVH (cfg.buildSBVH=true) uses Ref-based recursion
			// that supports spatial splits with reference duplication.
			if( cfg.buildSBVH ) {
				BuildSBVH( inputPrims );
			} else {
				Build( inputPrims );
			}
		...
		//  by construction in BuildRecursive / BuildSBVHNode), recomputing
		...
		//  is reordered + may have duplicates from SBVH).  For
	...
	protected:
		...
		//////////////////////////////////////////////////////////////////
		//  SBVH builder (Tier 1 §1).
		//
		//  Per-reference Ref struct lets a single primitive appear
		//  multiple times in the leaf array with each occurrence carrying
		//  its own clipped bbox.  Recursion is move-based: each call
		//  owns its `refs` vector, partitions into left/right (with
		//  possible duplication for spatial splits), and recurses on
		//  std::move'd halves.  Final DFS-ordered leaf primitive list
		//  is accumulated in `finalPrimOrder` and copied into `prims[]`
		//  at the end of build.
		//
		//  Spatial-split decision per node:
		//    1. Compute object-split SAH cost (centroid-based binning).
		//    2. Compute spatial-split SAH cost (extent-based binning,
		//       with straddler duplication overhead).
		//    3. Pick whichever is < leafCost; spatial only if budget
		//       allows.
		//
		//  Bbox clipping for straddlers: simple per-axis clip
		//  (max=splitVal on left side, min=splitVal on right side).
		//  Looser than true Sutherland-Hodgman polygon clipping but
		//  conservative-correct and an order of magnitude simpler.
		//////////////////////////////////////////////////////////////////

	public:
		struct Ref
		{
			uint32_t    primIdx;   // index into the original input prims list
			BoundingBox box;       // (potentially clipped) bbox for this reference
			Point3      centroid;  // midpoint of `box`
		};

	protected:
		// Mutable state during SBVH build.
		struct SBVHBuildState
		{
			std::vector<Element>  inputPrims;     // immutable; refs[i].primIdx indexes here
			std::vector<uint32_t> finalPrimOrder; // DFS-ordered leaf prim indices
			uint32_t              initialRefCount;
			uint32_t              maxRefCount;
			uint32_t              currentTotalRefs;
		};

		void BuildSBVH( const std::vector<Element>& inputPrims )
		{
			Timer t; t.start();

			GlobalLog()->PrintEx( eLog_Info,
				"BVH:: Building SBVH over %u primitives (binCount=%u, maxLeafSize=%u, dupBudget=%.2f)",
				(unsigned)inputPrims.size(), cfg.binCount, cfg.maxLeafSize, (double)cfg.sbvhDuplicationBudget );
			...
			// 1. Build root refs from input prims (one ref per prim, full bbox).
			std::vector<Ref> rootRefs;
			rootRefs.reserve( inputPrims.size() );
			BoundingBox rootBox(  Point3( RISE_INFINITY, RISE_INFINITY, RISE_INFINITY ),
			                      Point3(-RISE_INFINITY,-RISE_INFINITY,-RISE_INFINITY ) );
			BoundingBox rootCBox( Point3( RISE_INFINITY, RISE_INFINITY, RISE_INFINITY ),
			                      Point3(-RISE_INFINITY,-RISE_INFINITY,-RISE_INFINITY ) );
			for( uint32_t i = 0; i < inputPrims.size(); ++i ) {
				BoundingBox b = ep.GetElementBoundingBox( inputPrims[i] );
				const Point3 c( ( b.ll.x + b.ur.x ) * 0.5,
				                ( b.ll.y + b.ur.y ) * 0.5,
				                ( b.ll.z + b.ur.z ) * 0.5 );
				rootRefs.push_back( Ref{ i, b, c } );
				rootBox.Include( b.ll );
				rootBox.Include( b.ur );
				rootCBox.Include( c );
			}
			...
			// 2. Initialize build state with duplication budget.
			SBVHBuildState st;
			st.inputPrims      = inputPrims;
			st.initialRefCount = (uint32_t)rootRefs.size();
			st.maxRefCount     = (uint32_t)( ( 1.0 + cfg.sbvhDuplicationBudget ) * st.initialRefCount + 0.5 );
			st.currentTotalRefs = st.initialRefCount;
			st.finalPrimOrder.reserve( st.maxRefCount );

			// 3. Recursive build.
			nodes.reserve( inputPrims.size() * 2 );
			nodes.push_back( Node{} );
			BuildSBVHNode( 0, std::move( rootRefs ), st, rootBox, rootCBox );

			// 4. Reorder prims into DFS order, with duplicates
			//    expanded for SBVH-split leaves.
			prims.resize( st.finalPrimOrder.size() );
			for( size_t i = 0; i < st.finalPrimOrder.size(); ++i ) {
				prims[i] = inputPrims[ st.finalPrimOrder[i] ];
			}

			t.stop();
			const double dupPct = ( st.currentTotalRefs > st.initialRefCount )
				? ( 100.0 * (double)( st.currentTotalRefs - st.initialRefCount ) / (double)st.initialRefCount )
				: 0.0;
			GlobalLog()->PrintEx( eLog_Info,
				"BVH:: SBVH built %u nodes (%u leaves), %u refs (+%.1f%% dup), in %u ms",
				(unsigned)nodes.size(),
				(unsigned)CountLeaves(),
				(unsigned)st.finalPrimOrder.size(),
				dupPct,
				(unsigned)t.getInterval() );
		}

		// Per-node SBVH build: refs is move-in (this call owns it).
		void BuildSBVHNode(
			uint32_t           nodeIdx,
			std::vector<Ref>   refs,
			SBVHBuildState&    st,
			...
			const BoundingBox& centroidBox )
		{
			...
			// Tighten parent box to actual covered refs.
			BoundingBox tightBox(
				Point3( RISE_INFINITY, RISE_INFINITY, RISE_INFINITY ),
				Point3(-RISE_INFINITY,-RISE_INFINITY,-RISE_INFINITY ) );
			for( const Ref& r : refs ) {
				tightBox.Include( r.box.ll );
				tightBox.Include( r.box.ur );
			}
			(void)parentBox;
			SetNodeBox( nodes[nodeIdx], tightBox );

			// Leaf-by-size termination.
			if( count <= cfg.maxLeafSize ) {
				MakeSBVHLeaf( nodeIdx, refs, st );
				return;
			}
			...
			// ---------- Object-split SAH ----------
			Scalar  objBestCost  = RISE_INFINITY;
			int     objBestSplit = -1;
			{
				std::vector<Bin> bins( B );
				const Scalar invSpan = (Scalar)B / ( cMax - cMin );
				for( const Ref& r : refs ) {
					int bid = (int)( ( AxisVal( r.centroid, axis ) - cMin ) * invSpan );
					if( bid < 0 ) bid = 0;
					if( bid >= (int)B ) bid = B - 1;
					bins[bid].count++;
					bins[bid].box.Include( r.box.ll );
					bins[bid].box.Include( r.box.ur );
				}
				std::vector<BoundingBox> leftBox( B - 1, BoundingBox(
					Point3( RISE_INFINITY, RISE_INFINITY, RISE_INFINITY ),
					Point3(-RISE_INFINITY,-RISE_INFINITY,-RISE_INFINITY ) ) );
				...
				for( uint32_t i = 0; i + 1 < B; ++i ) {
					if( leftCount[i] == 0 || rightCount[i] == 0 ) continue;
					const Scalar cost = cfg.sahTraversalCost +
						cfg.sahIntersectionCost * (
							SurfaceArea( leftBox[i] ) * (Scalar)leftCount[i] +
							SurfaceArea( rightBox[i] ) * (Scalar)rightCount[i]
						) / parentArea;
			...
			// ---------- Spatial-split SAH ----------
			//
			// Bin per-ref bbox extents along the split axis (NOT centroids).
			// Each ref enters bin floor(ll) and exits bin floor(ur);
			// ref contributes its bbox to every bin in [enterBin, exitBin].
			// Spatial-split duplicates count straddlers on both sides.
			Scalar  spatBestCost   = RISE_INFINITY;
			int     spatBestSplit  = -1;
			Scalar  spatSplitVal   = 0.0;
			const Scalar pMin = AxisVal( tightBox.ll, axis );
			const Scalar pMax = AxisVal( tightBox.ur, axis );
			const bool   spatBudgetOK = ( st.currentTotalRefs < st.maxRefCount );
			if( spatBudgetOK && pMax - pMin > 1e-12 ) {
				std::vector<BoundingBox> binBox( B, BoundingBox(
					Point3( RISE_INFINITY, RISE_INFINITY, RISE_INFINITY ),
					Point3(-RISE_INFINITY,-RISE_INFINITY,-RISE_INFINITY ) ) );
				std::vector<uint32_t> binEnter( B, 0 );
				std::vector<uint32_t> binExit ( B, 0 );

				const Scalar invSpan = (Scalar)B / ( pMax - pMin );
				const Scalar binWidth = ( pMax - pMin ) / (Scalar)B;
				for( const Ref& r : refs ) {
					int b0 = (int)( ( AxisVal( r.box.ll, axis ) - pMin ) * invSpan );
					int b1 = (int)( ( AxisVal( r.box.ur, axis ) - pMin ) * invSpan );
					if( b0 < 0 ) b0 = 0;
					if( b0 >= (int)B ) b0 = B - 1;
					if( b1 < 0 ) b1 = 0;
					if( b1 >= (int)B ) b1 = B - 1;
					// Per-bin clipping: each bin gets the ref's bbox CLIPPED
					// to the bin's range along the split axis.  This is what
					// makes spatial-split SAH meaningful — without it, every
					// bin gets the full bbox and SAH always prefers the
					// non-duplicating object split.  Other axes are not
					// clipped (the ref's full extent on them survives).
					for( int b = b0; b <= b1; ++b ) {
						const Scalar binMin = pMin + binWidth * (Scalar)b;
						const Scalar binMax = pMin + binWidth * (Scalar)( b + 1 );
						BoundingBox clipped = r.box;
						const Scalar lo = AxisVal( clipped.ll, axis );
						const Scalar hi = AxisVal( clipped.ur, axis );
						SetAxis( clipped.ll, axis, ( lo > binMin ) ? lo : binMin );
						SetAxis( clipped.ur, axis, ( hi < binMax ) ? hi : binMax );
						binBox[b].Include( clipped.ll );
						binBox[b].Include( clipped.ur );
					}
					binEnter[b0]++;
					binExit [b1]++;
				}
				...
				BoundingBox accBox(
					Point3( RISE_INFINITY, RISE_INFINITY, RISE_INFINITY ),
					Point3(-RISE_INFINITY,-RISE_INFINITY,-RISE_INFINITY ) );
				uint32_t enteredSoFar = 0;
				for( uint32_t i = 0; i + 1 < B; ++i ) {
					if( binEnter[i] > 0 || binExit[i] > 0 ) {
						accBox.Include( binBox[i].ll );
						accBox.Include( binBox[i].ur );
					}
					enteredSoFar += binEnter[i];
					leftBox[i]   = accBox;
					leftCount[i] = enteredSoFar;
				}
				accBox = BoundingBox(
					Point3( RISE_INFINITY, RISE_INFINITY, RISE_INFINITY ),
					Point3(-RISE_INFINITY,-RISE_INFINITY,-RISE_INFINITY ) );
				uint32_t exitedSoFar = 0;
				for( int i = (int)B - 1; i >= 1; --i ) {
					if( binEnter[i] > 0 || binExit[i] > 0 ) {
						accBox.Include( binBox[i].ll );
						accBox.Include( binBox[i].ur );
					}
					exitedSoFar += binExit[i];
					rightBox[i - 1]   = accBox;
					rightCount[i - 1] = exitedSoFar;
				}

				for( uint32_t i = 0; i + 1 < B; ++i ) {
					if( leftCount[i] == 0 || rightCount[i] == 0 ) continue;
					// Add cost of duplicates: anything in [0..i] that exits
					// past i is duplicated.  Approximation: leftCount + rightCount
					// already counts straddlers twice (entered ≤ i, exited > i),
					// so cost terms are correct.
					const Scalar cost = cfg.sahTraversalCost +
						cfg.sahIntersectionCost * (
							SurfaceArea( leftBox[i] ) * (Scalar)leftCount[i] +
							SurfaceArea( rightBox[i] ) * (Scalar)rightCount[i]
						) / parentArea;
					if( cost < spatBestCost ) {
						spatBestCost  = cost;
						spatBestSplit = (int)i;
					}
				}
				if( spatBestSplit >= 0 ) {
					spatSplitVal = pMin + binWidth * (Scalar)( spatBestSplit + 1 );
				}
			}

			// ---------- Choose split or leaf ----------
			const bool objWins  = ( objBestSplit >= 0 ) && ( objBestCost  < leafCost );
			const bool spatWins = ( spatBestSplit >= 0 ) && ( spatBestCost < leafCost );
			if( !objWins && !spatWins ) {
				MakeSBVHLeaf( nodeIdx, refs, st );
				return;
			}
			const bool useSpatial = spatWins && ( !objWins || spatBestCost < objBestCost );
			...
			if( useSpatial ) {
				const Scalar splitVal = spatSplitVal;
				leftRefs.reserve( count );
				rightRefs.reserve( count );
				for( const Ref& r : refs ) {
					const Scalar lo = AxisVal( r.box.ll, axis );
					const Scalar hi = AxisVal( r.box.ur, axis );
					if( hi <= splitVal ) {
						leftRefs.push_back( r );
						leftCBox.Include( r.centroid );
					} else if( lo >= splitVal ) {
						rightRefs.push_back( r );
						rightCBox.Include( r.centroid );
					} else {
						// Straddler — clip box to each side and duplicate.
						Ref lr = r;
						Ref rr = r;
						SetAxis( lr.box.ur, axis, splitVal );
						SetAxis( rr.box.ll, axis, splitVal );
						lr.centroid = Point3(
							( lr.box.ll.x + lr.box.ur.x ) * 0.5,
							( lr.box.ll.y + lr.box.ur.y ) * 0.5,
							( lr.box.ll.z + lr.box.ur.z ) * 0.5 );
						rr.centroid = Point3(
							( rr.box.ll.x + rr.box.ur.x ) * 0.5,
							( rr.box.ll.y + rr.box.ur.y ) * 0.5,
							( rr.box.ll.z + rr.box.ur.z ) * 0.5 );
						leftRefs.push_back( lr );
						rightRefs.push_back( rr );
						leftCBox.Include( lr.centroid );
						rightCBox.Include( rr.centroid );
						st.currentTotalRefs++;  // one extra ref
					}
				}
				if( leftRefs.empty() || rightRefs.empty() ) {
					// Pathological — both sides should be non-empty by SAH
					// selection, but defend against numeric edge cases.
					MakeSBVHLeaf( nodeIdx, refs, st );
					return;
				}
			} else {
				const Scalar splitVal = cMin + ( cMax - cMin ) * (Scalar)( objBestSplit + 1 ) / (Scalar)B;
				leftRefs.reserve( count );
				rightRefs.reserve( count );
				for( const Ref& r : refs ) {
					if( AxisVal( r.centroid, axis ) < splitVal ) {
						leftRefs.push_back( r );
						leftCBox.Include( r.centroid );
					} else {
						rightRefs.push_back( r );
				...
				if( leftRefs.empty() || rightRefs.empty() ) {
					// Median fallback (matches Phase 1 BuildRecursive behavior).
					leftRefs.clear();  rightRefs.clear();
					std::vector<Ref> sorted = refs;
					std::sort( sorted.begin(), sorted.end(),
						[axis]( const Ref& a, const Ref& b ){
							return AxisVal( a.centroid, axis ) < AxisVal( b.centroid, axis );
						});
					const size_t mid = sorted.size() / 2;
					leftRefs.assign(  sorted.begin(),       sorted.begin() + mid );
					rightRefs.assign( sorted.begin() + mid, sorted.end()         );
					leftCBox = BoundingBox(
						Point3( RISE_INFINITY, RISE_INFINITY, RISE_INFINITY ),
						Point3(-RISE_INFINITY,-RISE_INFINITY,-RISE_INFINITY ) );
					rightCBox = leftCBox;
					for( const Ref& r : leftRefs )  leftCBox.Include(  r.centroid );
					for( const Ref& r : rightRefs ) rightCBox.Include( r.centroid );
				}
			}

			// Free `refs` storage now that left/right are populated.
			refs.clear();
			refs.shrink_to_fit();

			// Allocate two children (contiguous) and recurse.
			const uint32_t leftIdx = (uint32_t)nodes.size();
			nodes.push_back( Node{} );
			nodes.push_back( Node{} );
			nodes[nodeIdx].firstPrimOrLeft = leftIdx;
			...
			BuildSBVHNode( leftIdx,     std::move( leftRefs  ), st, tightBox, leftCBox  );
			BuildSBVHNode( leftIdx + 1, std::move( rightRefs ), st, tightBox, rightCBox );
		}

		void MakeSBVHLeaf( uint32_t nodeIdx, const std::vector<Ref>& refs, SBVHBuildState& st )
		{
			const uint32_t firstPrim = (uint32_t)st.finalPrimOrder.size();
			for( const Ref& r : refs ) {
				st.finalPrimOrder.push_back( r.primIdx );
			}
			nodes[nodeIdx].firstPrimOrLeft = firstPrim;
			nodes[nodeIdx].primCount       = (uint16_t)refs.size();
			nodes[nodeIdx].splitAxis       = 0;
		}

		// Per-axis coordinate setter for the bbox-clipping path.
		static inline void SetAxis( Point3& p, uint8_t axis, Scalar val )
		{
			if( axis == 0 ) p.x = val;
			else if( axis == 1 ) p.y = val;
			else                 p.z = val;
		}
