//***************************************************************************
// rise3dsmax - [scontext.cpp] RISE Renderer for 3D Studio MAX.
// 
...
SContext::SContext(RiseRenderer* r, BGContext* bgc, MAXBSDF* bsdf)
{
	...
	sld = new RISEShadowingLightDesc();
}
...
void SContext::CalcNormals()
{
	...
	for (int i = 0, cc = 2; i < 3; i++, cc--) {
		...
		// to a smoothing group
			...
			vxNormals[i] = VectorTransform(pInst->normalObjToCam, pInst->mesh->getFaceNormal(faceNum));
		}
	}
	...
	normal = Normalize(baryCoord.x*vxNormals[0] + baryCoord.y*vxNormals[1] + baryCoord.z*vxNormals[2]);
}
...
void SContext::SetHitPos(const Point3& p)
{
	hitPos = p;
}

void SContext::SetViewDir(const Point3& v)
{
	viewDir = v;
}
...
// Set normal is used by material after computing bump values
void SContext::SetNormal( Point3 p )
{
	normal = p;
}
...
Point3 SContext::GNormal(void)
{
	...
	return VectorTransform(renderer->view.affineTM, pInst->mesh->getFaceNormal(faceNum) );
}
...
{
	if( pBSDF ) {
		// The BSDF is the light desc
		return pBSDF;
	} else {
		RenderLight* rLight = (RenderLight*)renderer->lightTab[lightNum];
		ExclList* excList = rLight->pLight ? rLight->pLight->GetExclList() : 0;

		// See if this instance of this INode is on the exclusion list
		if( excList ) {
			for( int i=0; i<excList->Count(); i++ ) {
				if( (*excList)[i] == pInst->GetINode() ) {
					// We are on the list
					return &nullLight;
				}
			}
		...
		// If the light is supposed to be shadowed, pass the sld
		if( rLight->pLight ) {
			if( rLight->pLight->GetShadowMethod() == LIGHTSHADOW_MAPPED ||
				rLight->pLight->GetShadowMethod() == LIGHTSHADOW_RAYTRACED ) {
				sld->pCaster = pRayCaster;
				sld->pLightDesc = rLight->pDesc;
				sld->affineTM = affineTM;
				sld->worldPtIntersection = worldPtIntersection;
				(LightDesc&)*sld = *rLight->pDesc;
				return sld;
			}
		}
...
int SContext::BumpBasisVectors(Point3 dP[2], int axis, int channel)
{ 
	getTVerts(channel);
	calc_size_ratio();
	Point3 bv[2];
	...
	ComputeBumpVec2D(axis, tv[channel], obpos, bv);
	bumpv[channel][0] = Normalize(bv[0]);
	bumpv[channel][1] = Normalize(bv[1]);
	dP[0] = bumpv[channel][0];
	dP[1] = bumpv[channel][1];
...
BGContext::BGContext(RiseRendererParams* rpar) { 
...
INode *RISEMapsContext::GetNode()
{
	return inst->pNode;
}

int RISEMapsContext::NodeRenderID()
{
	return inst->nodeID;
}

void RISEMapsContext::GetCurrentViewParams(ViewParams &vp)
{
	vp = renderer->view;
}

void RISEMapsContext::GetSubRendParams(SubRendParams &srp)
{
	srp.rendType	= renderer->rendParams.rendType;
	srp.fieldRender	= renderer->rendParams.bRenderFields;
	srp.evenLines	= 0;
	srp.doingMirror	= 0;
	srp.doEnvMap	= 0;
	srp.devWidth	= renderer->rendParams.devWidth;
	srp.devHeight	= renderer->rendParams.devHeight;
	srp.devAspect	= renderer->rendParams.devAspect;
	srp.xorg		= 0;
	srp.yorg		= 0;
	srp.xmin		= renderer->rendParams.nMinx;
	srp.xmax		= renderer->rendParams.nMaxx;
	srp.ymin		= renderer->rendParams.nMiny;
	srp.ymax		= renderer->rendParams.nMaxy;
}

int RISEMapsContext::SubMtlIndex()
{
	return subMtl;
}

void RISEMapsContext::SetSubMtlIndex(int mindex)
{
	subMtl = mindex;
}

void RISEMapsContext::FindMtlPlane(float pl[4])
{
}

void RISEMapsContext::FindMtlScreenBox(Rect &sbox, Matrix3* viewTM,int mtlIndex)
{
}

Box3 RISEMapsContext::CameraSpaceBoundingBox()
{
	return inst->camBox;
}

Box3 RISEMapsContext::ObjectSpaceBoundingBox()
{
	return Box3();// TBD
}

Matrix3 RISEMapsContext::ObjectToWorldTM()
{
	return inst->objToWorld;
}

RenderGlobalContext *RISEMapsContext::GetGlobalContext()
{
	return &renderer->rendParams;
}

int RISEMapsContext::Render(Bitmap *bm, ViewParams &vp, SubRendParams &srp, Point4 *clipPlanes, int nClipPlanes)
{
	/*
	RiseRendererParams riseparams;
	// Setup ViewParams:
	riseparams.devWidth = srp.devWidth;
	riseparams.devHeight = srp.devHeight;
	riseparams.devAspect = srp.devAspect;
	riseparams.projType = renderer->rendParams.projType;

	riseparams.ComputeViewParams(vp);

	riseparams.nMinx = srp.xmin;
	riseparams.nMiny = srp.ymin;
	riseparams.nMaxx = srp.xmax;
	riseparams.nMaxy = srp.ymax;

	riseparams.renderer = renderer;

	return renderer->RenderImage(riseparams, renderer->rendParams.time, bm, NULL, 0);
	*/
	return 1;
}
