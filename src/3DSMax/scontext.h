...
class RISEShadowingLightDesc;

// ShadeContext for evaluating materials
class SContext : public ShadeContext {
public:
	SContext(RiseRenderer* r, BGContext* bgc, MAXBSDF* pBSDF);
	virtual ~SContext( );
	...
	void DPdUVW(Point3 [3], int channel);
	...
	void	SetHitPos(const Point3& p);
	void	SetViewDir(const Point3& v);
	void	CalcNormals();
	void	SetCamPos(const Point3& p) { cameraPos = p; }
	...
	// RISE specific stuff
	const RISE::IRayCaster*		pRayCaster;			// Valid only between when it is set in the MAXShaderOp:Shade function and when the function exits
	RISEShadowingLightDesc* sld;
	Matrix3 affineTM;
...
private:
	...
	// The MAX BSDF, if we are using this context for computing the BSDF
	// value rather than the full shading value
	MAXBSDF*	pBSDF;

	// The null light basically does nothing
	NullObjLight nullLight;
};
...
#define FARZ -1.0e10f
 ...
 class BGContext: public ShadeContext {
	public:
		...
		BGContext(RiseRendererParams *rpar);
...
#include "MAXBSDF.h"
#include "RISEShadowingLightDesc.h"
